\chapter[EHI  Erste Hilfe und erweiterte Erste Hilfe]{\fbox{EHI}\\Erste
Hilfe und erweiterte Erste Hilfe}
\label{chp:EHI}
...
\Z{Unzumutbar ist die Erste Hilfe für den
...
Am Beginn stehen die \Es{Lebensrettenden Sofortmaßnahmen}, welche noch in
\E{Allgemeine Sofortmaßnahmen} und in am Patienten gesetzten \E{Lebensrettenden
Sofortmaßnahmen} unterteilt werden können. Zu den \E{Allgemeinen Sofortmaßnahmen} zählt 
...
\E{Lebensrettenden Sofortmaßnahmen} zählt die stabile Seitenlage bei Bewusstlosigkeit, die
...
Sofortmaßnahmen wird der \Es{Notruf} abgesetzt, falls dies
noch nicht durch eine 2. Person geschehen ist.

Anschließend wird 
\Es{weitere Erste Hilfe} geleistet, wie zum Beispiel die
Versorgung von einfacheren Verletzungen und die psychologische Betreuung des
Patienten. Der eintreffende Rettungsdienst übernimmt den Patienten für die \Es{Sanitätshilfe} und führt den
\Es{Transport} durch. Im Spital erfolgt schließlich die
\Es{ärztliche Betreuung} des Patienten. 
...
durchatmen und sich danach einen \Es{Überblick} über die
...
Bei der Überblickgewinnung kann der Ersthelfer \Es{Gefahren erkennen}.
Diese kann für den Patienten, den Ersthelfer und dazukommende Dritte bestehen.
Der Ersthelfer muss \Es{Absperrmaßnahmen durchführen} und
...
evt. \Es{Spezialkräfte} angefordert und ihnen sämtliche
...
Fahrzeug in einem angemessenen \Es{Sicherheitsabstand} zum Gefahrenbereich an.
Besondere \Es{Gefahrenquellen} wie Kurven,
...
Hilfsmittel, wie \Es{Warnleuchte} oder \Es{Warndreieck}, sowie
...
eine \Es{Warnweste} anzuziehen und mit bereits
geöffnetem \Es{Warndreieck} (Pannendreieck) dem 
...
aufzustellen:
\begin{itemize}
  \item \E{Ortsgebiet}: unmittelbar bis zu ca. 50\,m
  \item \E{Freilandstraße}: ca. 150\,m
  \item \E{Autobahn}: ca. 250-400\,m
\end{itemize}
}{
\begin{itemize}
  \item \E{Ortsgebiet}: unmittelbar bis zu ca. 50\,m
...
Gewicht des Zuges, ca. 600-800\,m). Das \Es{Haltesignal} für den Lokführer ist
eine \Es{kreisende Armbewegung}. Dadurch wird er gewarnt und leitet 
...
% Als Beispiel zu nennen wäre ein noch eingeschaltetes \Es{Elektrowerkzeug}
...
Ein beginnender \Es{Fahrzeugbrand} kann unter Umständen
durch stoßweises Sprühen direkt auf den Brandherd mit einem Feuerlöscher bekämpft
werden. Sollten \Es{Kleidungsstücke} brennen, werden die Flammen
...
Achtung beim \Es{Zimmerbrand}! Die
...
In mit \Es{Gas gefüllten Räumen}\footnote{Austreten von Gas aus der
...
Bei einem \Es{Stromunfall} wird zwischen
...
Viele Gefahren laueren aber auch bei \Es{Freizeit- und Sportunfällen}. Zum
...
Wenn ein \Es{Verschütteter} gerettet werden soll, muss der Ersthelfer darauf
...
Eine Besonderheit stellen \Es{Gefahrenguttransporte} dar
...
\Fazit{Der Helm ist immer abzunehmen!}

Am besten wird der Sturzhelm von zwei Helfern abgenommen
(\E{Zweihelfermethode}), zur Not funktioniert es jedoch auch mit nur einem
Helfer (\E{Einhelfermethode}). Das Wichtigste ist, dass
...
\\
Schritt 4 
\ACh{Instruktor 1}{0.8.0}{Weg lassen. }\ACh{GABS}{2010-02-19}{Helm kippen bis
Nasenspitze sichtbar?} &
% Schritt 5 
Kopf stabilisieren, Helm abziehen.
&
...
% entwickelt.}
% 
% \Es{Der Helm ist immer abzunehmen, außer der Patient verweigert dies!}
% 
% {
...
das \Es{Bewusstsein} und die \Es{Atmung}. 
...
laute und deutliche \Es{Ansprechen} des Patienten. Idealerweise mit einfachen
Anweisungen wie:
...
Gleichzeitig kann man den Patienten an seinen Schultern \Es{berühren} und
leicht rütteln.
...
Bereich des Kopfes ein \Es{Schmerzreiz} gesetzt werden. Jede Reaktion wird als
Lebenszeichen gewertet. Erfolgt keine Reaktion muss der
Ersthelfer \Es{um Hilfe rufen}, oder besser -- wenn
...
\Es{Atemwege freimachen}. Im Verdachtsfall wird die
Mundhöhle auf Fremdkörper kontrolliert. Anschließend wird der Kopf
nackenwärts \Es{überstreckt}, damit die zurückfallende
Zunge die Atemwege nicht blockiert. Nun muss die
Atemkontrolle durch \Es{Hören}, \Es{Sehen} und
\Es{Fühlen} durchgeführt werden. Dafür wird das Ohr über
...
Ersthelfer zu der Erkenntnis kommen, dass der Patient zwar \Es{kein Bewusstsein} hat, aber 
eine \Es{ausreichende Atmung} und daher einen \Es{vorhandenen Kreislauf}. 
In einer solchen Situation liegt die Notfalldiagnose \T{Bewusstlosigkeit} vor. Der
...
Handgriffen eine \T{stabile Seitenlage} herzustellen.
\begin{enumerate}
...
des Gesundheitszustandes zu vermeiden. Sehr wichtig ist der \Es{Wärmeerhalt}. Der
...
\Es{laufende Kontrolle der Atmung} durchgeführt werden. Der Ersthelfer hat
...
geschlossenen Räumen, wird dem Patienten die benötigte \Es{Frischluft}
zugeführt. Auch der \Es{psychischen Betreuung} des Patienten
...
Nach der Kontrolle der Vitalfunktionen, kann der
Ersthelfer zum Ergebnis kommen, dass der Patient \Es{kein Bewusstsein} und 
\Es{keine normale Atmung} hat, d.h. \Es{kein Kreislauf vorhanden} ist.
In einer solchen Situation liegt die Notfalldiagnose \T{Atem- und
...
% \Es{Herz-Lungen-Wiederbelebung (Reanimation)} begonnen werden. Diese dient der
...
% Teilschritten, der \Es{Herzdruckmassage} zur Nachahmung einer
% Herztätigkeit und der \Es{Beatmung} zur Zuführung von Sauerstoff in die
% Lungen.
% }{
% \begin{itemize}
%     \item Herz-Lungen-Wiederbelebung (Reanimation)
...
\Layout{0}{Technik}
{
\Es{Position und Druckpunkt} Der Patient muss auf einem
...
auf. Dieser liegt in der \Es{Mitte des Brustkorbs}.

\Es{Arm- und Körperhaltung} Die richtige
...
Das Schlimmste, das der Ersthelfer falsch machen kann, ist
 ...
 einen oder anderen Fehler kommen. Ein Bett ist \Es{keine
 feste Unterlage}! Es darf nicht vergessen werden, den
 Patienten auf den Boden oder eine andere harte Unterlage zu
 legen.

Grundsätzlich
kann es auch zu \Es{Verletzungen durch den Ersthelfer} kommen. Dazu zählt der
...
\Es{ungenügenden Zirkulation} des Blutkreislaufs. 
...
{
Die \Es{Atemfrequenz} des Erwachsenen beträgt 12-16\,\nicefrac{x}{min}, d.h.
wir atmen 12-16\,\nicefrac{x}{min} ein und aus. Das \Es{\underline{max.} (!)
...
Atemzug beträgt jedoch nur etwa 500\,ml (vgl. \See{topic:atemvolumina}).
...
21\,\%. Der \ce{O2}-Gehalt in der \Es{Ausatemluft} beträgt
...
{
Ein \Es{zu hohes Atemzugsvolumen} führt zu einem
höheren Druck im Nasen-Rachenraum, sobald die Lunge
vollständig mit Luft befüllt ist. Die Luft findet nur den
Ausweg über die Speiseröhre in den Magen. Dies kann während einer Herz-Lungen-Wiederbelebung meist unbemerkt geschehen. Bei einem \Es{zu niedrigem Atemzugsvolumen} ist die
\ce{O2}-Versorgung nicht sichergestellt, da die Luft nicht bis in die
Lunge kommt. Bei Bartträgern kann es zu einer
\Es{ungenügenden Abdichtung} kommen. Eine \Es{zu hohe
Atemfrequenz} hindert den Patienten am Ausatmen. Wenn es beim Freimachen
der Atemwege zu einer \Es{mangelnden Überstreckung} des Kopfes
kommt, kann es zu einer unzureichenden Belüftung der Lunge kommen. Bei jeder Atemspende ist der \Es{Brustkorb} des Patienten zu \Es{beobachten}. Dieser
muss sich deutlich heben und senken. Immer auch auf den \Es{Selbstschutz}
...
}\ACh{GABS}{2010-02-19}{Wieso? Hinweis auf AED wär doch sinnvoll.} 
&
\\\addlinespace\bottomrule
\end{tabularx}
...
\begin{description}
  \item[Eintreffen von \Es{kompetenter Hilfe.}] Der Ersthelfer darf aber beim
  ...
  \item[Der Patient zeigt \Es{Lebenszeichen}.] Entweder
  ...
  bzw. atmet beim Beatmungsvorgang dem Ersthelfer entgegen. \Es{Erbrechen zählt \E{nicht} als Lebenszeichen.}
...
der Patient \Es{Lebenszeichen}, ist eine
...
Bei Eintreffen von \Es{kompetenter Hilfe} muss der Ersthelfer
dem Rettungspersonal Auskunft über relevante Umstände und
die gesetzten Maßnahmen geben. 

Wenn der \Es{Ersthelfer erschöpft} ist, sollte er (falls überhaupt
...
% quellend bei venösen Blutungen sein. Als \Es{lebensrettende Sofortmassnahme}
% dient die \Es{Blutstillung}. 
...
\Layout{0}{Blutstillung durch Zudrücken
}{
Mit einem \Es{direkten Druck in die stark blutende Wunde} mittels Finger,
Handballen oder Faust -- je nach Größe der Verletzung -- mit einer Wundauflage 
kann diese gestoppt werden. Ein \Es{Hochhalten der verletzten Extremität} verstärkt
die Wirkung. Dies ist auch die \Es{einzige Möglichkeit Halsschlagaderblutungen zu
...
\Es{sterilen Wundauflage}, einem \Es{Druckkörper} \footnote{Druckkörper: Als Druckkörper
...
\Es{Befestigungsmittel} \footnote{Befestigungsmittel: Als Befestigungsmittel eignet sich eine
Dreiecktuchkrawatte oder eine Mullbinde}. Ein \Es{Hochhalten der verletzten
...
Die Abbindung ist die \Ca{letzte Wahl!}}
 
Das Abbinden \Es{funktioniert} aus anatomischen Gründen
\Es{nur am Oberarm und Oberschenkel}, denn dort befindet
sich nur ein Knochen, gegen den sämtliche Blutgefäße
gedrückt werden können. Dazu darf nur ein \Es{breites und
...
der Extremität. Für die weitere Versorgung des Patienten ist es wichtig, den \Es{Zeitpunkt der Abbindung} zu notieren, am besten gleich am Oberarm bzw.
...
{
Die abgetrennten Gliedmaßen sind \Es{immer in das Spital mitzunehmen}. Nach
Versorgung des Patienten ist das \Es{Amputat steril} zu \Es{verbinden}. Für
den Transport gilt es \Es{kühl und trocken} zu \Es{lagern}. Dazu eignen
...
{
Der Schock ist eine \Es{lebensbedrohliche Kreislaufstörung}, bei der es zu einer
\Es{Unterversorgung der lebenswichtigen Organe}, wie Hirn,
...
bei den lebenswichtigen Organen führt zu einer \Es{Schädigung der Organe}, die
in weiterer Folge zum \Es{Versagen der Organe} führt. Der Schock
...
Der Patient kann eine \Es{blasse Haut} haben. Weiters besteht die Möglichkeit, dass er
\Es{zittert und friert}, aber paradoxerweise dazu einen
\Es{kalten, klebrigen Schweiß} auf der Stirn hat. Der Patient kann außerdem \Es{verwirrt, unruhig
und ängstlich} sein. Er besitzt eine \Es{beschleunigte, flache Atmung} und
einen \Es{beschleunigten, kaum tastbaren Puls}.
}{
\begin{itemize}
...
% werden. Z.B. die \Es{Ursache beseitgen} kann der Ersthelfer nur bei
% Blutverlust mittels Blutstillung. \Es{Beengende Kleidungsstücke öffnen} kann
% dagegen immer helfen. Das wichtigste ist jedoch die \Es{situationsgerechte
...
% \Es{Schutz vor Wärmeverlust}, ein \Es{beruhigender Zuspruch} und die
% \Es{Beobachtung} des Patienten sind Massnahmen, die der Ersthelfer bei jedem
...
Da der Schock aus drei verschiedenen Ursachen entstehen kann, muss auch
in der Behandlung ursachengerecht und damit differenziert vorgegangen
werden.  Das wichtigste ist jedoch die \Es{situationsgerechte
Lagerung} des Patienten. Bei Bewusstlosigkeit stabile Seitenlage, bei Blut- und
...
lauten wie folgt:
\Es{\Code{122} Feuerwehr}, \Es{\Code{133} Polizei} und \Es{\Code{144}
Rettung}. Vor einigen Jahren kam durch die EU eine weitere Notrufnummer dazu:
\Es{\Code{112} Euro-Notruf}. Die Besonderheit dieser Nummer liegt darin, dass
...
% Notrufnummern. Die beiden relevantesten sind: \Es{\Code{01/406 43 43}
% Vergiftungsinformationszentrale} und \Es{\Code{141} Ärztefunkdienst}. 
}{
\begin{itemize}
...
Die
\Es{Vergiftungsinformationszentrale} (VIZ) dient nur dazu
Auskunft über die Risiken und Behandlungen von sicher festgestellten Vergiftungen zu
geben (\E{Expertenrat}). Der Anruf bei der VIZ erspart
nicht einen Anruf beim Rettungsdienst.

Der \Es{Ärztefunkdienst} (\Abk{ÄFD})
...
{
Bagatellverletzungen sind \Es{kleine, oberflächige Wunden}, wie z.B.
...
\begin{enumerate}
    \item \Es{Es kommt nichts in die Wunde
...
\item \Es{Es kommt nichts aus der Wunde heraus.}
...
Infektionsgefahr
\\\addlinespace
\noindent\Es{Insektenstiche}
&
Bei allergischen Reaktionen oder bekannter entsprechender Allergie
\\\addlinespace
\noindent\Es{Schusswunden}
&

\\\addlinespace
\noindent\Es{Wunden im Genitalbereich}
&
...
Schweregrad der Schädigung der Haut eingeteilt werden. Dies geschieht mittels drei Grade. Beim \Es{1.~Grad} kommt es zur Rötung der Haut, einer
Schwellung des Gewebes und zu Schmerzen. Der \Es{2.~Grad} ist charakterisiert
durch Blasenbildung. Es bestehen weiterhin
Schmerzen. Beim \Es{3.~Grad} kommte es zur Schorfbildung. Die Schmerzen hier sind abhängig von der Tiefe der Schädigung
der Haut.
}{
\begin{itemize}
...
besagt, dass die \Es{Handfläche des Patienten  1\,\%}
...
Bei Kindern ist ab \Es{5\,\%}, bei Erwachsenen ab \Es{10\,\%}
...
\Layout{0}{\TxSymptome{} und Kennzeichen}
{
Am Beginn hat der Patient \Es{Schmerzen}, Atem- und Herzfrequenz sind erhöht.
Je weiter die Temperatur sinkt, desto \Es{teilnahmsloser}
und \Es{müder} wird der Patient. Zu Letzt kommt es zum \Es{Absinken
...
\Layout{0}{\TxSymptome{} und Kennzeichen}
{
Der Patient hat \Es{Schmerzen} im Bereich des betroffenen Gelenkes, es
kann auch zu einer \Es{Schwellung} rund um die geschädigte Stelle kommen. Mit
unter nicht gleich vorhanden, sondern erst am nächsten Tag ersichtlich,
kann sich auch ein \Es{Bluterguss} bilden. Auch eine \Es{Einschränkung der
...
\Layout{0}{\TxSymptome{} und Kennzeichen}
{
Der Patient hat starke \Es{Schmerzen} im Bereich des verletzten Gelenkes,
die betroffene Extremität kann nicht mehr bewegt werden
(\Es{Bewegungsunfähigkeit}). Außerdem kommt es zu einer \Es{abnormen
Stellung}, welche \Es{federnd fixiert} gelagert ist. 
}{
\begin{itemize}
    \item Schmerzen.
...
\Es{geschlossenen Knochenbruch}, bei dem die Haut über der Bruchstelle noch
intakt ist. Und dem \Es{offenen Knochenbruch}, bei dem auf Grund der
...
\Layout{0}{\TxSymptome{} und Kennzeichen}
{
Es gibt \E{sichere} und \E{unsichere} Kennzeichen für einen Knochenbruch. Zu
den \T{sicheren Kennzeichen} zählt die \Es{Fehlstellung} (abnorme Stellung,
Verkürzung, Treppenbildung), die \Es{abnorme Beweglichkeit} und die
\Es{Krepitation} (das hörbare und fühlbare Aneinanderreiben der Knochen). \T{Unsichere Kennzeichen} sind \Es{Schmerzen}, \Es{Schwellungen}, \Es{Bewegungseinschränkungen} und \Es{Blutergüsse}. }{
\begin{itemize}
...
Knochenbruchs ist daher der \Es{Blutverlust}. Je größer der Blutverlust ist
umso größer ist wiederum die Gefahr, dass der Patient in den \Es{Schock}
gerät. Bei einem offenen Knochenbruch besteht außerdem die Gefahr einer \Es{Wundinfektion}. Bei Rippenbrüchen kann
es zu \Es{Begleitverletzungen innerer Organe} kommen. 
}{
\begin{itemize}
    \item Blutverlust.
    \item Schock.
...
unterschiedliche Kennzeichen. Es kann mit \Es{Rauschzuständen} und
\Es{Erbrechen bzw. Durchfall} beginnen, über \Es{Bewusstseinsstörung} zu
\Es{Atem- und Kreislaufstörung} führen.
}{
\begin{itemize}
    \item Rauschzustände
