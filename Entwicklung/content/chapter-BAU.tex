...
\section{\HeadingKeyword{Vitalwerte}}
...
dass es zu einer Unterversorgung des Gehirns mit Blut kommen
kann.
}
{%&
\begin{itemize}
...
% TODO Grassl Foto: Ermittlung der Pulsfrequenz (Radialispuls, Auszählen, Pulsoxy)
...
\subsection{Messung des \HeadingKeyword{Blutdrucks}}
...
(\I[Auswurfphase!Blutdruckmessung]{Auswurfphase}, 
\I[Systole!Blutdruckmessung]{Systole}) 
erreicht er sein Maximum,
und sinkt danach in der 
\I[Füllungsphase!Blutdruckmessung]{Füllungsphase} 
des Herzens
(\I[Diastole!Blutdruckmessung]{Diastole}) 
wieder ab, 
bis das Herz neuerlich Blut auswirft.
Dementsprechend können zwei Werte erhoben werden: Der
\begin{itemize}
  \item \II[Blutdruck!systolischer!Messung]{Systolische Blutdruck}
  (Spitzendruck), und der
  \item \II[Blutdruck!diastolischer!Messung]{Diastolische Blutdruck}
  (Minimaldruck).
\end{itemize}
Im Normalfall
...
(\T{mm\,Hg}, 
\ce{Hg} ist das chemische Symbol für Quecksilber).%
\index{Millimeter Quecksilbersäule}

Siehe auch 
\FullPrettyRef{topic:mechanische-herzfunktion} 
(Systole, Diastole, mechanische Herzfunktion), 
und \FullPrettyRef{topic:blutdruck} 
(Blutdruck).
}{
\begin{itemize}
...
\subsubsection{\HeadingKeyword{Manuelle Blutdruckmessgeräte}}
...
einem Doppel- und einem Einzelkopfstethoskop}%1 Titel
{}%2 Beschreibung
{}%3 Label
{Ch. Pallinger}%4 Quelle
{MfG}%5 Lizenz
{./images/pallinger-christoph-aass/Blutdruck_32768_export_01024.jpg}%6 Datei
{}%7 Breitenmodifizierer
{}%8 Float
% vormals \Layout{37}

\subsubsection{Exkurs: \HeadingKeyword{Stethoskop}}
...
\subsubsection{Bludruckmessung nach \HeadingKeyword{Riva-Rocci}}
...
\begin{Synopsis}
  \SynopsisItem{Der \E{diastolische} Blutdruckwert 
  kann nur bei der \E{auskultatorischen} Methode ermittelt werden!}
\end{Synopsis}


\TextSlide{Auskultatorische Methode}
{%
...
verschwindet, ist der \EE{diastolische Wert}. Es
kann vorkommen, dass das Pochen nicht verschwindet, sondern
...
% TODO Grassl Foto: Blutdruckmessung, Nagelbettprobe


\Picture
%[]% Float
{Prinzip
    der auskultatorischen RR-Messung}% Titel
{In Abhängigkeit vom
    außen anliegenden Manschettendruck kommt es zu einem
    unterschiedlich starken Blutfluss und dadurch
    entstehenden, hörbaren, Strömungsgeräuschen.}% Beschreibung
{}% Label
{Lena Hirtler}% Quelle
{MfG}% Lizenz
{./images/hirtler-aass/Riva-Rocci_edited.pdf}% Datei
{}% Breitenmodifizierer


\TextSlide{Palpatorische Methode}
{%
Die Manschette sollte faltenfrei und nicht über Kleidung, in
der Mitte des Oberarms angelegt werden und sich, wenn
möglich, auf \E{Herzhöhe} befinden\ZFootnote{Wenn die
...
\EE{Der diastolische Wert kann mit dieser Methode \E{nicht}
ermittelt werden!}
...
\TextSlideTeaser
{Was ist besonders zu beachten?}%1 Titel
{%
...
\begin{itemize}
  ...
  \end{enumerate}
\end{itemize}}%3 Slide
{./images/sign_cardive.jpg}%4 Bilddatei
{}%5 Bildtitel
{}%6 Bildbeschreibung
...
\begin{Danger}
  \item Shunt-Arme sind tabu für die Blutdruckmessung!
\end{Danger}
...
\section{\HeadingKeyword{Apparative} Untersuchungen}

\subsection{Die \HeadingKeyword{Pulsoxymetrie} misst die Sauerstoffsättigung und Pulsfrequenz}
...
\begin{itemize}
  \item \StepHeading{Fingerclip-Geräte}: Diese vereinen Messgerät und
  Sensor in einem Gerät.
  \item \StepHeading{Handpulsoxymeter}: Diese bestehen aus einem
  Handmessgerät und einem per Kabel mit dem Messgerät verbundenen Sensor.
  \item \StepHeading{Funktionsmodul eines Überwachungsmonitors}:
...
\Picture
[H]% Float
{Handpulsoxymeter}% Titel
{Sauerstoffsättigung 96\PC*, Pulsfrequenz 89\PerMin*}% Beschreibung
{}% Label
{Christoph Pallinger}% Quelle
{MfG}% Lizenz
{./images/pallinger-christoph-aass/Pulsoxymeter_32779-00943pt}% Datei
{}% Breitenmodifizierer
...
\begin{itemize}
  \item \StepHeading{Einmalsensoren} \ldots{} sind üblicherweise
  ...
  \item \StepHeading{Mehrwegsensoren} \ldots{} sind ebenso für
...
zu verwenden. Nur bei einwandfreier Funktion darf das
...
\begin{Synopsis}
  \SynopsisItem{Pulsoxymetrie: 
  Eine ausreichende Blutversorgung der Körperstelle, 
  an der der Sensor angebracht ist, 
  ist notwendig.}
\end{Synopsis}
...
\EE{\VonBis{96}{100}\PC*}. Bei Patienten mit chronischer Lungenerkrankung findet man
jedoch oft sehr niedrige Werte (z.\,B.
90\PC*), ohne dass die Patienten Beschwerden haben.
}{
\begin{itemize}
  \item Normalwert: \VonBis{96}{100}\PC*
  \item chronisch lungenkranke Patient: auch Werte < 90\PC* möglich ohne
  Beschwerden
\end{itemize}
}{}{}{}
...
\begin{Synopsis}
  \SynopsisItem{Die Beurteilung der Sauerstoffsättigung muss
  berücksichtigen, ob und wieviel Sauerstoff der Patient
  bereits erhält.}
  \SynopsisItem{Behandle den Patienten und nicht das Gerät!}
\end{Synopsis}
...
aufbereitet werden, dies muss dann nach jedem Patientenkontakt erfolgen. 
Die Geräte sind üblicherweise nicht
...
\subsection{\HeadingKeyword{Blutzuckermessung}}
...
auch in \E{Millimol pro Liter} (\MmolL),} 
gemessen. Der
...
\subsection{\HeadingKeyword{Körpertemperatur}}
...
% TODO Grassl Foto: Messung der Körpertemperatur

\clearpage

\subsection{Das \HeadingKeyword{EKG} zeigt die elektrische Herzaktivität}
...
\TextSlide
{\Z{EKG -- Ablauf einer Erregung}}%1 Titel
{%
\Z{Die einzelnen Abschnitte einer EKG-Kurve zeigen die Erregung der unterschieldichen Teile des Erregungsleitungssystems im Zeitverlauf an. Die \I{P-Welle} entspricht dabei der Vorhoferregung, 
die \I{PQ-Zeit} der Überleitung vom Vorhof in die Kammern (\I{AV-Überleitung}).
% 
Der \I{QRS-Komplex} wird durch die Kammererregung erzeugt, 
wohingegen die \I{ST-Strecke} 
und die \I{T-Welle} 
die Erregungsrückbildung repräsentieren.
}% Z
}%2 Text
{%
\Z{\begin{itemize}
...
%{./images/wikipedia-cc-by-sa/EKG_Komplex}%4 Bilddatei
%{Eine elektrische Herzaktion}%5 Bildtitel
%{}%6 Bildbeschreibung
%{WMC:Hank}%7 Quelle
%{CC-BY-SA-2.0-DE}%8 Lizenz
%% Vormals \Layout{20}

\begin{PictureSeries}{}{Schema einer elektrischen Herzaktion}
\PictureSeriesRow
{2}% Anzahl Spalten}
{./images/wikipedia-cc-by-sa/EKG_Komplex}% Pfad Bild 1
{Eine elektrische Herzaktion. 
\Z{Die einzelnen Abschnitte im zeitlichen Verlauf repräsentieren unterschiedliche Stadien der Erregung: \newline
\textbf{P}-Welle $\approx$ Vorhoferregung, \newline
\textbf{PQ}-Zeit $\approx$ AV-Überleitung, \newline
\textbf{QRS}-Komplex $\approx$ Kammererregung, \newline
\textbf{ST}-Strecke + \textbf{T}-Welle $\approx$ Erregungsrückbildung.}
\SourcePicture{WMC:Hank / CC-BY-SA-2.0-DE}}% Text Bild 1
{./images/anatomie/Reizleitungssystem_1.png}% Pfad Bild 2
{Reizleitungssystem, Schema. \newline
\textbf{1}: Sinusknoten \newline
\textbf{2}: AV-Knoten \newline 
\SourcePicture{J. Heuser,
basierend auf der Arbeit von Patrick J. Lynch; illustrator;
C. Carl Jaffe; MD; cardiologist Yale University Center for
Advanced Instructional Media; CC-BY}}% Text Bild 2
{}% Pfad Bild 3
{}% Text Bild 3
{}% Pfad Bild 4
{}% Text Bild 4
...
\begin{Danger}
  \item Das EKG zeigt nur die \E{elektrische} Aktivität,
  nicht aber die tatsächliche Muskelarbeit an! 
  \item Es erlaubt
  \EE{keine Aussage über die Auswurfleistung! }
\end{Danger}
\begin{Synopsis}
  \SynopsisItem{Grundsatz: 
  Selbst ein pulsloser Patient kann für einige Zeit
  ein unauffälliges EKG aufweisen!}
\end{Synopsis}

\subsubsection{Ein EKG kann verschiedene \HeadingKeyword{Ableitungen} haben}
\label{topic:ekg-ableitungen}

\TextSlide{\TxBeschreibung}{
Je nach Position der Elektroden 
(und elektrischer Schaltung) 
unterscheiden sich die abgeleiteten elektrischen Herzströme.
%
Um die elektrischen Herzströme aus verschiedenen Perspektiven in jeweils einer Kurve abzubilden 
und beurteilen zu können, 
diese Kurven jedoch zwischen verschiedenen Untersuchungen 
und Patienten vergleichbar zu machen, 
haben sich Standard-Elektrodenpositionen 
und \I{Standardableitungen} etabliert. 
%
Die gebräuchlichsten sind die Extremitätenableitungen
...
\begin{itemize}
  \item 6 Extremitätenableitungen\Z{: \EKG{I}, \EKG{II},
  \EKG{III}, \EKG{aVL}, \EKG{aVR}, \EKG{aVF}}
  \item 6 Brustwandableitungen\Z{:
...
\begin{itemize}
  \item 6 Extremitätenableitungen
  \item 6 Brustwandableitungen
\end{itemize}
}{}{}{}
...
rechten Schulter). Wichtig ist, dass die horizontalen Elektrodenpärchen
immer auf \EE{gleicher Höhe} angebracht werden!
%
Zu den Positionen der Elektroden siehe \FullPrettyRef{tab:ekg-elektroden-position}.

Aus den Informationen der \EE{vier Elektroden} erzeugt das
...
Störungen zu der jeweiligen Region des Herzens. 
%
Aus den Informationen der \EE{sechs Elektroden} erzeugt das
\Abk{EKG}-Gerät \EE{sechs Ableitungen}\Z{:
\VonBis{\EKG{V1}}{\EKG{V6}}}. 
%

Zu den Positionen der Elektroden siehe \FullPrettyRef{tab:ekg-elektroden-position}.
Die \EE{richtige Positionierung} der
...
kann. 
\hfill\Commentary[EKG, Positionierung der EKG-Elektroden]
{%
...
\begin{itemize}
    ...
    \item 6 Ableitungen
    \item Genaue Positionierung wichtig!
\end{itemize}
}{}{}{}

\TextSlide
{Praxis des EKG-Schreibens}%1 Überschrift
{%
In der Praxis ergeben sich daraus zwei Anwendungsfälle:

\begin{itemize}
    \item \T{Rhythmusstreifen}: 
    Bestehend aus den \I{Extremitätenableitungen} 
    zur Beurteilung des Herzrhythmus und zum Monitoring.
    %
    \item \T{12-Kanal-EKG}: 
    Bestehend aus den \I{Extremitätenableitungen} und den 
    \I{Brustwandableitungen} 
    zur Diagnostik des Herzrhythmus 
    und zur Diagnose und Lokalisation von Herzschädigungen, 
    vorwiegend des Herzmuskels. 
    %
...
{%
\begin{itemize}
  \item Rhythmusstreifen: 6 Extremitätenableitungen
  \item 12-Kanal-EKG: Extremitäten- + Brustwandableitungen
\end{itemize}
}%3 Slide
...
\TableWide
[H]%1 Float
{Farbe und Standardpositionen der EKG-Elektroden}%2 Titel
{}%3 Beschreibung
{\label{tab:ekg-elektroden-position}}%4 Label/Hook
{}%5 Quelle
{}%6 Lizenz
{%
\begin{tabulary}{\linewidth}{lLL}
\addlinespace\toprule\addlinespace
\noindent\TabHeading{Elektrode} 
& 
\noindent\TabHeading{Position} 
&
\noindent\TabHeading{Alternative Position}
\\\addlinespace\midrule\addlinespace
\multicolumn{3}{c}{\TabSub{Extremitätenableitungen}}
\\\addlinespace\midrule\addlinespace
...
Rechte Leiste 
\\\addlinespace\midrule\addlinespace
\multicolumn{3}{c}{\TabSub{Brustwandableitungen}}
\\\addlinespace\midrule\addlinespace
\textbf{\EKG{V1}} & 4. Zwischenrippenraum rechts vom Sternum
\\\addlinespace
\textbf{\EKG{V2}} & 4. Zwischenrippenraum links vom Sternum
\\\addlinespace
\textbf{\EKG{V3}} & Mitte zwischen \EKG{V2} und \EKG{V4}
\\\addlinespace
\textbf{\EKG{V4}} & 5. Zwischenrippenraum links in der Verlängerung
der Schlüsselbein-Mittellinie
\\\addlinespace
\textbf{\EKG{V5}} & 5. Zwischenrippenraum vordere Achsellinie (zw. \EKG{V4} und \EKG{V6}) 
\\\addlinespace
\textbf{\EKG{V6}} & 6. Zwischenrippenraum in der mittleren
Achsellinie 
...
\PictureWide
[H]% Float
{Unauffälliger 12-Kanal-EKG-Befund}% Titel
{}% Beschreibung
{}% Label
{WM}% Quelle
{PD}% Lizenz
{./images/wikipedia-pd/12_lead_generated_sinus_rhythm}% Datei
{}% Breitenmodifizierer
...
und Brustwandableitungen}% Titel
{}% Beschreibung
{}% Label
{WmCo/Madhero88}% Quelle
{PD}% Lizenz
{./images/geraete/ECGcolor_edited}% Datei
{}% Breitenmodifizierer
...
\PictureWide
{Ein Multifunktionsgerät mit Monitor, nicht-invasiver Blutdruckmessung, Pulsoxymetrie 12-Kanal-EKG und integriertem Defibrillator mit SAED-Funktion (Corpuls\textsuperscript{3}\texttrademark)}% Titel
{}% Beschreibung
{\label{fig:corpuls3}}% Label
{Christoph Pallinger}% Quelle
{MfG}% Lizenz
{./images/pallinger-christoph-aass/Corpuls_32863-01445pt}% Datei
{}% Breitenmodifizierer
{}% Float
...
\subsection{Die \HeadingKeyword{\ce{CO2}-Messung (Kapnometrie)} ist ein Standardverfahren bei beatmeten Patienten}

\TextSlide
{\TxBeschreibung}%1 Überschrift
{%
\DefinitionInPara{Als \T{Kapnometrie} bezeichnet man die Messung von \II{Kohlendioxid} (\ce{CO2}) in der Atemluft.} 
%
Mit Hilfe von \ce{CO2}-Analysatoren 
kann man den prozentuellen Anteil (Volumen-\PC) 
oder den Partialdruck (mm\,Hg) an Kohlendioxid 
in der Ausatemluft messen 
und graphisch darstellen. 
%
Diese Verfahren sind wichtig, 
um eine \EE{Fehllage} 
von Hilfsmitteln zur Atemwegssicherung 
(z.\,B. \I{Larynxtubus} (\FullPrettyRef{topic:larynxtubus}), 
\I{Endotrachealtubus} (\FullPrettyRef{topic:intubation-assistenz}))
%des Tubus 
zu erkennen 
bzw. auszuschließen 
und kann als Zielwert dienen um die \EE{Beatmungsparameter} 
bei der Beatmung
optimal für den Patienten einstellen zu können. 
%
Zusätzlich kann man durch die \ce{CO2}-Messung eine zuverlässige Aussage 
über die \EE{Durchblutung der Lunge} 
(\E{Lungenperfusion}, \FullPrettyRef{topic:lungenperfusion}) 
treffen.
%
\Z{Die Messung kann in 3 Phasen eingeteilt werden:}
%
\Z{\begin{compactitem}
  \item 1. Phase: 
  Das Atemgas aus dem Totraum der vorangegangenen Inspiration wird abgeatmet.
  \item 2. Phase: Ein Gemisch aus Totraumgas und Alveolargas wird abgeatmet.
  \item 3. Phase: Reines Alveolargas wird abgeatmet.
\end{compactitem}}%Z
%
Am aussagekräftigsten ist der Messwert am Ende der Ausatmung 
da hier die höchste \ce{CO2}-Konzentration gemessen wird. 
%
Daher leitet sich auch die Abkürzung \T{et\ce{CO2}} ab:
%
Sie steht für endtidaler Kohlendioxidanteil 
(\ce{CO2} am Ende des Atemzyklus).

}%2 Text
{%
\begin{itemize}
  \item Kapnometrie: Kohlendioxid-Messung (\ce{CO2})
  \item et\ce{CO2}: endtidaler Kohlendioxidanteil 
  (\ce{CO2} am Ende des Atemzyklus)
  \item Anwendung:
  \begin{itemize}
    \item Erkennung von Fehllagen bei Atemwegssicherung
    \item Zielwert für Beatmungsparameter
  \end{itemize}
\end{itemize}
}
...
\Text
{Kapnometrie und Kapnographie}%1 Überschrift
{%
Für die \ce{CO2}-Messung gibt es unterschiedliche Methoden.
%
Bei der \T{Kapnometrie} werden die et\ce{CO2}-Werte als Partialdruck in mm\,Hg und/oder als Konzentration in Vol.-{\PC} angegeben.
%
Bei der \T{Kapnographie} wird 
zusätzlich zu den Werten der Kapnometrie 
auch noch die \ce{CO2}-Konzentration im zeitlichen Verlauf 
als Kurve graphisch dargestellt.
}%2 Text

\Picture
[ht]% Float
{Kapnographie}% Titel
{Eine typische \ce{CO2}-Kurve und ein et\ce{CO2}-Wert in der Norm von 36\,mm\,Hg.}% Beschreibung
{}% Label
...
\TextSlide
{Anwendungshinweise}%1 Überschrift
{%
Bei allen Messvorrichtungen muss ein \EE{Bakterienfilter} 
zwischen Tubus und Messeinrichtung eingebaut sein, 
da die Feuchtigkeit der Ausatemluft die Geräte beschädigen kann. 
}%2 Text
{
\begin{itemize}
  \item Bakterienfilter verwenden 
\end{itemize}
}

\TextSlide
{Beurteilung}%1 Überschrift
{%
Die Normwerte beim gesunden Menschen liegen bei ca. 
\Range{4}{5}\PC* \ce{CO2}-Konzentration 
bzw. \Range{35}{40} mm\,Hg \ce{CO2}-Partialdruck.
%



\subparagraph{Erhöhte Werte} deuten 
auf eine 
\begin{inparaitem}
  \item \EE{höhere \ce{CO2}-Produktion} des Körpers 
  (z.\,B. bei Fieber),  
  \item auf ein \EE{zu niedriges Atemminutenvolumen} oder
  \item auf eine schwere \EE{Lungenerkrankung} (z.\,B. COPD) hin.
\end{inparaitem}

\subparagraph{Erniedrigte Werte} 
(bzw. keine Werte) bei der \ce{CO2}-Messung 
deuten unter anderem auf eine 
\begin{inparaitem}
  \item \EE{Fehllage} des Tubus, 
  \item ein \EE{zu hohes Atemminutenvolumen} 
  oder eine 
  \item \EE{schlechte Lungenperfusion} 
  z.\,B. in Folge einer Lungenembolie 
  oder eines erniedrigten Herzzeitvolumens
  hin.
\end{inparaitem}
%
...
{%
\begin{itemize}
  \item[\SymbolFindingNormal]
  \Range{4}{5}\PC* \ce{CO2}-Konzentration;
  \Range{35}{40} mm\,Hg \ce{CO2}-Partialdruck
  \item Erniedrigt: 
  \begin{itemize}
    \item {Fehllage} des Tubus, 
    \item {Zu hohes Atemminutenvolumen} 
    \item {Schlechte Lungenperfusion} 
  \end{itemize}
  \item Erhöht: 
  \begin{itemize}
    \item {Höhere \ce{CO2}-Produktion} des Körpers 
    \item {Zu niedriges Atemminutenvolumen}
    \item Schwere Lungenerkrankung
  \end{itemize}
\end{itemize}
}
...
\begin{Danger}
  \item Ein erniedrigter \ce{CO2}-Wert oder ein schlagartiger Abfall des Wertes sind ein kritisches Alarmzeichen.
\end{Danger}

\Text
{\ce{CO2}-Messung mit Einmaldetektoren}%1 Überschrift
{%
Bei dieser Methode der \ce{CO2}-Erkennung wird der Patient durch einen Indikator mit Lackmuspapier beatmet. 
%
Da \ce{CO2} einen sauren pH-Wert hat, 
verfärbt sich der Indikator bei jeder Ausatmung. 
%
Durch die Verfärbung kann eine ungefähre Aussage über die \ce{CO2}-Konzentration in der Ausatemluft getroffen werden.
%
Am weitesten verbreitet ist das Modell \I{Easy Cap{\texttrademark} 2} der Firma Nellcor. 
%
Die Indikatorfarbe wechselt von rosa bis violett 
(kein \ce{CO2} in der Einatemphase) 
zu gelb (\ce{CO2} in der Ausatemphase). 
%
Da Feuchtigkeit die Messung verfälschen kann ist auch hier der Einsatz eines Bakterienfilters verpflichtend.
}%2 Text

\Text
{Kapnometrie unter Reanimation}%1 Überschrift
{%
Die Kapnometrie bzw. Kapnographie hat in der Reanimationssituation einen wichtigen Stellenwert. 
%
Durch die Messung des et\ce{CO2} kann eine zuverlässige Aussage 
über die Perfusion im kleinen Kreislauf 
und damit auch über die Qualität der Herzdruckmassage getroffen werden.
%
Ziel ist es, unter Reanimationsbedingungen ein 
et\ce{CO2} von \EE{über 9\,mm\,Hg} zu erreichen. 
%
Wird dieses Ziel nicht erreicht, 
so muss die Qualität der Herzdruckmassage überprüft 
und ggfs. verbessert werden. 
%

Sollte trotz hochqualitativer Herzdruckmassage der Zielwert von über 9\,mm\,Hg nicht erreicht werden, 
so kann die Ursache in einer Durchblutungsstörung im Lungenkreislauf 
(z.\,B. aufgrund einer Lungenembolie, \FullPrettyRef{topic:lungenembolie}) liegen. 
%
Ein plötzlicher Anstieg des et\ce{CO2} unter Reanimation ist ein Hinweis auf einen \EE{wiedererlangten Spontankreislauf} (ROSC) 
und ist als Lebenszeichen zu beurteilen.
}%2 Text

\begin{Danger}
  \item In der Reanimation kann durch die et\ce{CO2}-Messung keine zuverlässige Aussage über die Tubuslage getroffen werden, 
  da die Lungenperfusion deutlich schlechter ist, 
  als bei einem Menschen mit Spontankreislauf!
\end{Danger}


%Quellenangabe:
%Larsen, Anästhesie und Intensivmedizin für die Fachpflege, 8. Aufl. Springer, 2012. ISBN: 987-3-642-28290-4
%Gebrauchsanweisung Easy Cap 2 von Nellcor





\section{\HeadingKeyword{Körperliche} Untersuchungen}

\subsection{Abtasten des \HeadingKeyword{Abdomens}}

\TextSlide{Abtasten des Abdomens}
{\label{topic:untersuchung-abdomen-abtasten}%
\index{Abdomen!abtasten}%
Das Abdomen kann man abtasten um die Anspannung der
Bauchdecke und Schmerzen zu beurteilen und
zuzuordnen, sowie um Fremdkörper zu ertasten.
...
% TODO Grassl Foto: Abtsaten Abdomen

% \clearpage % TEMP temp clearpage
\subsection{Der \HeadingKeyword{Neurocheck} ist eine überblickshafte neurologische Untersuchung}
\label{topic:neurocheck}
\index{Neurocheck}
%\begin{multicols}{2}

\Text{{\TxBeschreibung}}
{\DefinitionInPara{Der \T{Neurocheck} ist eine einfache, überblickshafte (\enquote{grobe})
...
\begin{enumerate}
  \item \StepHeading{Bewusstseinslage}: Für eine grobe Einschätzung
  ...
  \begin{enumerate}
    ...
    \item Situation
  \end{enumerate}
  \item \StepHeading{Pupillen}: Begutachtung und Lichtreaktion
  \begin{enumerate}
    \item Pupillen gleich weit (\I{isokor})?
    ...
    \begin{enumerate}
      ...
      (\I[isokor!-e Lichtreaktion]{isokore Lichtreaktion})?
    \end{enumerate}
  \end{enumerate}
  \item Lichtscheu und Sehstörungen?
  \item \StepHeading{Kraftprobe}: Grobe Prüfung an oberer und
  unterer Extremität, beurteilt wird die Kraft und die
  Seitengleichheit.
  \begin{enumerate}
    \item \StepHeading{Händedruck}: 
    Patient überkreuzt die Hände geben 
    (bei kräftigen Patienten nur Zeige- und Mittelfinger) 
    und Patienten bitten kräftig zuzudrücken.
    \item \StepHeading{Pedalprobe}: Mit Handflächen von unten gegen die Zehen und Vorfuß drücken
    und Patienten bitten dagegen zu
    drücken.
  ...
  \item
  \StepHeading{Blutzuckermessung}: Eine Blutzuckermessung gehört zu
  einem vollständigen Neurocheck.
  \item \StepHeading{Hirndruckzeichen} 
  (\FullPrettyRef{topic:hirndruckzeichen})  
  sind eine typische Kombination von Symptomen 
  und deuten auf eine Erhöhung des Druckes innerhalb des Hirnschädels hin 
  (z.\,B. aufgrund einer Verletzung oder Blutung).
  %
  Hier sind eigentlich keine neuen Untersuchungsschritte notwendig, 
  sondern es erfolgt eine bewusste Neubewertung bereits erhobener Befunde:
  \begin{enumerate}
    \item Übelkeit, Erbrechen
    \item Bewusstseinsstörungen
    \item Vitalwertveränderung: 
    Niedrige Herzfrequnz, hoher Blutdruck (HF\,$\downarrow$, RR\,$\uparrow$)
    \item Ungleichweite Pupillen (\prettyref{fig:anisokorie-bau}), 
    verlangsamte Lichtreaktion
    \item \Z{Strecksynergismen}
  \end{enumerate}
\end{enumerate}
}{
\begin{itemize}
...
\begin{PictureSeriesWide}{}{Pupillen}
\PictureSeriesRowWide
{2}% Anzahl Spalten}
{./images/trauma/pupillendifferenz.jpg}% Pfad Bild 1
{Ungleiche Pupillen (Anisokorie) 
\SourcePicture{Sebastian Gabriel, Lizenz: MfG}\label{fig:anisokorie-bau}}% Text Bild 1
{./images/gabriel-sebastian-aass/N-5/IMG_20150306_132141-01341px}% Pfad Bild 2
{Enge Pupille 
\SourcePicture{Sebastian Gabriel, Lizenz: MfG}}% Text Bild 2
{}% Pfad Bild 3
{}% Text Bild 3
{}% Pfad Bild 4
{}% Text Bild 4
...
\clearpage % TEMP temp clearpage
\subsection{Der \HeadingKeyword{Traumacheck} dient dem Erkennen von Verletzungen}
...
Patienten.}
%
Teile davon werden bereits während des Einschätzungsblocks
(\FullPrettyRef{topic:einschaetzungsblock}) 
bei der 
\II[Schnelle Trauma-Untersuchung]{Schnellen-Trauma-Untersuchung}
(\I{STU}) durchgeführt, 
um vital bedrohliche Verletzungen rechtzeitig zu erkennen.  
Bei vital bedrohten, zeitkritischen Patienten wird
ggfs. \E{kein vollständiger} Traumacheck 
durchgeführt. 
}
...
\begin{itemize}
  ...
  \item \StepHeading{Sichtkontrolle} von Mund, Nase und Ohren auf Blutungen 
  (mit einer Lampe)
  \item Bei Blutungen aus Ohren oder Nase
  \T{Tupfertest}\index{Tupfertest}\label{topic:tupfertest}:
  Etwas Blut aus Ohren oder Nase wird mit einem Tupfer auffangen.
  Aufgrund der Kapillarwirkung steigt das Blut am Tupfer auf, wobei Liquor schneller aufsteigt.
  Sollte daher im Blut Liquor enthalten sein 
  bildet sich um den roten Blutfleck ein heller, bernsteinfarbener Hof.  
  \item \StepHeading{Pupillen}: Begutachtung und Lichtreaktion
    \begin{enumerate}
      ...
      \end{enumerate}
    \end{enumerate}
  \item \StepHeading{Inspektion und Abtasten}:  
  mit festem Griff von Kopf bis Fuß. 
  ...
  \begin{itemize}
    \item \StepHeading{Schädel}: Schädeldach, Hinterkopf,
    Stirn, Wangenknochen, Nasenbein, Kiefer
    \item \StepHeading{Hals und Halswirbelsäule}:  
    Tastschmerz? 
    
    \Ca{Keine Manipulation bei Verdacht auf Wirbelsäulenverletzung!}
    \item \StepHeading{Obere Extremität}: Schultern, Arme, Hände
    \item \StepHeading{Brustkorb}: vorne und seitlich 
    (Prellmarken?
    Atemmechanik? 
    Hautemphysem? 
    Gegen leichten Druck einatmen lassen)
    \item \StepHeading{Bauch}: in 4 Quadranten (Prellmarken?
    Abwehrspannung/harte Bauchdecke?),
    \item \StepHeading{Becken}: von der Seite andrücken, schmerzhaft?
    \item \StepHeading{Rücken und Wirbelsäule}:  Tast-, Klopfschmerz?
    
    \Ca{Keine Manipulation bei Verdacht auf Wirbelsäulenverletzung!}
    \item \StepHeading{Untere Extremität}: 
    Beine (sichtbare Verletzungen?), 
    Füße (Schuhe und Socken ausziehen)
  \end{itemize}
  \item \StepHeading{Durchblutung, Motorik, Sensibilität} (\T{DMS}) der Peripherie (Finger und Zehen) \label{topic:dms}
  \begin{itemize}
    \item \StepHeading{Durchblutung}: 
    \T{Nagelbettprobe} (\EE{Rekapillarisierungszeit}):
    Leichter Druck auf Nagelende bis Nagelbett weiß ist,
    dann loslassen. 
    Die Zeit bis Nagelbett wieder rosarot ist sollte
    nicht länger als \VonBis{1}{2} Sekunden sein.
    Seitenvergleich! 
    Eine verlängerte Rekapzeit deutet auf
    eine Durchblutungsstörung hin: 
    Einseitig oder beidseitig? 
    Lokal oder systemisch?
    \item \StepHeading{Motorik}: Bewegen lassen
    \item \StepHeading{Sensibilität}: 
    \Speech{\enquote{Spüren Sie das? 
    Welchen Finger habe ich berührt?}} 
  \end{itemize}
\end{itemize}
...
% TODO Grassl Foto: Die wichtigsten Schritte der STU
