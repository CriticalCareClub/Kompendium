...
\Layout{2}{Richtungsangaben}{
Die Medizin benutzt für anatomische Beschreibungen oft
bestimmte Richtungsangaben oder deren Abkürzungen. Im
Sanitätsdienst wird man damit meist in Arzt- und
Entlassungsbriefen konfroniert.
...
\begin{description}
  ...
  \index{cranial|TLike}
  \item[caudal] fußwärts (eigeltich: schwanzwärts)
  \index{caudal|TLike}
...
\Layout{0}{Gliederung des Körpers}{%
...
\begin{figure}[H]
    \includegraphics[width=\linewidth]{./images/anatomie/Animal_cell_structure_de-edited.pdf}
...
Zellen gleicher Art bilden also Gewebe. Wenn sich verschiedene Gewebearten
zusammentun entsteht hieraus ein Organ. Organe, die dem gleichen Ziel dienen,
können zu Organsystemen zusammengefasst werden (z.B. Verdauungstrakt,
Harnbildendes und -leitendes System).

Man kann allgemein 4 Gewebearten unterscheiden:
\begin{itemize}
  \item Epithelgewebe: bildet die Oberfläche (=Haut) und kleidet Hohlorgane aus,
  Sinneszellen gehören auch in diese Gruppe.
  \item Binde- und Stützgewebe: Unter Bindegewebe versteht man eine Art
  "`Lückenfüller"', es verbindet andere Gewebe miteinander. Hierzu gehört u.a.
  auch das Fett. Knochen und Knorpel gehören zum Stützgewebe.
  \item Muskelgewebe: die verschiedenen Muskelarten werden an anderer Stelle
  beschrieben.
  \item Nervengewebe: Hierzu gehören die Zellen zur elektrischen
  Informationsübertragung des Körpers. \end{itemize} \A{ Diskussion: Was ist ein
Organ?
...
hauptsächlich als
   Stützgerüst der \EE{Haltung} des Körpers (aufrechte Haltung)
    ...
    und als \E{Ursprungs- und Ansatzfläche für die Muskulatur}. Daher wirde
...
d.\,h. von roten und weißen Blutkörperchen sowie den
...
\A{Vorschlag: "`Der menschliche Körper besteht aus ???
...
Bruch.}. \subparagraph{\T{Kompakta}} (Kortikalis, "`Knochenrinde"'), \index{Kortikalis}feste äußere Schicht,
...
notwendig. Deshalb kann man nach Frakturen auch Nägel in die Markhöhle einsetzen ohne größere
...
Rippen-Brustbein-Verbindungen des Brustkorbes (Thorax, \prettyref{topic:thorax}) \Z{(Syndesmose, Synchondrose,
Synostose)}.

\begin{itemize}
\item Syndesmose: "`Bandhaft"', bindegewebige Verbindung zwischen Knochen, z.B.
zwischen Elle und Speiche und Schienbein und Wadenbein.
\item Synchondrose: "`Knorpelhaft"', knorpelige Verbindung zwischen Knochen,
z.B. die Bandscheiben und die Symphyse.
\item Synostose: "`Knochenhaft"', knöcherne Verbindung zwischen Knochen, z.B.
die Schädelnähte.
\end{itemize}
...
\Layout{0}{Es gibt verschiedene Arten von Gelenken}
{%
\index{Gelenke!Arten}%
\index{Gelenk!Kugel-}%
\index{Gelenk!Ei-}%
...
das \T{Sattelgelenk} (Daumengrundgelenk), 
das \T{Scharniergelenk} (Fingergliedgelenke, Gelenk zwischen Oberarmknochen und
Elle) \Z{sowie das Zapfengelenk (z.\,B. Radioulnargelenk)}.
...
\begin{inparadesc}
  \item[A:] Kugel\-gelenk (z.\,B. Hüft\-gelenk),
  \item[B:] Ei\-gelenk (z.\,B. Hand\-wurzel\-gelenk),
  \item[C:] Sattelgelenk (Daumen\-grund\-ge\-lenk),
  \item[D:] Scharniergelenk (z.\,B.
  Finger\-glied\-ge\-lenke, Ge\-lenk zwi\-schen Elle und Ober\-arm\-kno\-chen),
  \item[E:] Zapfengelenk (z.\,B. Radioulnargelenk).
\end{inparadesc}
...
beim Erwachsenen durch \E{Schädelnähte} 
\index{Schädelnähte} miteinander verzahnt und verknöchert sind.
...
auf \Z{(außer wenn man auf dem Kopf steht)}. Hirnnerven treten durch diverse Löcher aus dem
...
\Ca{Hirnstammeinklemmung!} ${\rightarrow}$ \Ca{Lebensgefahr}, da das Kreislauf- und Atemzentrum abgedrückt
wird.
}
{
\begin{itemize}
...
knöchernen, übereinander "`gestapelten"' \T{Wirbel}
...
Es gibt 
\begin{itemize}
    \item 7 \EE{Hals}wirbel \Z{(Cervical-Wirbel, \VonBis{C1}{7})},
    \item 12 \EE{Brust}wirbel  \Z{(Thorakal-Wirbel, \VonBis{Th1}{12})},
    \item 5 \EE{Lenden}wirbel  \Z{(Lumbal-Wirbel, \VonBis{L1}{5})},
    \item 5 verschmolzene \EE{Kreuzbein}wirbel \Z{(Os sacrum, \VonBis{S1}{5})}, sowie
    \item \VonBis{3}{4} \EE{Steißbein}"`wirbel"' \Z{(Os coccygis)}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \VonBis{3}{4} \EE{Steißbein}"`wirbel"' 
...
Ausnahmen hierzu bilden der 1. Wirbel ("`Atlas"', hat keinen Wirbelkörper) und
das Kreuzbein (besteht aus zusammengewachsenen Wirbeln, Querfortsätze sind zu
Gelenkfächen mit den Beckenschaufeln ausgewachsen, Wirbelloch ist durchgängig
verknöchert) bzw. das Steißbein (verkümmerte Wirbeln, kein Wirbelloch, meist
...
Herz) sowie die Oberbauchorgane (Leber, Magen). Zwischen den
...
Atemmechanik, \prettyref{topic:atemmechanik}).

Die Intaktheit des Brustkorbes ist für die Atmung von großer
Bedeutung. Nach Rippenfrakturen kann es durch spitze
...
\noindent Nach einem Unfall kann es zu einer Fraktur des
Beckengürtels kommen (Beckentrauma,
\prettyref{topic:beckentrauma}). Stehen und Gehen ist dann
...
ausgeprägten \T{Schenkelhals} mit "`Knick"' zwischen
Kopf-Hals-Bereich und Körper. Körperfern endet der
...
Jeder Muskel hat einen \EE{Ursprung} (\Speech{"`Wo kommt er
her?"'}) und einen \EE{Ansatz} (\Speech{"`Wo geht er
hin?"'}). Diese beiden Punkte bestimmen die Bewegung, die
der Muskel durchführen kann. Der Ursprung ist
definitionsgemäß immer der Punkt, der stillgehalten wird. Der Ansatz dagegen
wird durch den Muskelzug auf ihn zu bewegt. Normalerweise
liegt der Ursprung eines Muskels näher beim Körper, der
Ansatz ist der Punkt, der weiter entfernter ist. Für alle
dieser Erklärungen gibt es natürlich Ausnahmen, konkret die
...
\EE{Pumpe}. Es ist somit Teil des \E{Blutkreislaufs}. Es liegt \EE{hinter dem Sternum\footnote{Brustbein}}
\A{\footnote{\A{EMHO: statt Fußzeile: "`Brustbein
(Sternum)"'. damit einheitlich.\par GABS: nein. Hier sollen
sie bereits primär den Begriff "`Sternum"' verwenden und
daran gewöhnt werden.}}} und teilweise im linken Brustkorb.
...
Das Herz ist in mehreren Schichten aufgebaut (von innen nach außen):
...
\noindent Im Herzbeutel befindet sich eine geringe Menge Flüssigkeit, die eine
reibungsarme Bewegung des Herzens ermöglicht. Wenn durch Eintritt von z.\,B. Blut nach einer Verletzung der
...
\Layout{0}{Mechanische Herzfunktion}{%
...
Rhythmus, vgl. \CO{[AED, ALS]} (\prettyref{chp:AED},
\ref{chp:ALS}).}.

\subparagraph{Mechanische Herzfunktion}
...
aus. Dabei pumpt es ca. \VonBis{4}{6}\Liter*  Blut (bzw. 1\,x das
...
\subparagraph{Elektrische Herzfunktion}
...
\VonBis{60}{100}\PerMin* elektrische Impulse (Erregungen) ab. 
...
\E{nachfolgende Station als Impulsgeber übernehmen} -- allerdings mit deutlich
...
Die Arterien verzweigen sich, weiter an Durchmesser abnehmend,
...
wird Kohlendioxid (\ce{CO2}) aufgenommen und abtransportiert}.

Das nun sauerstoffarme (und \ce{CO2}-reiche) Blut gelangt über kleine
\EE{Venolen} in das Venensystem und fließt in den
\EE{Venen} herzwärts. Am Weg zum Herzen nehmen die Venen
an Durchmesser immer mehr zu, bis sie letztendlich in die großen Venen
...
\begin{labeling}[:]{Kapillargefäßexx}
...
kann künstlich (chirurgisch) angelegt, angeboren oder akzidentiell\TODO{GABS}{2011-03-07}{akzidentiell erklären}{} entstanden
...
und \EE{in mehreren Schichten} aufgebaut. Von innen nach
außen gibt es drei Schichten:
...
geeigneten Blutversorgung ihrer Wand dort eigene Gefäße \Z{(Vasa vasorum)}. Arterien die
...
des schichtweisen Aufbaus der Blutgefäße ist wichtig für
das Verständnis eines Aneurysmas.} 
}{
\begin{itemize}
  \item 3 Schichten
...
Zu
den festen Bestandteilen gehören die \E{roten Blutkörperchen}
...
in den Extremitäten enger gestellt -- weniger Wärme geht
nach außen verloren, das Blut und dementsprechend die Wärme wird "`zentralisiert"'. Andersrum
...
Die roten Blutkörperchen sind quasi die wichtigsten des
ganzen Körpers -- ohne ihren Sauerstofftransport funktioniert im Körper gar nichts. Das Protein, das
für die Bindung des Sauerstoffes an die Erythrozyten verantwortlich ist, heißt
\T{Hämoglobin}. Es ist auch der \E{Blutfarbstoff}. Ein Mangel an Hämoglobin wird als \T{Anämie} bezeichnet.
...
Blutgerinnung und das Immunsystem (Antikörper, \Z{Immunglobuline}). Durch die
...
\EE{Gehirn} und \EE{Rückenmark} bilden morphologisch\TODO{GABS}{2011-03-07}{morphologie erklären}{} und funktionell eine
...
\subsubsection{Aufgaben allgemein}
...
\begin{itemize}
    ...
    \item Abgabe  von zweckentsprechenden Impulsen (Steuerung, Reaktion, {\dots})
\end{itemize}



\subsubsection{Spezielle Aufgaben und Aufbau}
...
\Z{Das Rückenmark lässt sich durch abgehende Spinalnerven in \E{Segmente} (insg.
...
und verlassen erst dort den
Wirbelkanal. Diese Konstruktion erinnert an eine Quaste
 bzw. einen Pferdeschweif, daher leitet sich auch der
 Fachausdruck \I[Cauda equina|Z]{Cauda equina}
 (\Lang{lat.} Pferdeschwanz) ab.} 
 }{
\begin{itemize}
...
Funktion -- global stimulierend oder hemmend --  in zwei
Systeme unterteilt werden: In den \EE{Sympathikus} und \EE{Parasympathikus}.
}
{
\begin{itemize}
    \item motorisch  
...
\emph{"`Fight and flight"'} -- \emph{"`Kampf und Flucht"'}
}{%
\T{Parasympathikus} 
...
lichtbrechenden Strukturen des Augapfels erreicht.
...
\begin{itemize}
  ...
  derbe Bindegewebsschicht. Sie ist zusammen mit dem Glaskörper verantwortlich für die Formgebung des Auges.
  \item \Z{Aderhaut (Choroidea): In der Aderhaut verlaufen
  alle Blutgefäße des Auges.}
  \item \T{Netzhaut} (Retina): In der Netzhaut befinden
  sich die Sinneszellen des Auges. Stäbchen für das Schwarz-Weiß- bzw. Dämmerungs-Sehen und Zapfen für das
  Farb-Sehen.
\end{itemize}
...
\begin{itemize}
  ...
  verschiedenfärbige (blau, braun, grün) Bereich rund um die Pupille. Sie ist eine dünne Haut mit einer
  eigenen Muskulatur -- vegetativ gesteuert -- und sorgt
  dafür, dass bei Bedarf die Pupille vergrößert bzw. verkleinert wird.
\end{itemize}
...
Hornhaut und Iris). Nun durchdringt er die \T{Linse}, sie bündelt das Licht und
...
Augenmuskeln: Sie drehen das Auge in jede Richtung und
...
abfließen kann, und die \T{Bindehaut} (Konjunktiva), die
bis auf die Hornhaut die gesamte Vorderseite des Auges und die
...
\begin{itemize}
  ...
  -- daher die Hörer des Stetoskopes immer in diese Richtung schauend einsetzen, sonst hört man
  nichts!
  ...
  \begin{itemize}
    ...
    Trommelfell verwachsen, ist teilweise durch das Trommelfell hindurch sichtbar, verbunden mit dem
  ...
  Flüssigkeit gefüllt. In der Schnecke werden die empfangenen Schallwellen nach ihrer Frequenz aufgelöst und durch
...
beträgt beim Erwachsenen ca. 1,6\Meter*{\texttwosuperior}.}
...
einer Erkrankung (z.\,B. beim Lungenödem, \prettyref{topic:lungenoedem}), ist die Sauerstoffaufnahme vermindert!} 
...
einem der beiden Blätter lassen sich diese nicht trennen, folglich wird das Zzweite passiv
...
\HeadingSub{\Lang{Term.}  \T{Gastrointestinaltrakt} (\T{GIT}), \Lang{Syn.} {Magen-Darm-Trakt}}
...
das Blut überstellt. Über das Blut werden die Nährstoffe an
ihre Zielzellen transportiert.

Abfallstoffe landen schlußendlich wieder im Darm (bzw. den Nieren) um 
...
\\\addlinespace\hhline{---}\addlinespace
\noindent\EE{Speiseröhre} (\T[Ösophagus@Oesophagus]{Ösophagus})
 &
 &
...
Speichel aus den Speicheldrüsen vermischt. Insgesamt 3 verschiedene
Speicheldrüsen fügen ihr Sekret dem Speisebrei zu (Ohrspeicheldrüse,
Unterzungenspeicheldrüse, Unterkierferspeicheldrüse).

Der Speichel dient schon im Mund der Zerkleinerung von
Stärke und Zucker in der Mahlzeit.

Der Erwachsene besitzt maximal 32 Zähne, jeweils 16 in Ober- bzw. Unterkiefer
(4x 2 Schneidezähne, 1 Eckzahn, 2 Vormahlzähne, 3 Mahlzähne). Diese Anzahl kan
insbesondere durch die unterschiedliche Ausprägung der hintersten Mahlzähne
("`Weisheitszähne"') variieren. Das Milchgebiss unterscheidet sich durch die
Anzahl (insg. 20, 4x 2 Schneidezähne, 1 Eckzahn, 2 Mahlzähne) von den bleibenden
Zähnen.
...
in die Luftröhre und damit zu einer Aspiration kommen kann.
}
{
\begin{itemize}
...
\Layout{0}{\TxBeschreibung}{
Der gesamte Verdauungstrakt ist durch einen grundsätzlich einheitlichen
Wandaufbau gekennzeichnet. Dieser besteht aus 4 Schichten (von innen nach
außen):
...
\subsection{Speiseröhre}
\index{Speiseröhre}
\label{topic:speiseroehre}


\HeadingSub{\Lang{Term.} \T[Ösophagus@Oesophagus]{Ösophagus}}


\Layout{20}{\TxBeschreibung}
{%
Die Speiseröhre (\T[Ösophagus@Oesophagus]{Ösophagus}) ist ein
ungefähr 25\CM* langer Muskelschlauch. Er dient dem \EE{Transport} über den Mund
aufgenommener Nahrung vom Rachen in den Magen. Diese
Fortbewegung funktioniert über eine wellenförmige Kontraktion
der Wandmuskeln der Speiseröhre. Im Ösophagus werden keine
verdauenden Enzyme ausgeschieden -- er dient rein zum Weitertransport
des Speisebreies.

\Z{Die Speiseröhre weist \E{drei Engstellen} auf, welche die
Nahrung überwinden muss. Die erste ist hinter dem Kehlkopf
(wenn hier ein Speisebrocken stecken bleibt, kann er außerdem
die Luftröhre verschließen. Landläufig nennt man das den
\Speech{"`Wiener-Würschtel-Tod"'}.). Die zweite Enge liegt
dort, wo die Aorta die Speiseröhre kreuzt. Die dritte Enge
entspricht dem Durchtritt des Ösophagus durch das
Zwerchfell.}
...
\E{Ösophagusvarizen}.\ZFootnote{Z.\,B. fortgeschrittene
...
\HeadingSub{\Z{\Lang{Term.} Gaster, Ventriculus.}}
...
\subsection{Zwölffingerdarm}

\HeadingSub{\Lang{Term.} \T{Duodenum}}
...
\subsection{Leber und Gallenblase}
\index{Leber}
\index{Gallenblase}
\label{topic:gallenblase}

\HeadingSub{\Lang{Term.} Hepar}

\Layout{2}{\TxBeschreibung}{%
Die Leber ist ein solides Organ und befindet sich im
\EE{rechten Oberbauch}. Sie hat einen linken und rechten
Lappen. Sie ist die "`Chemiefabrik"' des Körpers: Sie
...
als fortschreitende "`Vernarbung"' der Leber bezeichnen),
führen kann. Über die \T{Pfortader} (Vena portae,
\ref{topic:pfortader}) erhält die Leber nährstoffreiches
Blut von Teilen des Darms um es zu entgiften, bevor es in
den normalen Blutkreislauf zurückkehrt.
...
Gallenflüssigkeit \E{gespeichert (!)} -- nicht produziert --
wird. Dadurch entsteht dem Körper kein Schaden, wenn man diese beispielsweise wegen
...
\begin{PictureSeries}{}{Bilderserie: Leber und Gallenblase}
\PictureSeriesRow{3}
{./images/wikipedia-pd/Liver_1.png}
{Eine Leber.
...
Pankreas.
\SourcePicture{WMC}}
{./images/wikipedia-pd/Gray1095-gall_bladder.png}
{Gallenblase.
...
\index{Pankreas}%
\HeadingSub{\Lang{Term.} \T{Pankreas}}

\Layout{60}{\TxBeschreibung}
{%
Das Pankreas ist ca. \VonBis{15}{22}\CM* lang und liegt
...
\clearpage % TODO temp clearpage
\subsection{Krumm- und Leerdarm}
\index{Krummdarm}%
\index{Leerdarm}%

\HeadingSub{\Lang{Term.} \T{Jejunum}, \T{Ileum}}

\Layout{60}{\TxBeschreibung}
{%
Jejunum (Krummdarm) und Ileum (Leerdarm) sind die zwei
weiteren Teile des Dünndarmes. Ihre Gesamtlänge beträgt ca.
3\Meter* . Ihre Hauptaufgabe besteht in der weiteren
Verdauung der Nahrung und der Nährstoffaufnahme. Hierfür dienen Darmzotten
-- kleine "`Ausstülpungen der Oberfläche"' -- zur
...
des Abwehrsystems (Peyersche Plaques) enthält. Sie sorgen dafür,
dass Bakterien aus dem Dickdarm nicht den weiteren Dünndarm
bevölkern können. Wenn dies nicht gelingt, resultiert eine
Entzündung des Darmes (Enteritis bis Gastroenteritis) mit
den Symptomen von Durchfall, Übelkeit und Erbrechen.}
}
{
\begin{itemize}
    \item Ca. 3\Meter* lang
...
\HeadingSub{\Lang{Term.} \T{Colon}}

Der Dickdarm (Colon) ist ca. 1,5\Meter* lang und wird in
...
\emph{"`-itis"' zeigt immer eine Entzündung an, vgl. Kapitel
Hygiene}}). Der Wurmfortsatz erfüllt beim Menschen keine
nennenswerte Funktion, außer der Arbeitsplatzsicherung von
Anästhesisten und Chirurgen (wegen der vielen
Blinddarm-Operationen). Es wird vermutet, dass er eine
...
\prettyref{topic:bauchfellentzuendung})), z.\,B. durch eine
...
"`ganz normal", unerwartet und plötzlich "`verfällt"' er.}
}{%
\begin{itemize}
...
liegt die rechte Niere etwas weiter fußwärts als die linke Niere. Die Nieren
besitzen wie die Milz eine Bindegewebskapsel.

Die Niere filtert ausscheidungspflichtige Stoffe (z.\,B. Stickstoff und Abfallstoffe
...
Säure-Basen-Haushalt im Körper (pH) sowie für den
Wasserhaushalt.

Zusätzlich bildet die Niere Hormone für die Blutdruckregulierung \Z{(Renin)} und für
die Blutbildung \Z{(Erythropoetin)}.

Mit zunehemendem Alter, insbesondere bei Diabetikern, ist die Niere eines der
Hauptorgane, die "`den Geist aufgeben"'. Daraus resultiert
eine chronische Niereninsuffizienz, die Patienten müssen regelmäßig zur Dialyse
("`Blutwäsche"'). Außerdem zeigen diese Personen häufig eine Anämie (Blutarmut) durch den
Mangel an dem Blutbildungshormon \Z{Erythropoetin}.
...
\begin{itemize}
  ...
  den Schleifen\-apparat entlang. Hier werden die meisten
  wichtigen Stoffe und der Groß\-teil des Wassers wiederum zurückgeholt,
...
\index{Harn!-röhre!Verlauf} und den Penis und mündet an der Eichel in die Außenwelt. Bei einer
Vergrößerung der Prostata kann der Harnabfluss bis zur
Unpassierbarkeit erschwert sein (\E{Harnverhalt}, \prettyref{topic:harnverhalten}).
...
\Layout{0}{\TxBeschreibung}
{%
\Z{Sie sind ein paariges, jeweils am oberen Nierenpol
gelegenes Organ und überdecken die Niere "`mützenformig"'.
Mit der Harnproduktion haben sie \E{nichts} zu tun. 

Die Hauptaufgabe der Nebennieren ist die \EE{Hormonproduktion}. Man kann die
Nebenniere im Querschnitt in Nebennierenrinde und -mark
unterteilen. In den jeweiligen Abschnitten werden jeweils
...
Kapitel "`Geschlechtsorgane"' gesondert besprochen.
...
\begin{itemize}
  \item innere und äußere Geschlechtsorgane: Hier wird einzig auf ihre Lage beim
  Erwachsenen Bezug genommen. (außen = sichtbar, innen = nicht sichtbar)
  \item primäre und sekundäre Geschlechtsorgane: Bei dieser Einteilung
  unterscheidet man zwischen Merkmalen, die bereits bei Geburt vorhanden sind
  (=primär), und Merkmalen, die sich erst in der Pubertät entwickeln
  (=sekundär).
  \begin {itemize}
...
\includegraphics[width=\linewidth]{./images/hirtler-aass/Genitale-m.pdf}
...
Unter einer Potenzstörung versteht man die Unmöglichkeit zur
...
\includegraphics[width=\linewidth]{./images/hirtler-aass/Genitale-w.pdf}
...
normalerweise nach dem Eisprung "`aufgefangen"' wird.
...
Wenn die Eizelle den "`falschen"' Weg nimmt, oder sich nach
der Befruchtung nicht in die Gebärmutter weiterbewegt, kann
es zu einer \EE{Eileiterschwangerschaft} kommen. Dies kann
zu einer Lebensgefährdung der Mutter führen, wenn durch das
Wachstum des Embryos der Eileiter "`gesprengt"' wird und
es zu massiven Blutungen kommt.
}
{
\begin{itemize}
...
zyklischen Veränderungen (siehe "`der weibliche Zyklus"').

Über den Hals mündet die Gebärmutter in die Scheide, die
Mündung wird als \T{Muttermund} bezeichnet.

Eine häufige Krebsart bei der Frau ist Gebärmutterhalskrebs
\Z{(Zervixkarzinom)}. Daher sollte sie regelmäßig beim
Gynäkologen einen Abstrich machen lassen um eine
...
einem Lunarmonat (Zeit von Vollmond bis Vollmond). Er wird von
Menstruation (Monatsblutung) zu Menstruation gerechnet.
...
des Zyklus statt, ca. am 14. Tag. Wenn es zu keiner Befruchtung der
Eizelle kommt, läuft der Zyklus normal weiter, es kommt zur
Menstruation.
...
am Tag 28 abgestoßen. Nach einer Befruchtung wandert die Eizelle weiter in den Uterus
