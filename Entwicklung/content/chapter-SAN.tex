...
{{\ldots} die obere Hälfte nochmals Richtung Patient
umschlagen\ldots }
{empty}%4
{}
%
...
\E{talwärts} zu blicken. Beim Fahren oder Tragen auf ebenen Flächen schaut der Patient immer in Fahrt-/Gehrichtung.
...
Siehe \FullPrettyRef{fig:lagerungsarten}. 
...
\begin{PictureSeriesWide}{}{Bilderserie: Lagerungsarten\label{fig:lagerungsarten}}
\PictureSeriesRowWide{3}
...
%
Der Sturzhelm ist grundsätzlich immer abzunehmen!
}{}{}{}{}
...
\begin{compactitem}
  \item 2 Helfer:
  \begin{compactitem}
    \item \ActorAction{ActorA}{A}{ \E{Hinterer} Helfer}
    \item \ActorAction{ActorB}{B}{ \E{Seitlicher} Helfer}
  \end{compactitem}
  \item 1 HWS-Schiene 
\end{compactitem}
}{}{}{}{}
...
\Text{Vorgehen}{
\begin{enumerate}
  \item \Actor{ActorA}{A}\Actor{ActorB}{B}
  Dem Patient \E{in Blickrichtung} nähern um
  unnötige Bewegungen des Kopfes zu vermeiden 
  und gleichzeitig 
  \ActorAction{ActorB}{B}{\E{auffordern den Kopf nicht zu bewegen.}}
    \item \ActorAction{ActorA}{A}{
    \EE{Fixieren}: Der \E{hintere}
    ...
    \item \ActorAction{ActorB}{B} {
    \EE{Ansprechen}: Der
    \E{seitliche} Helfer öffnet das Visier, spricht den Patienten von vorne an und öffnet den Kinnriemen.}
    \item \ActorAction{ActorB}{B}{
    Der \E{seitliche} Helfer
    ...
    \item \Actor{ActorA}{A}\Actor{ActorB}{B} 
    \EE{Helm abziehen}: 
    \ActorAction{ActorA}{A}{
    Der \E{hintere} Helfer zieht den Helm in einer
    \EE{S-förmigen Bewegung} ab: 
    den Helm \EE{zuerst kippen bis die Nasenspitze} sichtbar ist, 
    dann die Unterkante in der \EE{gegengleichen Bewegung} zum Helfer bewegen.}
    
%<<<<<<< HEAD
    \ActorAction{ActorB}{B}{Während der Helm abgezogen wird,
    rutscht der \E{seitliche} Helfer mit seiner Haltehand
    etwas nach.}
    \item \ActorAction{ActorA}{A}{
    Der \E{hintere} Helfer zieht den
%=======
%    {\color{HelferB}Während der Helm abgezogen wird,
%    rutscht der \E{seitliche} Helfer mit seiner unteren Hand der Helmkante nach bis er den Kopf in der Hand hält.}
%    \item {\color{HelferA}Der \E{hintere} Helfer zieht den
%>>>>>>> 0c9ee42ccf913f2de3296e86b0238d5706499055
    Helm vorsichtig fertig ab und legt ihn sicher ab.}
    \item \ActorAction{ActorA}{A}{ 
    \EE{Der \E{hintere} Helfer
    übernimmt den Kopf} im doppelten C-Griff: 
    Daumen und Zeigefinger um ein Ohr. 
    Der Kopf darf  \EE{\E{nicht} auf Zug} gehalten werden, die \Abk{HWS}
    darf \EE{nicht gestaucht} werden.
    Der Kopf wird in  Neutralposition
    stabilisiert. 
    \cite{ITLSde6,AsboeLehrmeinungRd2011-07}
    ...
    Dies ist konsistent mit \Abk{ITLS} (6.\,A.)
    \cite{ITLSde6}}}
    \item \ActorAction{ActorB}{B}{
    Der \E{seitliche} Helfer legt
...
\begin{compactitem}
  \item 2 Helfer:
  \begin{compactitem}
    \item {\color{ActorA}\Actor{ActorA}{A}: \E{Hinterer} Helfer}
    \item {\color{ActorB}\Actor{ActorB}{B}: \E{Seitlicher} Helfer}
  \end{compactitem}
  \item 1 HWS-Schiene 
\end{compactitem}
}{}
...
\begin{enumerate}
    \item {\color{ActorB}\Actor{ActorB}{B}
    Patient auffordern \EE{den Kopf nicht zu bewegen.}}
    \item {\color{ActorA}\Actor{ActorA}{A}
    Der hintere Helfer \EE{stabilisiert} den Kopf und die \Abk{HWS}
    in Neutralposition ohne Zug und ohne Stauchung.}
    \item {\color{ActorB}\Actor{ActorB}{B}
    Beengende und behindernde
    \EE{Kleidung entfernen}.}
    \item {\color{ActorB}\Actor{ActorB}{B}
    \EE{Größenmessung}: Der seitliche
    Helfer misst die Entfernung von Schulter bis Unterkiefer in Fingerbreiten
    aus.}
    \item {\color{ActorB}\Actor{ActorB}{B}
    Stifneck entsprechend einstellen:
...
\begin{enumerate}
    ...
    fest ist.
    \item Vorformen, Ventil verschließen. Wenn vorhanden Leintuch oder Tragetuch auf
    die Matratze legen.
...
zur Erleichterung der Rettung mit
...
Armtragetuches wenig Vorteile bieten.
...
Verletzungen \EE{vom Knie abwärts}. Verletzungen des
...
\begin{enumerate}
  ...
  \item vorsichtiges Wiederholen zu zweit am verletztem Bein; ein Helfer stabilisiert das Bein, der andere öffnet den
  Schuh; Socken ausziehen; DMS am verletzten Fuß
  ...
  \EE{Das Bein wird so auf dem Boden liegend stabilisiert, der Helfer verbleibt so, bis die Schiene abgesaugt wird.}
  ...
  immer wieder fest nach. Der andere Helfer lässt den Stiefelgriff aus und formt die Schien an den Fuß.
  \item Fertig abgesaugt: Ventil schließen, Absauggerät abschalten. 
...
sich an den Arm anformen, muss jedoch noch mit einer
Mullbinde oder mit Dreiecktuchkrawatten fixiert werden. Die
\EE{Anpassung} erfolgt an der \EE{gesunden} Extremität. Trotz Aluminiumkernschiene muss an der betroffenen Extremität ein Armtragetuch angelegt werden.
}
...
\begin{compactitem}
    ...
    \item Verbandsmaterial\footnote{Druckverband: 
    Unter Umständen ist die Verwendung von flexiblen Verbandsmaterial (z.\,B. PehaHaft{\texttrademark}) ausreichend 
    bzw. je nach Wundlokalisation zu bevorzugen.}  
    (z.\,B. Dreiecktuchkrawatte)
\end{compactitem}
}{}{}{}{}
...
\begin{enumerate}
    ...
    Blutung stillen (Der Patient kann dies je nach Zustand
    unter Anleitung selbst tun).
    \item Sterile Wundauflage auf die Wunde drücken (zur Blutstillung).
    \item Druckkörper dem Wundverlauf folgend über die sterile
    Wundauflage legen, weiterhin drücken.
    \item Mit einem Verband
  	(z.\,B.
