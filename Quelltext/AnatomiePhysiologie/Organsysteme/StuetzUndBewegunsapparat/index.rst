...
-  :dfn:`Kugelgelenk`: Dreiachsig. Die Bewegung ist in drei
   Ebenen (inkl. Rotationsbewegungen) gleich gut möglich.

   Beispiel: Hüftgelenk.

-  :dfn:`Eigelenk`: Zweiachsig. Die Bewegung ist in zwei
   Ebenen möglich, wobei in einer Ebene ein größerer Bewegungsumfang
   möglich ist. Eine Rotationsbewegung ist nur sehr eingeschränkt
   durchführbar.

   Beispiel: Handwurzelgelenk.

-  :dfn:`Sattelgelenk`: Zweiachsig. Es ist eine Bewegung in
   zwei Ebenen möglich.

   Beispiel: Daumengrundgelenk.

-  :dfn:`Scharniergelenk`: Eine Bewegung ist nur in einer
   Ebene möglich.

   Beispiele: Fingergliedgelenke, Gelenk zwischen Oberarmknochen und
   Elle.

-  :dfn:`Zapfengelenk`: Einachsig. Es ist nur eine Bewegung
   in einer Ebene möglich, z. B. die Rotation zweier Knochen
   zugeneinander.

   Beispiel: Gelenk zwischen Elle und Speiche.

-  :dfn:`Zusammengesetztes Gelenk`:
   Manche Gelenke erscheinen von außen betrachtet wie ein einzelnes
   Gelenk, bei genauerer Betrachtung der Gelenksstruktur erkennt man
   aber, dass es sich um eine Kombination mehrerer Gelenke handelt.

   Beispiel: *Ellenbogengelenk*: Hier befinden sich in einem
...
-  :dfn:`Glatte Muskulatur`: Nicht willentlich
   kontrollierbar, nicht ermüdbar, ausdauernd. Sie kommt u.a. in vielen
   Verdauungsorgangen (z. B. Darm) und den Blutgefäßen vor.

-  :dfn:`Herzmuskulatur`: Quergestreifter Muskel, der jedoch
   besondere Eigenschaften hat: nicht willentlich steuerbar, nicht
   ermüdbar und verfügt über eigenen Impulsgeber (Reizbildungs- und
