...
& \multicolumn{5}{c}{\Es{Struktur der \"Osterreichischen Rechtsordnung}}
\\\addlinespace
...
\begin{description}
    \item[\T{\"Offentliches Recht}:] Staat ${\leftrightarrow}$ B\"urger
    \item[\T{Privatrecht}:] B\"urger ${\leftrightarrow}$ B\"urger
\end{description}
...
\begin{itemize}
    ...
    \item \T{Verwaltungsrecht}: Umfasst Regelungen die sich idR. nur an bestimmte Personenkreise richten und von Verwaltungsbeh\"orden
    (nicht von Gerichten) bei Versto{\ss} insofern sanktioniert werden, dass (vorrangig) Geldstrafen verhängt werden.
    
    \item \T{Verfassungsrecht}: Jener Teil der Rechtsordnung, der die grundlegenden Bestimmungen \"uber Staatsgewalt, ihre Organe und
    ihre Verhältnisse zu den Staatsb\"urgern sowie
...
\begin{itemize}
    ...
    zuständiger Vertreter	w\"unscht behandelt zu werden}
       ...
       \item {\mdseries
       \Es{\T{Geschäftsf\"uhrung ohne Auftrag} im Notfall}: der
...
\T{Erf\"ullungsgehilfen} bezeichnet. Er ist nicht
...
\Layout{2}{Tatbestandserf\"ullung}
{
...
\begin{enumerate}
    ...
    \item Rechtsfolgen: drohende Konsequenzen und Ausweichm\"oglichkeiten
\end{enumerate}
}{}{}{}{}
...
\begin{itemize}
    ...
    \item \E{Notwehr\"uberschreitung im Affekt} \Z{{\S}~3
    Abs. 2 StGB}:
    \begin{itemize}
        \item {\mdseries
    ...
    die Gefahr einer K\"orperverletzung (KV) iSd. {\S}{\S}
    83\,ff StGB und die Sachbeschädigung iSd. {\S}\,125 StGB
    darstellt. Dabei darf nur auf angemessene Weise (N\"otigung und leichte KV) angehalten werden, wenn
...
und Bernd können das verhältnismä{\ss}ig gelindeste Zwangsmittel wählen um sich
gegen den unmittelbaren Angriff zu \E{schützen}. Sie sind
jedenfalls \E{verpflichtet}, unverz\"uglich die Polizei und
\"uber diese den Amtsarzt zu verständigen. Trotzdem sollte Gewalt durch Sanitäter immer als letzte Option verwendet werden, auch wegen dem Risiko sich durch das Anhalten selbst zu verletzen. Daher der Grundsatz: Selbstschutz geht vor Fremdschutz (bzw. \"ubertriebenen Heldenmut).
}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item a) \Es{K\"orperverletzung} - {\S}{\S}\,83 ff StGB: Sonderfall der HB-Voraussetzungen: es ben\"otigt nur med. Indikation und lege artis Durchf\"uhrung; die Aufklärung ist hierf\"ur irrelevant
    ...
    \item Zivilrecht:
    \begin{itemize}
        \item a) \Es{Schadenersatzanspr\"uche} - {\S}{\S}
        1293 ff ABGB: die Heilbehandlung muss alle drei
        Voraussetzungen erf\"ullen (auch Aufklärung);
...
indem man die Blutzuckermessung f\"ur den Sanitäter ausdr\"ucklich
zulies.
...
{\quotedblbase}gefälligst{\textquotedblleft} durchzuf\"uhren sind. 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item K\"orperverletzung \Z{({\S}{\S}\,83 ff StGB)}:
        \begin{itemize}
    ...
    \begin{itemize}
        \item
        Schadenersatz wegen K\"orperverletzung \Z{({\S}{\S}\,1293 ff ABGB)}:
...
%\begin{multicols}{2}
\subparagraph*{\Es{Allgemeines zur Patientenverf\"ugung}:}
...
\subparagraph{\Es{zwei Arten der Patientenverf\"ugung}:}

\begin{enumerate}
    \item \T{Verbindliche Patientenverf\"ugung}:
    ...
    \item \T{Beachtliche Patientenverf\"ugung}:
    
    \begin{itemize}
        \item {\mdseries
        \Es{grundsätzlich}: alle Verf\"ugungen, die nicht
        die obigen Voraussetzungen erf\"ullen}
        
        \item {\mdseries
        \Es{Umfang der Beachtlichkeit}: blo{\ss} relative
        Beachtlichkeit:}
...
\subparagraph*{\Es{Suche nach Patientenverf\"ugung} --
\Z{{\S}\,12 PatVG:}}~
...
\begin{itemize}
    ...
    \item
    \T{NKV} -
    \MarginNo{\includegraphics[width=\linewidth]{./images/venflon_in_situ.jpg}}\Z{{\S}\,11 Abs. 1 Z 2} SanG: zusätzlich zu den oben genannten Kompetenzen darf der NKV jene Medikamenten der Arzneimittelliste 2 verabreichen, die \"uber ven\"osen Zugang verabreicht werden k\"onnen.
...
\end{itemize}
{\mdseries
\Es{\"Uberschreiten der Kompetenzstufen}:}
...
\begin{itemize}
    ...
    SanG    
    \item nat\"urlichen Rechtsgrundsätzen iSd. {\S}\,7 ABGB und dem
...
sich strafbar wegen K\"orperverletzung und riskiert uU.
...
\Es{Wem gegen\"uber muss man Auskunft erteilen?}
...
\begin{itemize}
    ...
    Patienten	betreuen, behandeln oder pflegen bzw. an denen der Patient im Zuge des Transportes \"ubergeben wird}
...
beim Sturz wenigsten selber abst\"utzen und bleibt (auch durch die abfedernde Wirkung der Katze) unverletzt. Doch Hr. Schmerz prallt mit Schädel und Brustkorb
...
\begin{itemize}
    ...
    \item {\mdseries
    konkret f\"ur K\"orperverletzungen: \Z{{\S}{\S}
    1325-1327 ABGB}}
...
\begin{itemize}
    ...
    \item {\mdseries
    konkret f\"ur Sachschäden: \Z{{\S}\,1331 ABGB}}
...
\begin{itemize}
    \item {\mdseries
    \Es{Behandlungspflicht f\"ur Krankenanstalten} (=
    ...
    \item {\mdseries
    \Es{\"Arztliche Behandlungspflicht} --	\Z{{\S}\,48 \"ArzteG}:}
       ...
       \item {\mdseries
       \Es{Behandlungspflicht f\"ur Sanitäter} -
       ...
       \item {\mdseries
       \Es{ersthelferische Versorgungspflicht f\"ur
       Durchschnittsb\"urger} - \Z{{\S}\,95 StGB}:}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item da sich ständig neue \"ubertragbare
...
eine Beh\"orde verpflichtet, doch sehr wohl zur Meldung an die weiterbehandelnden
...
Patientin selber keine Anzeige einbringen m\"ochte.}
...
\emph{"`Geh, schalt doch das H\"orndl und die Blitzer ei,
damit dee uns endli Plotz machen und wir einr\"ucken
kännan!"'}
