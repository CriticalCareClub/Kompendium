---
status: final
---

:::{index} single: Guedel-Tubus
:::
:::{index} single: Tubus; Guedel-
:::
:::{index} single: Oropharyngealtubus
:::

(guedeltubus)=
(guedel-tubus)=
(AATW-KP-Guedel-Tubus)=
(Oropharyngealtubus)=

# Oropharyngealtubus (Guedel-Tubus)

Der {dfn}`Guedel-Tubus` ist ein bogenförmiges, abgeflachtes Plastikrohr, welches zum Offenhalten der Atemwege bei tief bewusstlosen Patienten eingesetzt werden kann.
Somnolente oder soporöse Patienten tolerieren den Guedel-Tubus in der Regel nicht, die Gefahr einen Brechreiz auszulösen ist hoch.

Der Guedel-Tubus wird über den Mund eingeführt und kommt mit seinem Ende im Rachen zu liegen.
Er erleichtert die Beatmung, da er die Zunge aus dem Weg räumt, bietet jedoch *keinen* Aspirationschutz.
Erschwerend kann es durch Verletzungen zu Blutungen und damit wiederum zun einer Gefährung des Atemweges kommen.
{numref}`Fig-BS-GuedelTubusEinfuehren` zeigt das Einführen und die Lage des Guedel-Tubus.


:::{margin} Video
- https://www.youtube.com/watch?v=BR96naISIIY
:::

::::{subfigure} AB|CD
:subcaptions: below
:name: Fig-BS-GuedelTubusEinfuehren
:class-grid: outline

Bilderserie: Einführen des Guedel-Tubus [₢ Lena Hirtler, {term}`ℓ MfG`]

![Abmessung der Größe: Abstand Mundwinkel - Ohrläppchen](/Bilder/Hirtler-Lena-CCCA/GuedelTubus1.*)

![Zuerst wird der Guedel-Tubus "verkehrt" herum eingeführt](/Bilder/Hirtler-Lena-CCCA/GuedelTubus2.\*)

![und anschließend mittels Drehbewegung endgültig positioniert.](/Bilder/Hirtler-Lena-CCCA/GuedelTubus3.\*)

![Position des Guedel-Tubus im Querschnitt](/Bilder/Hirtler-Lena-CCCA/PositionGuedel-color.\*)

::::


