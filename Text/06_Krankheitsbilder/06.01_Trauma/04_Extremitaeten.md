...
Eine **Amputation** {index}` <! Amputation>` (*Lat.* *Amputatio* (*Amp.*)) bezeichnet einen kompletten oder inkompletten Abriss bzw. Abtrennung (von Teilen) einer Extremität oder eines anderen Körperteils, sodass deren Durchblutung ganz oder teilweise aufgehoben ist. Den abgetrennten Körperteil nennt man
**Amputat**{index}` <Amputat>`.
