...
\begin{itemize}
  \item Lederhaut \Z{(Sklera)}: Die Hornhaut ist eine derbe
  Bindegewebsschicht. Sie ist zusammen mit dem Glaskörper verantwortlich für die Formgebung des Auges.
  \item \Z{Aderhaut (Choroidea): In der Aderhaut verlaufen
  alle Blutgefäße des Auges.}
  \item Netzhaut \Z{(Retina)}: In der Netzhaut befinden sich
  die Sinneszellen des Auges. Stäbchen für das Schwarz-Weiß- bzw. Dämmerungs-Sehen und Zapfen für das
  Farb-Sehen.
\end{itemize}
...
\begin{itemize}
  \item Hornhaut \Z{(Cornea)}: Die Hornhaut ist eine
...
Linse. Sie bündelt das Licht und sorgt dafür, dass wir
scharf sehen können.
...
Nun landet der Lichtstrahl auf der Netzhaut. Die
...
abfließen kann, und die Bindehaut \Z{(Konjunktiva)}, die bis
...
Trennlinie ist hier das \EE{Trommelfell}, dass das Außenohr
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Hammer \Z{(Malleus)}: ist fest mit dem Trommelfell
    verwachsen, ist teilweise durch das Trommelfell hindurch sichtbar, verbunden mit dem
    \item Amboss \Z{(Incus)}: dieser gibt die erhaltenen
    Schwingungen weiter an den
    \item Steigbügel \Z{(Stapes)}: ist mit der Schnecke
    verbunden
  \end{itemize}
...
\Layout{20}{\TxBeschreibung}
{%
Die Speiseröhre (Ösophagus) ist ein ungefähr 25\CM* langer
Muskelschlauch. Er dient dem \EE{Transport} über den Mund
aufgenommener Nahrung vom Rachen in den Magen. Diese
Fortbewegung funktioniert über eine wellenförmige Kontraktur
der Wandmuskeln der Speiseröhre. Im Ösophagus werden keine
verdauenden Enzyme ausgeschieden -- er dient rein zur
Fortbewegung des Speisebreies.

\Z{Die Speiseröhre weist \E{drei Engdtellen} auf, welche die
...
-- genauer: den Zwölffingerdarm -- weiter.
}{
\begin{itemize}
...
Einerseits ist es verantwortlich für die Enzymproduktion für
die Verdauung. Diese werden über den
Pankreas-Ausführungsgang in den Zwölffingerdarm abgegeben.
Andererseits produziert das Pankreas zwei wichtige Hormone,
Insulin und Glukagon, die regulierende Stoffe für den
Zuckerstoffwechsel des Körpers sind. Insulin sorgt eine
Verringerung des Blutzuckers durch eine Förderung der
Zuckerentfernung aus dem Blut. Glukagon hingegen ist
...
3\Meter* Ihre Hauptaufgabe besteht in der weiteren Verdauung
der Nahrung und der Nährstoffaufnahme. Hierfür dienen Darmzotten
...
\Z{Sie dient der immunologischen Überwachung des Blutes und
ist daher Teil des Immunsystems. Die Milz filtert das Blut, zu
alte Erythrozyten werden durch weiße Blutkörperchen in der
...
Sie überdecken die Niere "`mützenformig"'. Mit der
Harnproduktion haben die Nebennieren nichts zu tun. Die
topographische Nähe hat mit einem Hormon für die
Blutdruckregulierung zu tun, mit dem die Nebenniere der
Niere dabei hilft.

Man kann die Nebenniere im Querschnitt in Nebennierenrinde
und -mark unterteilen. Es werden jeweils andere Hormone
produziert.
...
Am wichtigsten ist, dass auch er Fasern des Hodenheber-Muskels enthält.
...
des Hodens begleitet mit unvorstellbaren Schmerzen und vegetativen
Symptomen wie Übelkeit und Erbrechen. Wenn diese Torsion
nicht schnellstmöglich operiert wird, stirbt dieser Hoden
vollständig ab. \Z{Bei der Operation wird der Hoden an den
Hodensack festgenäht, damit dies nicht nocheinmal vorkommen
kann.}
}{%
\begin{itemize}
  \item Hodenheber-Muskel
...
nach dem Eisprung "`aufgefangen"' werden soll.

Die Befruchtung der Eizelle findet im Eileiter statt.
Dadurch kann man sich auch vorstellen, dass es, wenn die
Eizelle den "`falschen"' Weg nimmt, oder sich na der
Befruchtung nicht in die Gebärmutter weiterbewegt, es zu
einer Eileiterschwangerschaft kommen kann. Dies kann zu
einer Lebensgefährdung der Mutter führen, wenn durch das
Wachstum des Embryos der Eileiter "`gesprengt"' wird und
dabei viel Blut verloren wird.
}{
Der Eileiter \Z{(Tuba uterina / Salpinx)} ist ein ca.
\VonBis{10}{18}\CM* langer Schlauch mit einer freien Öffnung
in die Bauchhöhle. Nahe dem Ovar zeigt der Eileiter einen
...
{%
\OldNew{
Die Gebärmutter (Uterus) kann grob in zwei Teile, dem Körper
und dem Hals, eingeteilt werden. In den Körper münden die
beiden Eileiter, die die befruchtete Eizelle in den Uterus
führen sollen.
Der Körper der Gebärmutter ist äußerst muskulös und die
Schleimhaut unterliegt bei der geschlechtsreifen Frau
zyklischen Veränderungen (siehe "`der weibliche Zyklus"').

Bei der Geburt ist der Gebärmutterkörper außerdem für die
Autreibung des Fetus zuständig. Über den Hals und den
Muttermund ragt der Uterus in die Scheide hinein.
...
In den Körper münden die beiden Eileiter, die die befruchtete
Eizelle in den Uterus führen sollen.

Der Körper der Gebärmutter ist äußerst muskulös um bei der
Geburt die Autreibung
des Fetus zu ermöglichen. 
...
Gynäkologen einen Abstrich machen lassen um eine Früherkennung zu ermöglichen.
}% New
}
{
\begin{itemize}
