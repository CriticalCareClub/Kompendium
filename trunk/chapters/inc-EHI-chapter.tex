...
\subsection{1) allgemeine Einleitung}

{
Erste Hilfe ist die Rettung von Menschen aus integritätsbedrohlichen
...
für den Rettungssanitäter unumstößliche Basis jeder seiner
...
{
Bei jeder Behandlung ist der Ersthelfer an zwingend einzuhaltende
Grundsätze gebunden, die sich wie folgt definieren lassen:}

{
1. Grundsatz: Jeder Mensch ist zur Durchführung von Erster Hilfe
verpflichtet! Sie darf nur bei Unzumutbarkeit abgelehnt werden!}

\textit{Unzumutbar ist die Erste Hilfe für den
Rettungssanitäter nur, wenn er durch Setzen   der Maßnahme sein
eigenes Leben und/oder seine Gesundheit gefährden würde.}

{
2. Grundsatz: Die zumutbare Erste-Hilfe-Maßnahme ist zumindest zu
versuchen und darf   nicht von vornherein als Unmöglich abgelehnt
werden!}
...
{
3. Grundsatz: Die Gesamtbehandlung des Patienten unterliegt einer
Interessenabwägung!}
...
seiner Besitztümer für den Schutz von höher-  wertigen Interessen
in Kauf zu nehmen. Als Maßstab fungiert die Beachtung einer  
vernünftigen Mittel-Ziel-Relation. Als Beispiel dient das
Aufschneiden der Kleidung   des Patienten zur besseren Untersuchung und
Behandlung.}

{
4. Grundsatz: Bei jeder Einzelbehandlung ist der Patient so schnell und
so schonend wie   möglich zu behandeln!}
...
\begin{enumerate}
    ...
    {
    {\textit{ Befindet sich der Patient in einem Zustand
    ...
    zwischen dem Schock und dem Schreck unterschieden werden. Der Begriff
    Schock bezeichnet vorrangig, und damit abweichend als im Volksmund
...
{
 ii) Erkrankungsbekämpfung:}

{
{\textit{ Sollte der Patient erkrankt sein, muss der Ersthelfer
versuchen die Erkrankung durch  vorrangige Linderung der Symptome zu
bekämpfen.}}{ }}

{
 iii) Vergiftungsbekämpfung:}

{
{\textit{ Ein an Vergiftungssymptomen leidender Patient ist so zu
...
{
Die Rettungskette erlaubt es auf einfache Weise wichtige Faktoren der
gesamten Patientenbehandlung darzustellen. Dazu zählt die Tatsache,
...
hier gültigen Satz: {\quotedblbase}Eine Kette ist immer nur so stark
ist wie ihr schwächstes Glied.{\textquotedblleft} Dies verdeutlicht
...
Notrufabgabe, aber vor allem der weiteren Ersten Hilfe vorgehen. Dieser
Umstand verdeutlicht den oben definierten 3. Grundsatz einer
Interessenabwägung.}
...
\section{II. lebensrettende Sofortmaßnahmen}


\bigskip

\subsection{3) Eintreffen am Notfall- bzw. Unfallort:}
...
{
 i) einen Überblick über die Situation verschafft,}

{
 ii) ruhig und gezielt auf die bevorstehenden Maßnahmen einstellt
und}

{
 iii) die bevorstehenden Maßnahmen nach Dringlichkeit sortiert.}
...
\subsection{4) Gefahrenbereich:}

{
Nach der Überblickgewinnung muss der Ersthelfer den Bereich des Not-
bzw. Unfalls für den Patienten aber vor allem sich selbst vor
Gefahren absichern. Bei seinem Vorgehen muss der Ersthelfer zuerst die
Gefahr erkennen, den Gefahrenbereich absichern und schließlich den
Patienten wenn nötig aus dem Gefahrenbereich retten.}


\bigskip

\subsubsection{4.1) Erkennen der Gefahr:}
...
{
 i) falsch verwendete Elektrogeräte}

{
 ii) unvorsichtiges Ausüben der Wohnungsreinigung}

{
 iii) unvorsichtiges Verwenden von Werkzeug}
...
{
 i) giftige Zimmerpflanzen (Bsp: Weihnachtsstern aus nicht
kontrollierten Anbau)}

{
 ii) ungesicherte Steckdosen}

{
 iii) in Getränkeflaschen abgefüllte Haushaltsreiniger}
...
{
 iii) Eisenbahnkreuzung}

{
Eine Besonderheit stellen Gefahrenguttransporter dar. Sie sind -- wie es
ihr Name schon sagt -- mit gefährlichen Gütern versehen und tragen
...
{
 i) Sturz auf der Skipiste}

{
 ii) Absturz beim Klettern}

{
 iii) Überbelastung bei Ausdauersport}


\bigskip

\subsubsection{4.2) Absichern des Gefahrenbereichs:}

{
Sobald die Gefahr erkannt wurde ist es primär notwendig den
Gefahrenbereich für den Patienten, für sich selber und für
dazukommende Dritte abzusichern. Methoden der Absicherung sind
vielfältig, da sie sich den jeweiligen Gegebenheiten anpassen
...
Gesundheitsberuf. In der Durchführung ist es aber oft möglich durch
eine Absicherungsmaßnahme das eigene sowie das des Patienten zu
bewahren. }
...
Als Beispiel zu nennen wäre ein noch eingeschaltetes Elektrowerkzeug
...
{
1. Grundsatz: Bei der Rettung darf immer nur Zug auf den
Patientenkörper ausgeübt    werden, niemals Druck!}

{
{\textit{  }}{\textit{Durch Zug werden die Muskeln des
...
{
2. Grundsatz: Der Patient ist bei Ausübung von Zug immer nur an den
Fixpunkten seines   Körpers anzufassen!}
...
{ 1) Tür des Fahrzeugs öffnen}

{ 2) Patient ansprechen und an der
Schulter berühren }

{ 3) Halbmondgriff: um den Brustkorb
des Patienten in Form eines Halbmondes umwandern und dabei drei
Teilschritte durchführen: }

{ 3.1) Schlüssel abziehen und auf das
Armaturenbrett legen}

{\itshape Bestimmte Fahrzeuge besitzen
...
{ 3.2) Handbremse anziehen}

{\itshape Manche Fahrzeugtypen oder
...
{\itshape Sollte der Gurt nicht zu
...
{ 4) Patient soweit nach vorne lehnen,
...
{ 3) mit der rechten Hand den rechten
...
{ 4) linke Hand greift auf des rechte
...
{ 1) Ersthelfer kniet sich hinter den
...
{ 2) Ersthelfer lehnt sich mit dem
Patienten nach vorne, steht auf und lehnt den Patienten auf einen
seiner Oberschenkel}

{ 3) Patient aus den Gefahrenbereich
herausziehen}

{ Alternativ:}

{ 1) siehe Schritt 1) oben}

 2) Ersthelfer steht nicht auf, sondern
...
Unterarmen eine Auflage für den Kopf des Patienten zu formen }
 ...
 2) Ersthelfer zieht den Patienten auf
...
{
{Bei der }{B}{ewusstseins- und
}{A}{tem}{k}{ontrolle
(BAK)}\footnote{\ {BAK: die Abkürzung BAK bezeichnete in ihrer
ursprünglichen Form die Wörter: }{B}{ewusstsein,
}{A}{tmung und }{K}{reislauf. Sie stammt
...
adaptiert, als sie heute für }{B}{ewusstseins- und
}{A}{tem}{k}{ontrolle steht.}}{ wird
...
{
{ }{\textit{Der Patient ist an der Schulter anzufassen.}}}
...
{\itshape
 Cave: Patienten die in Folge eines Unfalls verunglückt sind dürfen
...
{
 i) direkte Ansprache einer Person:}
...
{\centering
Der Helm ist immer abzunehmen, außer der Patient verweigert dies!
\par}
...
{ {\textit{Beim Doppelte-C-Griff werden
...
andere Hand den vorderen}}{ }{\textit{Halsbereich
...
{ 1) Den zum Ersthelfer gelagerten Arm
des Patienten in einem rechten Winkel auflegen.}

{ 2) Unter das vom Ersthelfer entfernte
...
{ 3) Hand- und Kniegelenk des Patienten
zum Ersthelfer ziehen.}

 4) Kopf des Patienten
überstrecken
...
\subsubsection{8) Wiederbelebung:}
...
{
 iii) Aufbrauchen der eigenen Kräfte:}
...
\section{9) Atemwegsverlegung durch Fremdkörper:}
...
{
  a) effektives Husten und Würgen}

{
 ii) ersthelferische Behandlung:}

{
  a) Patient beim Husten unterstützen durch kreisförmiges Reiben am 
...
{
  c) Wärmeerhalt sichern, psychische Betreuung, laufende Kontrolle}
...
{
  a) ineffektives (stummes) Husten}

{
  b) Aussetzen der Atmung}

{
  c) nach gewisser Zeit Bewusstlosigkeit}

{
 ii) ersthelferische Behandlung:}

{
{  a) Rückenschläge: 5x zwischen die Schulterblätter
...
{
  b) wurde das Objekt nicht abgehustet und ist der Patient weiterhin bei
   Bewusstsein: erneute 5 Rückenschläge mit anschließender
Kontrolle}

{
  c) wurde das Objekt nicht abgehustet und ist der Patient bewusstlos
...
{
  d) wurde das Objekt abgehustet und ist der Patient bei Bewusstsein: }
...
{
  e) wurde das Objekt abgehustet und ist der Patient bewusstlos:}
...
\subsection{10.1) Schockursachen:}
...
\subsection{11.1) Arten von Verletzungen und ihre ersthelferische Behandlung:}
...
{
 i) plötzliches Hinfallen, oft verbunden mit einem Aufschrei}

{
 ii) generalisierte Krämpfe}

{
 iii) Schaumaustritt aus dem Mund (uU. mit Blutbeimengung)}

{
 iv) Urin- und Harnabgang}

{
 v) während des Anfalls keine Atmung erkennbar}

{
 vi) nach Beendigung des Krampfes Eintritt von Verwirrtheit,
...
{
 i) Lagerung mit erhöhtem Oberkörper}

{
 ii) beengende Kleidung öffnen}

{
 iii) Kühlung des Kopfes mit kalten Tüchern}
...
{
 i) erhöhte Temperatur: (37{\textdegree}-38{\textdegree}C)}
...
{
 ii) leichtes bis hohes Fieber: (38{\textdegree}-40{\textdegree}C)}
...
{
 iii) sehr hohes Fieber: ({\textgreater} 40{\textdegree}C)}
...
{
 iv) Fiebertod: (42,6{\textdegree}C)}

{
  a) Eiweiß beginnt zu gerinnen; Stoffwechsel bricht zusammen; Tod }
...
\subsubsection{12.1.10) Unterkühlung:}
...
{
 i) Abwehrstadium: (34{\textdegree}-36{\textdegree}C)}
...
{
ii) Erschöpfungsstadium: (31{\textdegree}-33{\textdegree} )}
...
{
iii) Lähmungsstadium: (27{\textdegree}-30{\textdegree}C)}
...
{
iv) Scheintod: (unter 27{\textdegree}C)}
...
Zersetzung der roten Farbträger bewirkt. (vgl.
