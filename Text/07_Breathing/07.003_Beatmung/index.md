...
Überschreiten des *Speiseröhren-Öffnungsdrucks* (*Magenbeatmung*, *Magenblähung*, *Erbrechen*) oder Lungenschäden. Es ist oft zu beobachten, dass (wahrscheinlich in Folge von Stress) Beatmungen zu schnell bzw. zu tief erfolgen. Dies kann katastrophale Auswirkungen haben, da es zu einer *Hyperventilation* und dadurch zu einer atmungsbedingen Alkalose kommt (Störung des Säure-Basen-Haushaltes, vgl.
...
(Endotrachealtuben, Larynxtuben, Larynxmasken, Combituben, …) angewendet. Um die Atemwege freizuhalten, kann u. U. unterstützend ein
...
Mittels Druckdifferenz (Δp, Driving Pressure) zwischen dem Innenraum (Lunge) und dem Außenraum (Beatmungsgerät) wird ein Gasfluss (Flow) erzeugt.
Um die Beatmung grundlegend zu kontrollieren bedient man sich folgender Parameter:
...
Bei den Spontanatmungsmustern muss der Patient jeden Atemzug grundsätzlich selber initieren.

Beim {dfn}`CPAP`-Modus ({dfn}`Continous Positive Airway Pressure`) wird der Druck in den Atemwegen gegenüber der Atmosphäre (p{sub}`aw`) ständig, also auch während der Atemruhephase und Inspiration, positiv gehalten (PEEP: Positive End Exspiratory Pressure, p{sub}`PEEP`).
Dadurch soll ein Kollabieren der Alveolen und die Bildung von Atelektasen verhindert werden.

Im {dfn}`PSV`-Modus ({dfn}`Pressure Support Ventilation`) triggert die Eigenatmung des Patienten eine Druckunterstützung und erleichtert so die Atemarbeit.
...
Je nach Gerät erlauben sie eine genauere Steuerung und Kontrolle der Beatmung und bieten z. T. umfangreiche Einstellungen und Beatmungsmuster zur Auswahl.
...
{dfn}`PEEP` steht für {dfn}`Positive End-Exspiratory Pressure` (positiver end-exspiratorischer Druck), d. h. positiver Druck in den Atemwegen (auch) am Ende der Ausatmungsphase.
