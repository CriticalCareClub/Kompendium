...
Das Sterben und der Tod gehören zu den grundlegensten und
unausweichlichsten Vitalfunktionen. 
Man
...
Patienten, welche an einer unheilbaren und todbringenden
Erkrankung leiden, 

Die \T{Palliativmedizin} und \T{Palliativpflege} beschäftigt
sich mit der Betreuung von Patienten während des Sterbens. Ziel
ist nicht die Heilung einer Krankheit, sondern das
Ermöglichen eines würdigen und schmerzfreien Todes. Die
Bedürfnisse der Patienten sind unterschiedlich und richten
sich zum Beispiel anch dem Alter, der Erkrankung, den
...
wiederholt, duchlaufen werden:

\begin{itemize}
  \item \EE{Abwehr}: Nicht wahrhaben wolln, Abwehr,
  Verdrängung
  \item \EE{Zorn}: Wütend auf das
  Schicksal (\Speech{"`Warum ausgerechnet ich und nicht
  ein anderer?"'}) und neidig auf das "`Glück"' Anderer
  \item \EE{Verhandeln} {\ldots} mit dem
  betreuendem Personal, mit Gott oder generell mit dem
  Schicksal
  \item \EE{Depression}: Traurige,
  depremierte Stimmungslage, Verlust des "`sinnerfüllten
...
reagieren kann. Was auf den ersten Blick wie "`launisches
Verhalten"' wirkt, ist oft durch die Phasen des
Sterbeprozesses erkärbar.

Der Umgang mit "`geplant"' sterbenden Patienten im
Rettungs- und Krankentransportdienst

Die Zeitdauer des Kontakts ist naturgemäß eher gering, und
die palliative Versorgung ist nicht unbedingt ein
Schwerpunkt während eines Transportes.  Dennoch gibt es
einige Dinge zu bedenken:

\begin{itemize}
  \item \EE{Angst und Ausnhamesituation}: Sowohl die Angehörigen wie
  auch der Patient befinden isch u.\,U. in einer Ausnahmesituation wenn eine (neuerliche)
  Klinikeinweisung bevorsteht. Oft kann man Angst vor der
  folgenden Ungewissheit beobachten. Hier ist es wichtig,
  selber ein \E{ruhiger Pol} bzw. ein \E{Fels in der Brandung}
  zu sein, an dem sich der Patient und die Anghörigen
  "`anhalten"' können.
  \item \EE{Sachebene als Brücke}: Sachliche und klare Informationen
  über das weitere Vorgehen sind für beide wichtig, und können
  auch somanche emotionale Reaktion kanalisieren.
  ...
  \item \EE{Eigene Interessen}:

\end{itemize}



\Layout{2}{Der Helfer und der Sterbende}
