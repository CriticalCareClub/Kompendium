...
\Layout{2}{Richtungs- und Lageangaben}{
...
\begin{itemize}
  ...
  \enquote{Lückenfüller}, es verbindet andere Gewebe miteinander. Hierzu gehört u.a.
...
Pschyrembel: \enquote{aus Zellen u. Geweben
...
\A{Vorschlag: \enquote{Der menschliche Körper besteht aus ???
...
{%
\T{Periost}: \E{Beinhaut}, überzieht den Knochen und
...
\T{Kompakta} (Kortikalis, \enquote{Knochenrinde}):
\index{Kortikalis}feste äußere Schicht, äußere Form, verantwortlich für Stabilität des Knochens

\T{Spongiosa}: \index{Knochenbälkchen}Knochenbälkchen
${\rightarrow}$ Zug- und Druckfestigkeit, rotes Knochenmark

\T{Markhöhle}: s.\,u.

\T{Epiphysenfuge}:  Wachstumsfuge

\T{Gelenkfläche}: \index{Gelenkfläche!Aufbau des
...
Bestrahlung, Chemotherapien bei Krebspatienten (\enquote{onkologische
...
{Bei den Gelenken kann man zwischen Gelenken \E{mit Gelenksspalt} (\enquote{echtes
Gelenk}, \Z{Diarthrose}) und Gelenken \E{ohne Gelenksspalt} (\enquote{unechtes
...
\VARIANT{VAR-ERNAEHRUNGSPAED}{
\begin{itemize}
  \item Syndesmose: \enquote{Bandhaft}, bindegewebige Verbindung zwischen Knochen, z.\,B.
  zwischen Elle und Speiche und Schienbein und Wadenbein.
  \item Synchondrose: \enquote{Knorpelhaft}, knorpelige Verbindung zwischen Knochen,
  z.\,B. die Bandscheiben und die Symphyse.
  \item Synostose: \enquote{Knochenhaft}, knöcherne Verbindung zwischen Knochen, z.\,B.
  die Schädelnähte.
\end{itemize}
}
...
\Fazit{Das, was man umgangssprachlich unter dem Begriff \enquote{Gelenk} versteht,
sind \enquote{echte} Gelenke mit Gelenksspalt.} 
}
{%
\begin{itemize}
  \item \enquote{echtes Gelenk}
  \item \enquote{unechtes Gelenk}
\end{itemize}
}{}{}{}

\Layout{20}
{Prinzipieller Aufbau eines \enquote{echten Gelenks}}
{%
...
knöchernen, übereinander \enquote{gestapelten} \T{Wirbel}
...
\begin{itemize}
    ...
    \item \VonBis{3}{4} \EE{Steißbein}\enquote{wirbel} \Z{(Os coccygis)}.
\end{itemize}
...
(\E{Wirbelsäulentrauma}\VARIANT{VAR-STANDARD}{, \prettyref{topic:wirbelsaeulentrauma}}). Bandscheiben
...
Nervenausfälle verursachen (\E{Bandscheibenvorfall}\VARIANT{VAR-STANDARD}{,
...
\begin{itemize}
    ...
    (\T{Discus}): dienen der \enquote{Stoßdämpfung}
...
\begin{itemize}
    ...
    \item \VonBis{3}{4} \EE{Steißbein}\enquote{wirbel} 
...
\VARIANT{VAR-ERNAEHRUNGSPAED}{
Ausnahmen hierzu bilden der 1. Wirbel (\enquote{Atlas}, hat keinen Wirbelkörper) und
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \Z{7 \enquote{echte}}
        \item \Z{3 \enquote{unechte}}
        \item \Z{2 \enquote{fliegende}}
    \end{itemize}
...
ausgeprägten \T{Schenkelhals} mit \enquote{Knick} zwischen
...
Jeder Muskel hat einen \EE{Ursprung} (\Speech{\enquote{Wo kommt er
her?}}) und einen \EE{Ansatz} (\Speech{\enquote{Wo geht er
...
\A{\footnote{\A{EMHO: statt Fußzeile: \enquote{Brustbein
(Sternum)}. damit einheitlich.\par GABS: nein. Hier sollen
sie bereits primär den Begriff \enquote{Sternum} verwenden und
...
Bewegung und schlimmstenfalls zum Tod kommen.
}
{
\begin{itemize}
  \item Hohlmuskel
...
{./images/hirtler-aass/Herz-edited-0800.png}{\AuthNotesPicLegalOk{}{}Das Herz.}
{Hirtler}
...
(\E{natürlicher \enquote{Schrittmacher}}). Er feuert mit einer Eigenfrequenz von ca.
...
\begin{itemize}
  ...
  \item Nachfolgende Stationen können bei Bedarf \enquote{übernehmen} (HF\,\Sign{↓})
\end{itemize}
...
\enquote{schönes} EKG haben} (allerdings nicht lange)!}
...
untersucherseitig aus einem Bügel mit zwei Oliven (\enquote{Ohrenstöpsel}). 
...
\begin{itemize}
    ...
    muskulösen Arterien.     
    \item \T{Kapillargefäße} sind dünne \enquote{Haargefäße},
...
Arterie
&
\enquote{Hauptarterie}, geht von der linken Herzkammer ab, von dort zweigen alle
...
nach außen verloren, das Blut und dementsprechend die Wärme wird \enquote{zentralisiert}. Andersrum
...
\VARIANT{VAR-STANDARD}{(\prettyref{topic:bikarbonatpuffer})} im Blut für eine Wiederherstellung dieses
pH-Wertes. 
}{
\begin{itemize}
...
\T{Erythrozytenkonzentrate} (\enquote{Ery-Konzentrate}), das sind \EE{Blutkonserven}, welche fast ausschließlich Erythrozyten enthalten.
...
Zellorganellen. Nach einer Lebensdauer von ca. 100 Tagen werden die \enquote{Senioren} der roten
...
%   gibt je nach Erkrankung verschiedene \enquote{Spezialisten} unter den Granulozyten:
...
gelöste Gerinnungsfaktoren aktiviert. Dadurch verwandelt sich das
zunächst ebenfalls im Plasma gelöste Fibrinogen in einen
klebrigen und später in einen festen, die Wunde
verschließenden Blutpfropfen (Thrombus).} Bei Patienten mit
Bluterkrankheit (Hämophilie) fehlen Gerinnungsfaktoren und es kommt daher
...
\item \T{Sensorisches} (\enquote{fühlend}, Reizweiterleitung von
...
(\Speech{\enquote{Hirn an Hand: Leg' dich auf die Herdplatte}}) und von den Organen
zum ZNS (\Speech{\enquote{Haut von Hand an Hirn: Hier ist es heiß!}}).
...
\T{Motorische Nerven} sorgen dafür, dass ein \enquote{Bewegungswunsch} in den
...
\emph{\enquote{Fight and flight}} -- \emph{\enquote{Kampf und Flucht}}
}{%
\T{Parasympathikus} 

Aktiviert den Ruhemodus.

\emph{\enquote{Rest and digest}} - \emph{\enquote{Rast und Verdauung}}
}
...
\begin{figure}[H]
    ...
    \Captionof{figure}{\enquote{Fight or flight, rest and digest}
    \SourcePicture{Hirtler.}}
    \end{center}
\end{figure}
...
einer Erkrankung (z.\,B. beim Lungenödem\VARIANT{VAR-STANDARD}{, \prettyref{topic:lungenoedem})}, ist die Sauerstoffaufnahme vermindert!} 
...
Die Pleura hat zwei \enquote{Blätter}. Eine Seite  liegt an der
...
\begin{itemize}
    ...
    \item Brustkorb mit Rippenfell \enquote{zieht an} → Lungenfell mit Lunge folgt
    Bewegung     
\end{itemize}
}
...
\Fazit{Wenn
der Brustkorb mit dem daran verwachsenen Rippenfell \enquote{anzieht}, folgt das
...
wollen ja auch immer dass sie die \enquote{Atemhilfsmuskulatur}
etc. unterstützen, dann sollten sie wenigstens auch
nachlesen können \enquote{wie man richtig atmet}
...
Ungleichgwicht wird beim Einatmen zunächst von einer
zusätzlichen Zugkraft des Zwerchfells und der äußeren Zwischenrippenmuskulatur hervor
gerufen und anschließnd beim Ausatmen durch Entspannung der
...
kleiner ist, als der Luftdruck in der Umgebung. Dieser
Druckunterschied wird durch einströmende Luft ausgeglichen
-- man hat eingeatmet. Da für das Einatmen Muskelarbeit
verrichtet werden muss, nennt man das Einatmen auch aktive
Phase der Atmung.

Das Ausatmen erfolgt passiv (passive Phase der Atmung). Das
Zwerchfell und die äußere Zwischenrippenmuskulatur
...
Sauerstoff, Kohlendioxid und Edelgasen (\enquote{Raumluft}).
...
\emph{\enquote{Von der Schüssel in die Schüssel.}}\label{tab:verdauungstrakt-uebersicht}}
...
\noindent\TabHeading{Mastdarm} (\TabHeading{Rektum})
 &
 &
\enquote{Speicherung} des ausscheidungsbereiten
Stuhls
\\\addlinespace\hhline{---}\addlinespace
...
Der Speichel beginnt schon im Mund mit der Aufspaltung von
Stärke und Zucker der Speise.

Der Erwachsene besitzt bis zu 32 Zähne. \VARIANT{VAR-ERNAEHRUNGSPAED}{Davon 
befinden sich jeweils 16 in Ober- bzw. Unterkiefer
(4x 2 Schneidezähne, 1 Eckzahn, 2 Vormahlzähne, 3
Mahlzähne).} Diese Anzahl kann insbesondere durch die
unterschiedliche Ausprägung der hintersten Mahlzähne
(\enquote{Weisheitszähne}) variieren. Das Milchgebiss unterscheidet sich durch die
Anzahl \VARIANT{VAR-ERNAEHRUNGSPAED}{(insg. 20, 4x 2
Schneidezähne, 1 Eckzahn, 2 Mahlzähne)} von den bleibenden Zähnen.
...
\Speech{\enquote{Wiener-Würschtel-Tod}}.). Die zweite Enge liegt
...
Lappen. Sie ist die \enquote{Chemiefabrik} des Körpers: Sie
produziert wichtige Stoffe, entgiftet und speichert Fette
und Zucker.
}{
\enquote{Chemiefabrik} des Körpers
}{}{}{}
...
als fortschreitende \enquote{Vernarbung} der Leber bezeichnen),
...
(\Speech{\enquote{Galle}}), welche der  \EE{Fettverdauung} dient.
...
\Layout{60}{\TxBeschreibung}
{%
Jejunum (Leerdarm) und Ileum (Krummdarm) sind die zwei
...
-- kleine \enquote{Ausstülpungen der Oberfläche} -- zur
...
dafür, dass Bakterien aus dem Dickdarm nicht den weiteren Dünndarm
...
\\\addlinespace\hhline{~-~}\addlinespace
\EE{Mastdarm} (\T{Rektum})\index{Mastdarm}&\enquote{Speicherung} des
...
\emph{\enquote{-itis} zeigt immer eine Entzündung an, vgl. Kapitel
...
einem Teil der gelben Fettschürze \Z{\enquote{großes Netz}}, sowie der Dickdarm und
der Dünndarm. } 
{GABS.}
...
\enquote{ganz normal", unerwartet und plötzlich \enquote{verfällt} er.}
}{%
\begin{itemize}
  ...
  \item \enquote{Aus heiterem Himmel}
\end{itemize}
}{}{}{}
...
Die Niere \EE{filtert ausscheidungspflichtige Stoffe} (z.\,B. Stickstoff und Abfallstoffe
...
\EE{Elektrolytkonzentrationen} im Blut und in den Geweben und damit für den
\EE{Säure-Basen-Haushalt} im Körper (pH) sowie für den
\EE{Wasserhaushalt}.
Außerdem bildet die Niere Hormone für die Blutdruckregulierung \Z{(Renin)} und für
die Blutbildung \Z{(Erythropoetin)}.

Mit zunehemendem Alter, insbesondere bei Diabetikern, ist die Niere eines der
Hauptorgane, die \enquote{den Geist aufgeben}. Daraus resultiert
eine \E{chronische Niereninsuffizienz}, die Patienten müssen regelmäßig zur Dialyse
(\enquote{Blutwäsche}, vgl. \FullPrettyRef{topic:niereninsuffizienz}). 
Außerdem zeigen diese Personen häufig eine Anämie (Blutarmut) durch den
...
\enquote{bearbeitet}. Das Blut wird in den Glomerula zunächst gefiltert. Dadurch entstehen 150\Liter* Primärharn (inkl.
...
Unpassierbarkeit erschwert sein (\E{Harnverhalt}\VARIANT{VAR-STANDARD}{, \prettyref{topic:harnverhalten}}).
...
Außer den \enquote{klassischen Bauchorganen} liegen auch andere
...
normalerweise nach dem Eisprung \enquote{aufgefangen} wird.
...
Wenn die Eizelle den \enquote{falschen} Weg nimmt, oder sich nach
...
Wachstum des Embryos der Eileiter \enquote{gesprengt} wird und
...
zyklischen Veränderungen (siehe \enquote{der weibliche Zyklus}).
