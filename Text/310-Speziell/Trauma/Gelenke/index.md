...
{index}` <! Verstauchung>`
(*Syn.* {dfn}`Distorsion`
{index}` <! Distorsion>`, *lat.* *Distorsio* (*Dist.*))
...
({dfn}`Distorsion`
{index}` <! Distorsion>`)
handelt es sich um eine Gelenksverletzung, bei welcher ein Knochen durch stumpfe Gewalteinwirkung kurz verschoben oder verdreht wird.
...
{index}` <! Verrenkung>`
(*Syn.* {dfn}`Luxation`
{index}` <! Luxation>`, *lat.* *Luxatio* (*Lux.*))
ist eine Gelenkverletzung, bei der zwei das Gelenk bildende Knochenenden aus ihrer normalen Stellung zueinander verschoben werden. Dabei kommt es meist zu Begleitverletzungen an der Gelenkkapsel bzw. den Gelenkbändern .

Die Verrenkung
({dfn}`Luxation`{index}` <! Luxation>`) ist eine Gelenksverletzung, bei der – im Gegensatz zur Verstauchung – der verschobene oder verdrehte Knochen nicht von selbst in das Gelenk zurück gesprungen ist. Die betroffene Extremität kann daher nicht mehr bewegt werden. Bei einer Luxation dürfen keine eigenständigen Einrenkversuche unternommen werden!
