...
Muskel, \protect\enquote{erbauen}.
...
\begin{itemize}
  ...
  \protect\enquote{Lückenfüller}, es verbindet andere Gewebe
  miteinander. Hierzu gehört u.\,a. auch das Fett. Knochen
  und Knorpel gehören zum Stützgewebe.
...
\T{Kompakta} (Kortikalis, \protect\enquote{Knochenrinde}):
...
Bestrahlung, Chemotherapien bei Krebspatienten (\protect\enquote{onkologische
...
{Bei den Gelenken kann man zwischen Gelenken \E{mit Gelenksspalt} (\protect\enquote{echtes
Gelenk}, \Z{Diarthrose}) und Gelenken \E{ohne Gelenksspalt} (\protect\enquote{unechtes
...
\Fazit{Das, was man umgangssprachlich unter dem Begriff \protect\enquote{Gelenk} versteht,
sind \protect\enquote{echte} Gelenke mit Gelenksspalt.} 
}
{%
\begin{itemize}
...
\begin{itemize}
    ...
    \item \VonBis{3}{4} \EE{Steißbein}\protect\enquote{wirbel} \Z{(Os coccygis)}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \VonBis{3}{4} \EE{Steißbein}\protect\enquote{wirbel} 
...
\VARIANT{VAR-ERNAEHRUNGSPAED}{
Ausnahmen hierzu bilden der 1. Wirbel (\protect\enquote{Atlas}, hat keinen Wirbelkörper) und
...
ausgeprägten \T{Schenkelhals} mit \protect\enquote{Knick} zwischen
...
Jeder Muskel hat einen \EE{Ursprung} (\Speech{\protect\enquote{Wo kommt er
her?}}) und einen \EE{Ansatz} (\Speech{\protect\enquote{Wo geht er
...
{\QSSO}schönes{\QSSC} EKG haben} (allerdings nicht lange)!}
...
\begin{description}
    \item[Extremitätenableitung] {\QSO}\I{Rhythmusstreifen}{\QSC}
    ...
    ist das 12-Kanal-\Abk{EKG} (\Speech{{\QSO}12er{\QSC}}) für die
...
herrscht. Wenn wir vom \emph{\protect\enquote{Blutdruck}} sprechen beziehen wir
...
untersucherseitig aus einem Bügel mit zwei Oliven (\protect\enquote{Ohrenstöpsel}). 
...
Arterie
&
\protect\enquote{Hauptarterie}, geht von der linken Herzkammer ab, von dort zweigen alle
...
nach außen verloren, das Blut und dementsprechend die Wärme wird \protect\enquote{zentralisiert}. Andersrum
...
\T{Erythrozytenkonzentrate} (\protect\enquote{Ery-Konzentrate}), das sind \EE{Blutkonserven}, welche fast ausschließlich Erythrozyten enthalten.
...
Zellorganellen. Nach einer Lebensdauer von ca. 100 Tagen werden die \protect\enquote{Senioren} der roten
...
%   gibt je nach Erkrankung verschiedene \protect\enquote{Spezialisten} unter den Granulozyten:
...
\item \T{Sensorisches} (\protect\enquote{fühlend}, Reizweiterleitung von
...
(\Speech{\protect\enquote{Hirn an Hand: Leg' dich auf die Herdplatte}}) und von den Organen
zum ZNS (\Speech{\protect\enquote{Haut von Hand an Hirn: Hier ist es heiß!}}).
...
\T{Motorische Nerven} sorgen dafür, dass ein \protect\enquote{Bewegungswunsch} in den
...
(\protect\enquote{Weisheitszähne}) variieren. Das Milchgebiss unterscheidet sich durch die
...
\Speech{\protect\enquote{Wiener-Würschtel-Tod}}.). Die zweite Enge liegt
...
als fortschreitende \protect\enquote{Vernarbung} der Leber bezeichnen),
...
(\Speech{\protect\enquote{Galle}}), welche der  \EE{Fettverdauung} dient.
...
-- kleine \protect\enquote{Ausstülpungen der Oberfläche} -- zur
...
\emph{\protect\enquote{-itis} zeigt immer eine Entzündung an, vgl. Kapitel
...
einem Teil der gelben Fettschürze \Z{\protect\enquote{großes Netz}}, sowie der Dickdarm und
der Dünndarm. } 
{GABS.}
...
Hauptorgane, die \protect\enquote{den Geist aufgeben}. Daraus resultiert
eine \E{chronische Niereninsuffizienz}, die Patienten müssen regelmäßig zur Dialyse
(\protect\enquote{Blutwäsche}, vgl. \FullPrettyRef{topic:niereninsuffizienz}). 
...
\protect\enquote{bearbeitet}. Das Blut wird in den Glomerula zunächst gefiltert. Dadurch entstehen 150\Liter* Primärharn (inkl.
...
gelegenes Organ und überdecken die Niere \protect\enquote{mützenformig}.
...
Kapitel \protect\enquote{Geschlechtsorgane} gesondert besprochen.
...
normalerweise nach dem Eisprung \protect\enquote{aufgefangen} wird.
...
Wenn die Eizelle den \protect\enquote{falschen} Weg nimmt, oder sich nach
...
Wachstum des Embryos der Eileiter \protect\enquote{gesprengt} wird und
...
zyklischen Veränderungen (siehe \protect\enquote{der weibliche Zyklus}).
