...
\begin{itemize}
    ...
    Patienten!
    \item Sprechen Sie laut und deutlich!\footnote{Evt. benötigt der
    Patient Hörhilfen. Diese müssen evt. erst eingeschalten werden.}
...
(Arztbrief) Auskunft über bestehende Vorerkrankungen liefern. Allgemeine Fragen,
wie z.B.
\begin{itemize}
    \item \Speech{"`Leiden Sie unter irgendwelchen
    Krankheiten?}
    \item \Speech{"`Bestehen bei Ihnen Vorerkrankungen?}
\end{itemize}
können anschließend durch symptom-spezifische Fragen ergänzt werden:
\begin{itemize}
    \item \Speech{"`Hatten Sie schon einmal einen
    Herzinfarkt?"'}
    \item \Speech{"`Leiden Sie unter hohem Blutdruck?"'}
    \item \Speech{"`Sind Sie zuckerkrank?"'}
\end{itemize}
...
\Layout{28}{Vitale Bedrohung -- Alarmzeichen
...
&
\TabHeading{Typische Symptome}
&
\TabHeading{Gefährliche Diagnosen}
\\\midrule
Bewusstsein
...
\marginpar{\Hand{Bei der Prüfung "`blind"' auf
