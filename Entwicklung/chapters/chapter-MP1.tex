...
\begin{itemize}
  \item \Es{Beschreibung der Medizinprodukte}: \CO{[MP1, MP2]} 
  (\ref{chp:MP1}, \ref{chp:MP2}).
  \item \Es{Untersuchungen}: \CO{[BAU]} (\ref{chp:BAU}).
  \item \Es{Sanitätshilfemaßnahmen}:  \CO{[SAN]} (\ref{chp:SAN}).
...
\Layout{20}{Bestandteile}{%
Ein Beatmungsbeutel besteht üblicherweise aus einem \Es{Luftzufuhrventil}, einem
\Es{Balg} und einem \Es{T-Stück} mit einem Ventil. 

An der Luftzufuhr kann eine
\Es{Sauerstoffleitung} und/oder ein \Es{Sauerstoffreservoir} angeschlossen
werden, um den Sauerstoffgehalt des Einatemgases zu erhöhen. Am
...
Beatmungspausen zwischenspeichert,  kann die \ce{O2}-Konzentration des
Einatemgases weiter gesteigert werden.~\Z{Andere \ce{O2}-Systeme sind am Markt
verfügbar (Demand-Ventile, etc.), aber nur begrenzt verbreitet.}

\Fazit{Wenn vorhanden soll der Beutel mit einer Sauerstoffzufuhr verbunden
werden.}

Am T-Stück gibt es
\E{3 Schenkel}: Einer kommt vom Beatmungsbalg, ein führt zum Patienten und einer
zum Auslass. Das \Es{Ventil} des T-Stückes \E{trennt die Ein- von der 
Ausatemluft} des Patienten. Am Patientenschenkel kann ein \Es{Bakterienfilter}
und Messvorrichtungen (z.\,B. zur \ce{CO2}-Messung) angebracht werden. Er wird
mit einer \Es{Beatmungsmaske} oder einem Tubus -- je nach Bedarf -- verbunden.
Am Ausführungsschenkel kann ein \Es{PEEP}-Ventil aufgesteckt werden. 
}{%
\begin{itemize}
...
{%
Es gibt Beatmungsbeutel als Einweg- und Mehrwegprodukt. Einwegprodukte müssen
nach Gebrauch entsorgt werden. 

Mehrwegprodukte können i.\,d.\,R. durch Desinfektion im
Einlegebad\ZFootnote{Autoklavierung ist meistens auch möglich und
wünschenswert, sofern die entsprechenden Einrichtungen vorhanden sind.}
wiederaufbereitet werden. Es ist unbedingt notwendig, dass der Beatmungsbeutel dazu \Es{komplett zerlegt} wird!
...
Blut oder Schleim kann vom Fachpersonal mittels Absaugpumpen aus dem
...
Ein \Es{Absaugkatheter} ist jenes Zubehör zur Absaugpumpe,
mit welchem die Sekrete aus Mund oder Nase abgesaugt werden.
Es ist ein flexibler, ca. 50\,cm langer Schlauch, der direkt
an die Absaugpumpe angeschlossen wird (siehe Abbildung
in \prettyref{tab:absaugvorrichtungen}). Die
meisten Absaugkatheter haben an jenem Ende das in Mund oder Nase
eingeführt wird eine kleine Öffnung, die das Festsaugen an
den Patienten verhindert. Die Absaugkatheter sind steril
verpackt und sollten auch dementsprechend behandelt werden.
Die farbige Kennung gibt den Durchmesser des Absaugkatheters
an. \Z{Die Farbcodierung kann \prettyref{tab:absaugkatheter}
entnommen werden.}
...
\begin{table}[H]
\begin{gWideParEnv}
\Captionof{table}{Verschiedene
Absaugvorrichtungen.\label{tab:absaugvorrichtungen}}
