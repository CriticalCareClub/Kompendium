...
%%% HEADER %%%
% right
\fancyhead[RO]{{\sffamily{\color{Grey}\CC{[\ChapterCode]}~\rightmark~|~}{\color{DefaultA}\thepage}}}
\fancyhead[RO]{\begin{tikzpicture}[overlay,remember picture]
      \fill [color=AltBlueSuperLight]
        ($(current page.north east) - (0pt,0pt)$)
        rectangle
        ($ (current page.south east) - (191.5pt,0cm) $);
  \end{tikzpicture}{\sffamily{\color{Grey}\CC{[\ChapterCode]}~\rightmark~|~}{\color{DefaultA}\thepage}}
}
...
\fancyhead[LE]{\begin{tikzpicture}[overlay,remember picture]
  ...
  \end{tikzpicture}{\sffamily{\color{DefaultA}\thepage}{\color{Grey}~|~\leftmark}}}
% %%% DRAFT %%%
