...
\end{savequote}
\chapter[{[ANA]} Anatomie und
Physiologie]{[ANA]\\Anatomie und Physiologie}
\label{chp:ANA}
...
verschiedenen Zellorganellen\index{Zellorganellen}. Sie wird von der
Zellmembran\index{Zellmembran} umhüllt. Zellorganellen sind wichtig für den
...
\Fazit{Das, was man landläufig unter dem Begriff "`Gelenk"' versteht, sind
"`echte"' Gelenke mit Gelenksspalt.} 
}
{%
\begin{itemize}
  \item "`echtes Gelenk"'
...
Ein Wirbel kann unterteilt werden in
\begin{itemize}
...
\Layout{0}{\TxBeschreibung}
{
Der Arm beginnt mit dem Schultergelenk. Dieses ist ein klassisches Kugelgelenk
- es ermöglicht die Bewegung in alle drei Raumebenen. Die beteiligten Knochen
im Schulterglenk sind Schulterblatt (Scapula) und Oberarmknochen (Humerus).

Im Ellenbogengelenk setzt sich der Oberarm in den Unterarm fort. Dieses Gelenk
wird aus Oberarmknochen und Elle (Ulna) sowie Speiche (Radius) gebildet. Das
Ellenbogengelenk ist ein zusammengesetztes Gelenk. Die Verbindung zwischen Elle
und Oberarmknochen ist ein Scharniergelenk und ermöglicht das Abwinkeln des
...
Die beiden Knochen des Unterarmes, Elle und Speiche, liegen bei Drehung der
Handfläche nach oben parallel, bei Drehung der Handfläche zum Boden hingegen
überkreuz. Die Speiche liegt daumenseitig, die Elle an der Seite des kleinen
Fingers. 

Nun folgt das Handwurzelgelenk - zwischen Handwurzelknochen und hauptsächlich
der Speiche. Es gibt insgesamt 8 Handwurzelknochen, die in zwei Reihen zu
jeweils 4 Knochen angeordnet sind. Das Handwurzelgelenk ist ein Eigelenk. Es
hat fast so viele Freiheitsgrade wie ein Kugelgelenk - in einer Ebene jedoch
ist die Bewegung durch de Ei-Form eingeschränkt (Kippbewegungen in Richtung von
Daumen und Kleinfinger).

Die Hand beginnt auf Höhe des Handwurzelgelenkes.

Hierauf folgen 5 Mittelhandknochen und dann die 5 Finger mit jeweils 2 (Daumen)
bis 3 Fingergliedknochen. Diese Knochen sind miteinander über Sattelgelenke
bzw. Scharniergelenke verbunden.
}
{
\begin{itemize}
...
\Layout{20}{TxBeschreibung}
{
Das Bein beginnt mit dem Hüftgelenk, einem Kugelgelenk, gebildet aus dem
Beckenknochen und dem Oberschenkelknochen (Femur). Im Gegensatz zum
...
bewerkstelligt. Der Oberschenkelknochen ist gekenzeichnet durch einen "`Knick"'
zwischen Kopf-Hals-Bereich und Körper. Körperfern geht der Oberschenkelknochen
im Kniegelenk in den Unterschenkel - bestehend aus Schienbein (Tibia) und
Wadenbein (Fibula) über.

Das Kniegelenk ist kein reines Scharniergelenk - Drehungen sind bei gebeugtem
Knie möglich. Im Kniegelenk sind nur Oberschenkelknochen und Schienbein
beteiligt - das Wadenbein nicht. Die Besonderheit des Kniegelenkes besteht
einerseits in den Bandbefestigungen - Kreuzbänder (2) und Seitenbänder (2)
werden gemeinsam mit den Menisken (2) häufig bei diversen Sportarten beleidigt.
Andererseits gibt es die Kniescheibe - sie dient der besseren Kraftübertragung
des Kniestreckers.

Schien- und Wadenbein gehen im Sprunggelenk in den Fuß über. Dort gibt es wie
in der Hand Fußwurzelknochen (7). Diese gehen wiederum in die Mittelfußknochen
(5) und dann in die Zehen mit jeweils 2 (Großzehe) bis 3 Gliedern über.
}
{\begin{itemize}
...
Jeder Muskel hat einen Ursprung (Wo kommt er her?)
...
Körper, der Ansatz der Punkt, der weiter weg ist. Der Ursprung ist
...
gibt es natürlich Ausnahmen (z.B. Atemhilfsmuskulatur), darauf soll aber nicht
...
\Layout{20}{\TxBeschreibung}
{%
Das Herz ist ein etwa faustgroßer \Es{Hohlmuskel} (Vorsicht: z.B. bei Kindern
deren Faust als Vergleich - nicht die Eigene!) und hat die Funktion einer
...
\Layout{0}{TxBeschreibung}
{
Das Blut macht ca. \Es{7-8\,\% des Körpergewichtes}
aus, das entspricht in etwa \Es{5-6\,l}. Das Blut besteht aus festen
Bestandteilen und flüssigen Bestandteilen (Blutplasma). Unter den festen
Bestandteilen versteht man die roten Blutkörperchen (Erythrozyten), die weißen Blutkörperchen
(Leukozyten) und die Blutplättchen (Thrombozyten). Das Blutplasma besteht zum
größten Teil aus Wasser. Außerdem enthält es Proteine und andere gelöste Stoffe.

\paragraph{Aufgaben}

Das Blut ist der Sauerstofflieferant des Körpers. Es holt sich den Sauerstoff
in der Lunge ab, bringt ihn zu den Zellen und nimmt auf dem Rückweg das
abzuatmende Kohlendioxid wieder mit. Weiters transportiert es die
über den Darm aufgenommenen Nährstoffe zu den entsprechenden
Weiterverarbeitungsstellen und sorgt für den Abtransport von Abfallprodukten in
Leber oder Niere.

Eine weitere wichtige Aufgabe ist die Regulation der
Körpertemperatur gemeinsam mit den Blutgefäßen. Be Kälte werden die Blutgefäße
...
funktioniert es be großer Hitze. Über eine Weitstellung der Gefäße versucht der
Körper soviel Hitze wie möglich loszuwerden.

Zu guter Letzt sorgt das Blut auch noch für einen ausgeglichenen
Säure-Basen-Haushalt im Körper - der pH des Körpers sollte immer zwischen 7,35
und 7,45 liegen. Bei Abweichungen sorgen bestimmte Puffersysteme im Blut für
eine Wiederherstellung dieses pH-Wertes.
} 
{
\begin{itemize}
  \item 5-6 Liter
...
\begin{table}[H]
\begin{gWideParEnv}
\Captionof{table}{Übersicht: Die Bestandteile des Blutes.}
...
Die Erythrozyten tragen die Blutgruppenmerkmale (A bzw. B) und die
Rhesusfaktoren. Das Kennzeichen der Erythrozyten ist die Abwesenheit des Kernes
bzw. jeglicher Zellorganellen.

Nach einer Lebensdauer von ca. 100 Tagen werden die "`Senioren"' der roten
Blutkörperchen in der Milz abgebaut.

Sie sind außerdem die häufigsten festen Blutbestandteile (4\,000\,000\,/$\mu$l).
}
{
\begin{itemize}
...
\Layout{0}{TxBeschreibung}
{
Die weißen Blutkörperchen dienen der Infekt- und Fremdkörperabwehr des Körpers.
Es gibt verschiedene Arten von Leukozyten:

\begin{itemize}
  \item Granulozyten: Diese Zellen bilden den Beginn der Immunabwehr. Es gibt
  je nach Erkrankung verschiedene "`Spezialisten"' unter den Granulozyten -
  neutrophile G. (bei Keimen und Fremdkörpern), eosinophile G. (bei Allergien
  und Parasiten) und basophile G.
  \item Lymphozyten: in diese Gruppe fallen Antikörper-bildende Zellen
  \item Monozyten: können sich durch zelleigene Bewegungen im Gewebe bewegen
  und Keime bzw. Fremdkörper verdauen.
\end{itemize}

Es befinden sich zwischen 4\,000 und 8\,000 Leukozyten pro $\mu$l im Blut des
Körpers.
}
{
\begin{itemize}
...
\paragraph{Thrombozyten}
\label{topic:thrombozyten}
\index{Thrombozyten}%

\Layout{0}{TxBeschreibung}
{
Die Blutplättchen sind kleine kernlose Scheibchen. Sie dienen der Blutgerinnung
und des Wundverschlusses.

Ihre Anzahl wird mit 150\,000 bis 300\,000 pro $\mu$l beziffert.
}
{
\begin{itemize}
...
\noindent Das ZNS ist im Wesentlichen für Steuer-, Regel- und Denkfunktionen
...
Einheit.Gemeinsam dekodieren sie die Signale aus der Peripherie. Sie wählen
diejenigen aus, die entweder unterbewusst verarbeitet oder bewusst
wahrgenommen werden sollen. Das Zentralnervensystem erzeugt schließlich
Signale, die meist als Antwort auf einen Reiz in die
...
\begin{itemize}
    \item Gehirn
    \begin{itemize}
        \item Großhirn: Das Großhirn besteht aus 2 Hälften, sie stehen über
        ...
        \item Zwischenhirn: Zum Zwischenhirn gehören unter anderem wichtige
        Regulationskerne wie Thalamus und Hypothalamus
        \item Kleinhirn: 2 Häften, feinere Windungen und Furchen auf der
        Oberfläche
        \item Hirnstamm: Ursprung der Hirnnerven, wichtige vegetative
        Funktionszentren
    \end{itemize}
    \item Rückenmark: Vermittler zwischen Körperperipherie und Gehirn
\end{itemize}
...
\Layout{0}{Zwischenhirn}
{
Im Zwischenhirn laufen alle Signale aus dem Körper zusammen. Der wichtigste
...
Funkionen des Körpers geregelt.
}
{
\begin{itemize}
...
einen Eigenapparat eingehende Signale verarbeitet werden (hierunter versteht
man Eigen- und Fremdreflexe).

Das Rückenmark lässt sich durch abgehende Spinalnerven in Segmente (insg. 31)
einteilen. Es gibt 8 Halssegmente, 12 Thorakalsegmente, 5 Lumbalsegmente und 5
...
\Layout{22}{Der Kopf besteht aus Schichten}
{
%\index{Epiduralraum}
%\index{Dura mater}
...
Körper kolabieren bei Verletzung. Dadurch kommt es auch bei relativ geringen
Kopfverletzungen zu vergleichsweise hohen Blutverlusten.

Hierauf folgt die Schädelkalotte. Diese ist innen fest mit der harten Hirnhaut
(Dura mater) verbunden - sie ersetzt hier das Periost des Knochens. In die Dura
...
Nervensystem und den peripheren Organen.

Es werden Hirnnerven und Spinalnerven unterschieden. Hirnnerven (insgesamt 12
Paare) sind Nerven, die ihren Ursprung im Hirnstamm haben. Global sind sie für
die Versorgung vom Kopf zuständig. Spinalnerven dagegen entspringen im
Rückenmark segmentweise. Für die Extremitäten bilden sie jeweils Nervengeflechte, sogenannte Plexus
(Plexus cervicalis, Plexus brachialis, Plexus lumbalis und Plexus sacralis).

Die Fortleitung innerhalb eines Nerven funktioniert über elektrische Impulse.
Zwischen Nerven und anderen Nerven bzw. den Zielorganen dienen chemische
Signale der Weiterleitung (in sog. Synapsen).
}
{
\begin{itemize}
...
\Layout{0}{\TxBeschreibung}
{
Motorische Nerven sorgen dafür, dass ein "`Bewegungswunsch"' in den
entsprechenden Muskeln ankommt. Auf das Nervensignal hin kommt es zur
Kontraktion des Muskels.

Sensorische Nerven liefern dem Gehirn Sinneseindrücke. Diese können von den
diversen Sinnesorganen kommen. So gehören Tastempfindungen,
...
Vegetative Nerven sind sozusagen die Drahtzieher im Hintergrund. Sie sorgen für
den reibungslosen Ablauf unbewusster Funktionen, wie z.B. die Herzfunktion, die
Verdauung und die Nierenfunktion. Die vegetativen Nerven können je nach ihrer
Funktion - global stimulierend oder hemmend -  in zwei Systeme unterteilt
werden: Sympathikus oder Parasympathikus.
}
{
\begin{itemize}
...
\Layout{0}{\TxBeschreibung}
{
Die Haut ist DIE Barriere gegen Einflüsse aus der Umwelt. Sie verhindert
...
\Layout{20}{Die Haut ist in Schichten aufgebaut}
{%
\index{Oberhaut}\index{Lederhaut}\index{Unterhaut}
Die oberflächlichste Schicht der Haut ist die Oberhaut (Epidermis). Sie besteht
aus einer mehr oder weniger dicken Hornhaut (am Dicksten an Handflächen und
Fußsohlen, am Dünnsten an den Augenlidern) und besitzt keine eigene
Gefäßschicht.

Darauf folgt die Lederhaut (Dermis). In ihr sind alle Hautanhangsgebilde
(Schwißdrüsen, Talgdrüsen, Haare und Nägel) befestigt. Weiters befinden sich in dieser
Hautschicht Tastkörperchen sowie freie Nervenendigungen (Schmerzempfindungen).
Außerdem finden sich hier zwei verschiedene Gefäßsysteme (eines
oberflächlicher, eines tiefer). Dieses dient wie bereits erwähnt der
Temperaturregulation.

Unter der Dermis befindet sich die Unterhaut (Subcutis). Sie besteht
...
dient dem Transport über den Mund aufgenommener Nahrung vom Rachen in den Magen. Diese
Fortbewegung funktioniert über eine wellenförmige Kontraktur der Wandmuskeln
der Speiseröhre. Im Ösophagus werden keine verdauenden Enzyme ausgeschieden -
er dient rein zur Fortbewegung des Speisebreies.

Morphologisch weist die Speiseröhre 3 Engen auf, die die Nahrung überwinden
...
In der Wand des Ösophagus finden sich zahlreiche Venen. Bei fortschreitender
Leberzirrhose (Umbau der Leber in Bindegewebe durch z.B. chronische Entzündung)
und damit resultierendem Verschluss der Pfortader fungieren diese Venen als
Umgehungskreislauf in die Vena cava. Wenn eine sogenannte Ösophagusvarize
platzt kann es zu einem großen Blutverlust kommen.
}
{
\begin{itemize}
    \item ca. 25\,cm langer Muskelschlauch
...
\Layout{0}{\TxBeschreibung}
{
Der Magen ist ein muskuläres Hohlorgan. Er beeinflusst Nahrungsspeicherung,
Nahrungsdurchmischung sowie Weiterleitung des Speisebreis.

Verantwortlich für den Beginn der Verdauung der Nahrung ist der durch die
Magenschleimhaut sezernierte Magensaft. } {
\begin{itemize}
