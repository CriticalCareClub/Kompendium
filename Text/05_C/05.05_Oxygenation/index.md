...
2. Der **Sauerstoffträger**, im Wesentlichen {ref}`Hämoglobin <Haemoglobin>`, und der Hüfner'schen Zahl, welche einen Umrechnungsfaktor darstellt. Sie gibt an, wieviel Milliliter Sauerstoff als Absolutwert pro Gramm Hämoglobin gebunden und transportiert werden kann[^huefnerschezahl]: $\color{purple}\rm{Hb} \times 1,34$

3. Der **Kreislauf** ({ref}`Kreislauf Ⓒ <Circulation>`), welcher durch den Cardiac Output $\color{red}\rm{CO}$ bestimmt wird.
   Der Cardiac Output ergibt sich aus Schlagvolumen und Frequenz: ${\color{red}\rm{CO}}={\color{red}{\rm{SV}} \times f}$
...
Die ausreichende Oxygenierung der Zellen ist immer ein Balaceakt:
