...
{%
Der \T{Sinusknoten} \Es{(1)} ist normalerweise der Impulsgeber
(\E{natürlicher "`Schrittmacher"'}), er feuert mit einer Eigenfrequenz von ca.
\VonBis{60}{100}\,\nicefrac{}{min} elektrische Impulse (Erregungen) ab. 

Eine solche Erregung wird über den rechten Vorhof zum  \T{AV-Knoten} \Es{(2)}
...
Im nächsten Schritt gelangen die Impulse über die Tawara-\Es{Schenkel} in die
rechte und linke Herzkammer, wo sie über die Purkinje-\Es{Fasern} die Muskulatur
erreichen.
...
\Es{dicht abgeschlossen}, es kommt von außen keine Luft dazu. Dadurch haften die
beisen Blätter aneinander, bei Zug (durch Unterdruck während der Atmung) an einem der
beiden Blätter lassen sich diese nicht trennen, folglich wird das Zweite passiv
...
Brustkorbs folgt, jedoch nicht est fixiert ist, sondern verschieblich bleibt. 
...
außerdem die einziger Struktur der Lunge, die schmerzempfindlich ist.} }
{
\begin{itemize}
...
\index{Atemhilfsmuskulatur}% Neben den Hauptatemmuskeln (Zwerchfell,
Zwischenrippenmuskulatur) gibt es noch eine Reihe weiterer Atemhilfsmuskeln die
die Atmung unterstützen. Diese verbinden den Thorax und die Oberarme. Fixiert man
...
dient dem \Es{Transport} über den Mund aufgenommener Nahrung vom Rachen in den
...
Hier \Es{münden 2 Ausführungsgänge} ein, der \Es{Gallengang} und der
\Es{Pankreasgang}. Aus dem Gallengang wird die \Es{Gallenflüssigkeit} aus der
\Es{Leber} abgegeben, welche für die \Es{Fettverdauung} wichtig ist (Emulsion
der Fette); aus dem \Es{Pankreas} wird der \Es{Bauchspeichel}, der Verdauungsenzyme
...
im \Es{rechten Oberbauch}. Sie hat einen linken und rechten Lappen. Sier ist die
...
Die Leber produziert u.\,a. wichtige, vom Körper benötigte, \Es{Eiweißstoffe}
(Proteine). 

\index{Pfortader}
Weiters sorgt sie für den Abbau von Giftstoffen (\Es{Entgiftung}).
...
Die Leber ist auch eine \Es{Verdauungsdrüse}: Sie produziert
die \T{Gallenflüssigkeit}
(\Speech{"`Galle"'}), welche der  \Es{Fettverdauung} dient.
...
Außerdem dient die Leber der \Es{Speicherung} von Fetten und Zucker.
}{
\begin{enumerate}
  \item "`Chemiefabrik"':
  \begin{itemize}
...
ist sie auch an der Blutbildung beteiligt; daher ist sie \Es{sehr gut
durchblutet}!

Sie ist weiters von einer festen \Es{Kapsel} umgeben.
}{
\begin{itemize}
