---
status: final
---


:::{index} pair: Absaugung; OP
:::
:::{index} single: Absaugpumpe
:::
:::{index} single: Yankauer
:::

(MP-Absauggeraete)=

# Absaugung


:::{seealso}

-   {ref}`Geringinvasive Ⓐ-Maßnahmen, Absaugung <Absaugung>`
:::


### Absaugungsvorrichtungen im OP

Absaugungssysteme dienen der Entfernung von Blut, Sekreten und Spüllösungen aus dem Operationsgebiet sowie der Sicherung der Atemwege.


Systemkomponenten
:   -   Vakuumquelle (zentrale Wandabsaugung oder mobile Absaugpumpe)
    -   Auffangbehälter (Sekretbehälter)
    -   Verbindungsschläuche
    -   Absaugkatheter bzw. Absaugaufsätze


Absaugaufsätze
:   -   Yankauer-Sauger
        -   Starres Rohr, atraumatisch
        -   Verwendung im OP-Feld (z. B. Blut, Spülflüssigkeit)
    -   Flexible Absaugkatheter
        -   Für endotracheale/tracheale Absaugung
    -   Feine Absaugkanülen
        -   Präzisionsarbeit (z. B. Neurochirurgie, HNO)


Anwendungsbereiche
:   -   Freihalten des OP-Feldes
    -   Absaugung von Blut und Sekreten
    -   Atemwegssicherung (z. B. bei Intubation)
    -   Absaugung von Spülflüssigkeiten


Sicherheitsaspekte
:   -   Sogstärke situationsgerecht einstellen
    -   Vermeidung von Gewebetrauma
    -   Regelmäßige Kontrolle auf Verstopfung oder Leckage
    -   Hygienische Handhabung (Kontaminationsvermeidung)


:::{subfigure} AB
:subcaptions: below
:name: Figure-Absaugvorrichtungen
:class-grid: outline
:gap: 20px

Bilderserie: Verschiedene Absaugvorrichtungen [© Ch. Pallinger, {term}`ℓ MfG`]

![Elektrische Absaugeinheit der Fa. Weinmann (Accuvac)](../../../Submodules/3134-medical-picture-collection/Pallinger-Christoph/Accuvac_32820_crop.000800px.jpg)

![Absaugkatheter zum Einführen in Mund oder Nase](../../../Submodules/3134-medical-picture-collection/Pallinger-Christoph/Absaugkatheter-Set_33082_cropped.000800px.jpg)
:::


:::{subfigure} AB
:subcaptions: below
:name: Fig-BS-Absaugung
:class-grid: outline
:gap: 20px

Absaugung

![Absaugung](../../../Submodules/3134-medical-picture-collection/Gabriel-Sebastian/Anaesthesia/ADU/GE-S5/IMG_115409.001341px.jpg)

![Absaugung (off)](../../../Submodules/3134-medical-picture-collection/Gabriel-Sebastian/Anaesthesia/Equipment/IMG_115426.001341px.jpg)

:::


