...
\E{Flüssigkeit}, das Zellplasma. Die Zelle wird von der
...
Zellverband bilden, welcher \T{Gewebe} genannt wird. Diese
...
{%
\subparagraph{\T{Periost}} \E{Beinhaut}, überzieht den Knochen und
...
\index{Kortikalis}feste äußere Schicht,
äußere Form, Verantwortlich für Stabilität des Knochens
\subparagraph{\T{Spongiosa}} \index{Knochenbälkchen}Knochenbälkchen
${\rightarrow}$ Zug- und Druckfestigkeit
\subparagraph{\T{Markhöhle}} s.\,u.
\subparagraph{\T{Epiphysenfuge}} Wachstumsfuge
...
verursachen als Nebenwirkung eine massive \EE{Schädigung
...
Knorpelgewebe}, daher ist ein \EE{Längenwachstum}
...
{%
Der Hirnschädel besteht aus der \EE{Schädelbasis} und dem \EE{Schädeldach}
...
Schädel aus. \EE{Das \T[Großes Hinterhauptsloch]{große
Hinterhauptsloch}\index{Hinterhauptsloch!Großes}  \Z{(Foramen magnum)} ist die
Durchtrittsstelle des Rückenmarks.} 

\subparagraph{Exkurs: \T{Hirnstammeinklemmung}}
\index{Hirnstammeinklemmung} 
Das \EE{große Hinterhauptsloch} ist die Durchtrittsstelle des Rückenmarks. Bei
einer \EE{Hirndrucksteigerung} (\E{Hirnödem, Hirnblutung}) ist es die einzige
Möglichkeit zur Ausdehnung des Gehirns! \EE{Dabei drückt sich das Gehirn
allerdings selber ab}, es kommt zu einer tödlichen \Ca{Hirnstammeinklemmung!}
${\rightarrow}$ \Ca{Lebensgefahr}, da das Kreislauf- und Atemzentrum abgedrückt
wird.
}
{
\begin{itemize}
...
Zwischen den
einzelnen Wirbeln finden sich \T{Bandscheiben}
(\T{Discus}), welche als \EE{Stoßdämpfer} dienen. Mit zunehmenden Alter können
...
Es gibt 
\begin{inparaitem}
    \item 7 \EE{Hals}wirbel \Z{(Cervical-Wirbel)},
    \item 12 \EE{Brust}wirbel  \Z{(Thorakal-Wirbel)},
    \item 5 \EE{Lenden}wirbel  \Z{(Lumbal-Wirbel)},
    \item 5 verschmolzene \EE{Kreuzbein}wirbel \Z{(Os sacrum)}, sowie
    \item 3-4 \EE{Steißbein}"`wirbel"' \Z{(Os coccygis)}.
\end{inparaitem}
...
\begin{itemize}
    ...
    \item 5 \EE{Lenden}wirbel  \Z{(Lumbal-Wirbel)}
    \index{Lendenwirbeln}
    \item 5 verschmolzene \EE{Kreuzbein}wirbel \Z{(Os
    sacrum)} \index{Kreuzbeinwirbel}
    \item 3-4 \EE{Steißbein}"`wirbel"' 
...
\E{Bandscheiben}. Der Wirbelbogen \EE{schützt das Rückenmark} vor Verletzungen,
...
Der  Brustkorb besteht \E{vorne} aus dem \T{Brustbein}
 (\T{Sternum}) und \EE{12
Rippenpaaren}, welche \E{hinten} mit der \EE{Brustwirbelsäule} (12
...
\T{Zwischenrippenmuskulatur}\index{Zwischenrippenmuskulatur} (wichtig für \EE{Atemmechanik}), in diese sind segmentale
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \Z{2 "`fliegende"'}
    \end{itemize}
    \item \EE{Brustwirbelsäule} (12 Brust\-wirbel), mit Rippen in gelenkiger Verbindung
\end{itemize}
}
...
aus \EE{Oberarmknochen} und \EE{Elle} (Ulna) sowie \EE{Speiche} (Radius)
...
Die beiden Knochen des \EE{Unterarmes} sind die \T{Elle} (Ulna) und die
\T{Speiche} (\T{Radius}) sind, \Z{sie liegen bei Drehung der Handfläche nach
oben parallel, bei Drehung der Handfläche zum Boden hingegen überkreuz.} Die \EE{Speiche} (Radius) liegt
\EE{daumenseitig}\footnote{Daher ist die Radialarterie daumenseitig zu tasten!},
die Elle an der Seite des kleinen Fingers.

Es folgt das \EE{Handwurzelgelenk}, welches hauptsächlich aus
der Speiche und den \E{Handwurzelknochen} gebildet wird. Das Handwurzelgelenk
ist ein \EE{Eigelenk}, \Z{es hat fast so viele Freiheitsgrade wie ein Kugelgelenk, in
...
Hierauf folgen \EE{5 \T{Mittelhandknochen}} und dann die 5 Finger mit jeweils 2
...
Sattelgelenke, die \EE{Fingergliedgelenke} sind \EE{Scharniergelenke}.
}{
\begin{itemize}
...
\index{Schenkelhals}
Das Bein beginnt mit dem \T{Hüftgelenk}, einem \EE{Kugelgelenk}, gebildet aus
...
bei diversen Sportarten verletzt. Andererseits gibt es die \EE{Kniescheibe}, sie
...
Als \EE{Knöchel} bezeichnet man die Knochengabel, welche aus Schien- und
Wadenbein auf Höhe des Sprunggelenks gebildet wird. Es gibt einen Innen- und
einen Außenknöchel (jeweils körperfernes Ende von Schien- bzw. Wadenbein). Bei
einer Verletzung durch Umknicken des Fußes nach außen (häufiger) bzw. innen kann
...
\begin{itemize}
    ...
    steuerbar, trainierbar, nicht ermüdbar, quergestreift.
...
Jeder Muskel hat einen \EE{Ursprung} (\Speech{"`Wo kommt er her?"'})
und einen \EE{Ansatz} (\Speech{"`Wo geht er hin?"'}), diese beiden Punkte bestimmen
die Bewegung, die der Muskel machen kann. Der Ursprung ist
definitionsgemäß immer der Punkt, der stillgehalten wird. Der Ansatz
dagegen wird durch den Muskelzug auf ihn zu bewegt. Normalerweise liegt der
Ursprung eines Muskels näher beim Körper, der Ansatz ist der Punkt, der weiter
entfernter ist. Für alle dieser Erklärungen gibt es natürlich Ausnahmen (z.\,B.
Unterstützung der \E{Atemhilfsmuskulatur}).
}
{
\begin{itemize}
...
\EE{Hohlmuskel} und hat die Funktion einer
\EE{Pumpe}. Es liegt \EE{hinter dem Sternum\footnote{Brustbein}}
...
Brustkorb. Unten liegt es auf \EE{dem Zwerchfell auf}.
...
aus. Dabei pumpt es ca. \VonBis{4}{6}\,l  Blut (bzw. 1\,x das
...
\Layout{0}{\TxBeschreibung}{%
Das Herz besitzt ein eigenständiges Erregungszentrum, welches elektrische
Impulse über ein Reizleitungssystem zum Herzmuskel leitet (\E{elektrische
Herzfunktion}). Jeder dieser Impulse wird normalerweise vom Herzmuskel mit einer
...
{%
Der \T{Sinusknoten} \EE{(1)} ist normalerweise der Impulsgeber
(\E{natürlicher "`Schrittmacher"'}), er feuert mit einer Eigenfrequenz von ca.
\VonBis{60}{100}\,\nicefrac{}{min} elektrische Impulse (Erregungen) ab. 

Eine solche Erregung wird über den rechten Vorhof zum  \T{AV-Knoten} \EE{(2)}
geleitet, dort gebündelt, und über das \T{His-Bündel}, welches sich im
Bereich der Vorhof-Kammer-Grenze befindet, in Richtung der Kammern
weitergeleitet. Der AV-Knoten und das His-Bündel \E{filtern}  und \E{verlangsamen} die
Erregungsleitung, um zu verhindern, dass zu viele Impulse die Herzkammern
erreichen. 

Im nächsten Schritt gelangen die Impulse über die Tawara-\EE{Schenkel} in die
rechte und linke Herzkammer, wo sie über die Purkinje-\EE{Fasern} die Muskulatur
erreichen.

Bei einem \E{Ausfall} eines Knotens, bzw. bei Unterbrechung der Erregungsleitung
(z.\,B. durch Gewebeschädigung nach einem Herzinfarkt), kann die jeweils
\E{nächste Station als Impulsgeber übernehmen} -- allerdings mit deutlich
langsamerer Frequenz\ZFootnote{D.\,h. jede der Stationen ist zur eigenständigen Erregungsbildung
fähig, sofern ihr nicht die übergeordnete Station zuvor kommt.}. Im
Extremfall, wenn das gesamte Leitungssystem ausfällt, können auch die
Herzmuskelzellen selbst einen minimalen Rhythmus
($\sim$\,30\,\nicefrac{}{min}) aufrecht erhalten.
...
\begin{itemize}
  ...
  \item Nachfolgende Stationen können bei Bedarf "`übernehmen"' (HF\,↓)
\end{itemize}
...
\Fazit{\EE{Herz + Gefäßsystem + Blut = Kreislauf}}
...
Der Kreislauf wird vom \EE{Stammhirn} gesteuert
...
\EE{Arteriolen} und haardünnen Gefäßen, den
Haargefäßen ("`\EE{Kapillaren}"').
...
\EE{Venolen} in das Venensystem und fließt in den
\EE{Venen} herzwärts. Am Weg zum Herzen nehmen die Venen
an Umfang immer mehr zu, bis sie letztendlich in die großen Venen
({\quotedblbase}\EE{obere} und \EE{untere
...
\begin{labeling}[\dots{}]{Kapillargefäßexxx}
    \item[\T{Arterien}] führen \E{vom Herzen \EE{weg}} und haben eine dicke
...
sein. Bei \EE{Dialyse-Patienten}  wird oft künstlich -- mittels einer Operation
...
Shunt \EE{nicht gestaut} werden; das betrifft insbesonders die
...
\Layout{0}{\TxBeschreibung}
{%
Das Blut macht ca. \EE{\VonBis{7}{8}\,\% des Körpergewichtes}
aus, das entspricht in etwa \EE{\VonBis{5}{6}\,l}. 
} 
{
\begin{itemize}
...
\EE{festen} Bestandteilen und \EE{flüssigen Bestandteilen} (Blutplasma). 
...
\Layout{0}{Aufgaben}{%
Die wichtigste Aufgabe des Blutes ist der \EE{Stoff- und Gastransport}. Es
...
\EE{Säure-Basen-Haushalt} im Körper -- der pH des Körpers sollte immer zwischen
...
\\
\noindent\Z{($\sim$\,4\,000\,000\,/\,{\textmu}l)}
&
\noindent\Z{($\sim$\,4\,000\,--\,8\,000\,/\,{\textmu}l)}
&
\noindent\Z{($\sim$\,150\,000\,--\,300\,000\,/\,{\textmu}l)}
&
\noindent$\sim$\,90\,\%
&
\noindent$\sim$\,8\,\%
...
(4\,000\,000\,/$\mu$l) und tragen die \EE{Blutgruppenmerkmale} (A, B bzw. 0 bei
Fehlen der Merkmale) und die Rhesusfaktoren. Es gibt sog.
\T{Erythrozytenkonzentrate} ("`Ery-Konzentrate"'), das sind \EE{Blutkonserven}, welche fast ausschließlich Erythrozyten enthalten.
...
\begin{itemize}
    ...
    \item Häufigsten Blutkörperchen \Z{(4\,000\,000\,/\,{\textmu}l)}
\end{itemize}
}{}{}{}
...
Die weißen Blutkörperchen sind ein wichtiger \EE{Teil des Immunsystems} und
...
verschiedene Arten von Leukozyten%
% % Aus Layout-technischen Gründen vorübergehend auskommentiert
% \ZFootnote{\begin{inparaitem}
%   \item \E{Granulozyten}: Sie bilden den Beginn der Immunabwehr. Es
...
%   \item \E{Monozyten}: Sie Können sich durch zelleigene Bewegungen im Gewebe
%   bewegen und Keime bzw. Fremdkörper verdauen.
% \end{inparaitem}}
.
}
}
...
\EE{Blutgerinnung} und dem Wundverschluss. \Z{Ihre Anzahl wird mit 150\,000 bis
300\,000 pro $\mu$l beziffert.} 
Es gibt \T{Thrombozytenkonzentrate}, das sind
\EE{Blutkonserven}, welche fast ausschließlich Thrombozyten beinhalten. 
}
{
\begin{itemize}
    \item Wichtige Rolle beim Wundverschluss
    \item Zelluläre Blutstillung
    \item \Z{(150\,000\,--\,300\,000\,/\,{\textmu}l)}
...
Die wesentliche Aufgabe des Plasmas ist der Transport von Nährstoffen an ihren
Zielort im Körper. Es enthält weiters die Bestandteile für die plasmatische
Blutgerinnung und das Immunsystem (Antikörper, Immunglobuline). Durch die
Elektrolyte ist es am Wasserhaushalt beteiligt und sorgt mit
Puffersubstanzen für einen ausgeglichenen pH-Wert im menschlichen Körper.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Wärmeverteilung
    	\item Proteine und Enzyme für \EE{Plasmatische Blutgerinnung} (Serum)
...
Informationen. Zum ZNS gehören \EE{Gehirn} und \EE{Rückenmark}. 
...
\EE{Gehirn} und \EE{Rückenmark} sind morphologisch und funktionell eine
...
\EE{Signale}, die -- oft als Antwort auf einen Reiz -- in die
...
Die beiden Großhirnhälften sind über den Balken miteinander verbunden.
...
\Layout{0}{Rückenmark}
{
Das Rückenmark bildet die \EE{Verbindung zwischen Gehirn und Peripherie}.
...
\EE{beide Richtungen} übertragen, d.\,h. vom ZNS zu den Organen
...
Die \EE{meisten Nerven entspringen segmentweise dem Rückenmark}
...
\T{Sensorische Nerven} liefern dem Gehirn \EE{Sinneseindrücke}. Diese können von
...
für den reibungslosen \EE{Ablauf unbewusster Funktionen}, wie z.\,B. die
...
werden: In den \EE{Sympathikus} und \EE{Parasympathikus}.
}
{
\begin{itemize}
    \item motorisch  
...
\Layout{0}{\TxBeschreibung}
{
Das Auge als Sehorgan ist ein spezialisiertes Sinnesorgan, dass sensibel auf
Lichtreize reagiert. Es besteht aus:

\begin{itemize}
  \item Augapfel
  \item Hilfseinrichtungen: Augenmuskeln, Augenlider und Tränendrüsen
\end{itemize} 

Das gesamte Sehorgan befindet sich in der Augenhöhle. Die lichtempfindlichen
Zellen liegen auf der Netzhaut des Auges und werden nach einer Passage durch die
lichtbrechenden Struckturen des Augapfels erreicht.

Der Augapfel hat eine annähernde Kugelform. Von außen nach innen hat er
verschiedene Schichten:

\begin{itemize}
  \item Lederhaut (Sklera): Die Hornhaut ist eine derbe Bindegewebsschicht. Sie
  ist zusammen mit dem Glaskörper verantwortlich für die Formgebung des Auges.
  \item Aderhaut (Choroidea): In der Aderhaut verlaufen alle Blutgefäße des
  Auges.
  \item Netzhaut (Retina): In der Netzhaut befinden sich die Sinneszellen des
  Auges. Stäbchen für das Schwarz-Weiß- bzw. Dämmerungs-Sehen und Zapfen für das
  Farb-Sehen.
\end{itemize}
...
\begin{itemize}
  \item Hornhaut (Cornea): Die Hornhaut ist eine durchsichtige Schicht von
  Bindegewebe. 
  \item Regenbogenhaut (Iris): Die Iris ist der verschiedenfärbige (blau,
  braun, grün) bereich rund um die Pupille. Sie ist eine dünne Haut mit einer
  eigenen Muskulatur - vegetativ gesteuert - und sorgt dafür, dass bei Bedarf
  die Pupille vergrößert bzw. verkleinert wird.
\end{itemize}

Wenn man einem Lichtstrahl folgt, der in das Auge einfällt, muss dieser zuerst
die Hornhaut durchdringen, darauf folgt die Vorderkammer (der Bereich zwischen
Hornhaut und Iris). Nun durchdringt er das wichtigste Element des Auges, die
Linse. Sie sorgt dafür, dass wir scharf sehen können.

Die Linse hat einen eigenen Aufhängungsapparat, der sie schmäler (für die
Fernsicht) und breiter (für die Nahsicht) machen kann. Nach der Linse folgt der
Glaskörper. Er hat eine gallertartige Konsistenz und verflüssigt sich mit dem
Alter. Zusammen mit der Lederhaut bestimmt er die pralle Form des Auges.

Nun landet der Lichtstrahl auf der Netzhaut. Die Informationen der Sinneszellen
(Stäbchen und Zapfen) werden im Sehnerv gesammelt und an das Sehzentrum im
Gehirn weitergeleitet.

Wichtig für die Funktion des Auges sind weiters die Augenmuskeln - sie drehen
das Auge in jede Richtung und liegen ebenfalls in der Augenhöhle. Für die
Befeuchtung des Auges gibt es die Tränendrüse, die einen Flüssigkeitsfilm auf
dem Auge produziert und der über den Tränen-Nasengang abfließen kann, und die
Bindehaut (Konjunktiva), die bis auf die Hornhaut die gesamte Vorderseite des
Auges und die Innenseite der Lider überzieht. Um das Auge beim Schlafen vor Austrocknung zu
schützen und den Tränenfilm gleichzeitig verteilen zu können, verfügen wir über
Augenlider, die willkürlich und unwillkürlich geschlossen werden können.
}
{
\begin{itemize}
  \item Schichten des Auges:
  \begin{itemize}
    \item Lederhaut (Sclera)
    \item Aderhaut (Choroidea)
    \item Netzhaut (Retina)
  \end{itemize}
  \item Lichtstrahl von vorne nach hinten:
  \begin{itemize}
    \item Hornhaut (Cornea)
    \item Vorderkammer
...
\Layout{0}{\TxBeschreibung}
{
Das Ohr ist das Organ für die akustische Sinneswahrnehmung des Körpers. Zum Ohr
gehört weiters das Gleichgewichtsorgan.

Man kann das Ohr in drei verschiedene Untergruppen unterteilen - Außenohr,
Mittelohr und Innenohr.

Zum Außenohr gehören Ohrmuschel und äußerer Gehörgang. Die Trennlinie ist hier
das Trommelfell, dass das Außenohr hermetisch vom Mittelohr abschließt. Im
Mittelohr befinden sich die Gehörknöchelchen. Es hat zum Druckausgleich eine
Verbindung zur Nase. So funktioniert es auch, dass man beispielsweise bei einem
Schnupfen durch Verstopfung des Ohr-Nasen-Ganges und surch aufsteigende Keime
eine Mittelohrentzündung bekommen kann. Mit dem Übergang der Gehörknöchelchen
zur Schnecke passiert man die Grenze zum Innenohr mit der Hörschnecke (Cochlea)
und dem Gleichgewichtsorgan. Das Innenohr befindet sich im Felsenbein.

Wenn man einer Schallwelle folgt, die von der Ohrmuschel eingefangen wurde,
durchläuft man folgende Strukturen der Reihe nach:

\begin{itemize}
  \item äußerer Gehörgang: dieser geht nach vorne oben - daher die Hörer des
  Stetoskopes immer in diese Richtung schauend einsetzen - sonst hört man
  nichts!
  \item Trommelfell: Es dient der Schallweiterleitung und zur Abgrenzung des
  Mittelohres und Innenohres gegen die Außenwelt.
  \item nun folgen die Gehörknöchelchen:
  \begin{itemize}
    \item Hammer (Malleus): ist fest mit dem Trommelfell verwachsen, ist
    teilweise durch das Trommelfell hindurch sichtbar, verbunden mit dem
    \item Amboss (Incus): dieser gibt die erhaltenen Schwingungen weiter an den
    \item Steigbügel (Stapes): ist mit der Schnecke verbunden
  \end{itemize}
  \item Schnecke (Cochlea): ist mit einer eigenen Flüssigkeit gefüllt. In der
  Schnecke werden die empfangenen Schallwellen nach ihrer Frequenz aufgelöst und durch
  die Sinneszellen über den Gehörnerven an das Hörzentrum im Gehirn
  weitergeleitet.
\end{itemize}

Das Gleichgewichtsorgan, als Teil des Innenohrs in direkter Nähe der
Schnecke im Felsenbein liegend, ist dafür zuständig, dass man ein
Gefühl der eigenen Lage im Raum hat und Beschleunigungen feststellen
kann. Hierfür hat es drei Bogengänge, die nach den drei Achsen im Raum
...
\Z{Sie macht ca. \nicefrac{1}{6} des Körpergewichts aus, ihre Oberfläche
...
Körperkerntemperatur bei wechselnden Witterungsverhältnissen.
...
Die
oberflächlichste Schicht der Haut ist die \T{Oberhaut} (Epidermis). Sie
besteht aus einer mehr oder weniger dicken \E{Hornhaut} (am Dicksten an
Handflächen und Fußsohlen, am Dünnsten an den Augenlidern) und besitzt keine
eigene Gefäßschicht.
...
(\EE{Schmerzempfindungen}). Außerdem finden sich hier zwei verschiedene
...
zur \EE{Thermoregulation}: Die Blutgefäße  der  Haut
...
\begin{itemize}
    ...
    \item \index{Lungenbläschen}\EE{Lungenbläschen}
...
sorgt dafür, dass die beiden Blätter seitlich gleiten können. 
...
\Fazit{Wenn
der Brustkorb mit dem daran verwachsenen Rippenfell "`anzieht"', folgt das
Lungenfell mit der Lunge der Bewegung → die Lunge wird wie eine Ziehharmonika
...
\begin{itemize}
    ...
    \item Brustkorb mit Rippenfell "`zieht an"' → Lungenfell mit Lunge folgt
    Bewegung     
\end{itemize}
}
...
"`Pneumothorax"' (\prettyref{topic:pneumothorax}).
\index{Pneumothorax!anatomische Erklärung}})


\Layout{0}{Welche Funktion hat aber nun die Pleura?}
{
...
aufstützen, können die Muskeln die Hebung des Brustkorbes unterstützen. Vgl.
dazu \emph{'Asthma', \ref{topic:asthma-bronchiale}} und \emph{'COPD',
\ref{topic:copd}}.
...
\Z{Volumenschwankungen des Brustraums bewirken
...
werden im Kapitel \CO{[VIT]} unter
...
\EE{21\,\%}  & Sauerstoff (\ce{O2}) & \EE{16\,\%}
\\\addlinespace
78\,\% & Stickstoff (\ce{N}) & 78\,\%
\\\addlinespace
\EE{0,03\,\%} & Kohlendioxid (\ce{CO2}) & \EE{4\,\%} 
\\\addlinespace
1\,\% & Edelgase  & 1\,\%
\\\addlinespace\bottomrule
\end{tabular}
...
\\\addlinespace\hhline{---}\addlinespace
\noindent\EE{Speiseröhre} (\T{Ösophagus})
 &
 &
...
\noindent\TabHeading{Mastdarm} (\TabHeading{Rektum})
 &
 &
"`Speicherung"' des ausscheidungsbereiten
Stuhls
\\\addlinespace\hhline{---}\addlinespace
...
dient dem \EE{Transport} über den Mund aufgenommener Nahrung vom Rachen in den
Magen. Diese Fortbewegung funktioniert über eine wellenförmige Kontraktur der
...
Leberzirrhose: Umbau der Leber in Bindegewebe durch z.B. chronische Entzündung
und damit resultierendem Verschluss der Pfortader. Dabei fungieren diese Venen
als Umgehungskreislauf in die Vena cava.} Wenn eine solche Ösophagusvarize
platzt, kann es zu einem großen Blutverlust kommen
...
Magenschleimhaut gebildete saure, aus \EE{Salzsäure} bestehende, \EE{Magensaft}.
Die Magenschleimhaut produziert ausserdem eine \EE{schützende Schleimschicht},
die die Oberfläche vor der starken Säure schützt. 

Nachdem die Wandmuskulatur den Speisebrei mit dem Magensaft \EE{durchmischt} hat,
gibt der \EE{Magenpförtner} \Z{(Pylorus)} den Speisebrei portionsweise an den
...
\Layout{0}{\TxBeschreibung}{%
Der Zwölffingerdarm ist der Beginn des Dünndarmes ab dem Pförtner des Magens
und misst ca. 25\,cm. Sein Verlauf ähnelt einem C. 

Hier \EE{münden 2 Ausführungsgänge} ein, der \EE{Gallengang} und der
\EE{Pankreasgang}. Aus dem Gallengang wird die \EE{Gallenflüssigkeit} aus der
\EE{Leber} abgegeben, welche für die \EE{Fettverdauung} wichtig ist (Emulsion
der Fette); aus dem \EE{Pankreas} wird der \EE{Bauchspeichel}, der Verdauungsenzyme
beinhaltet, zugeführt.

Die Kenntnis des gemeinsamen Ausführungspunktes von Galle und Pankreas ist
wichtig, um pathologische Vorgänge dieser beiden Organe zu verstehen. Ein
Gallenstein beispielsweise, welcher im Ausführungsgang nahe der Mündung in den
Zwölffingergang stecken bleibt, führt durch Rückstau der Pankreasflüssigkeit zu
einer akuten Entzündung der Bauchspeicheldrüse.
}
{%
\begin{itemize}
\item Beginn des Dünndarmes, ca. 25\,cm lang
\item 2 Gänge münden:\begin{itemize}
...
\subsection{Die Leber und die Gallenblase}
\index{Leber}
\index{Gallenblase}
...
Die Leber ist ein solides Organ und befindet sich
im \EE{rechten Oberbauch}. Sie hat einen linken und rechten Lappen. Sier ist die
"`Chemiefabrik"' des Körpers: Sie produziert wichtige Stoffe, entgiftet und
speichert Fette und Zucker. 
}{
...
Die Leber produziert u.\,a. wichtige, vom Körper benötigte, \EE{Eiweißstoffe}
(Proteine). 

\index{Pfortader}
Weiters sorgt sie für den Abbau von Giftstoffen (\EE{Entgiftung}).
Die Leber nimmt dabei durch die Giftstoffe mit der Zeit selbst auch Schaden,
welches zu chronischen Leberschädigungen, wie z.\,B. einer Leberentzündung
(Hepatitis) oder Leberzirrhose (könnte man als fortschreitende "`Vernarbung"'
der Leber bezeichnen), führen kann. Über die \T{Pfortader} (Vena portae,
\ref{topic:pfortader}) erhält die Leber nährstoffreiches Blut von Teilen des 
Darms um es zu entgiften, bevor es in den normalen Blutkreislauf zurückkehrt. 

\index{Gallenflüssigkeit}
Die Leber ist auch eine \EE{Verdauungsdrüse}: Sie produziert
die \T{Gallenflüssigkeit}
(\Speech{"`Galle"'}), welche der  \EE{Fettverdauung} dient.

\index{Gallenblase}
\index{Gallengang}
Von der Leber leiten die Gallengänge die Gallenflüssigkeit in den
Zwölffingerdarm. An den Gallengängen hängt die \T{Gallenblase}, in welcher die
Gallenflüssigkeit \E{gespeichert (!)} - nicht produziert - wird. Dadurch
entsteht dem Körper kein Schaden, wenn man diese beispielsweise wegen
Gallensteinen entfernen muss.
...
Magen. Es hat einen \EE{Ausführungsgang}, welcher
zusammen mit dem Gallengang \EE{in den Zwölffingerdarm}
(Duodenum) mündet. Man kann es grob unterteilen in Kopf und Schwanz - der
Pankreaskopf liegt in der C-Schlaufe des Zwölffingerdarmes, die Spitze des
Pankeas reicht bis zur Milz.

\paragraph{Aufgaben} 

Das Pankreas hat \EE{zwei} wesentliche Funktionen. Einerseits ist es
verantwortlich für die Enzymproduktion für die Verdauung. Diese werden über
den Pankreas-Ausführungsgang in den Zwölffingerdarm abgegeben. Andererseits
produziert das Pankreas zwei wichtige Hormone, Insulin und Glukagon, die
regulierende Stoffe für den Zuckerstoffwechsel des Körpers sind. Insulin sorgt
eine Verringerung des Blutzuckers durch eine Förderung der Zuckerentfernung aus
dem Blut. Glukagon hingegen ist verantwortlich für die Steigerung der Menge an
Zucker im Blut.
}
{
\begin{itemize}
  \item Produktion von Verdauungsenzymen
  \item Produktion von Insulin und Glukagon
\end{itemize}
}
...
\Layout{60}{\TxBeschreibung}
{
Jejunum (Krummdarm) und Ileum (Leerdarm) sind die zwei weiteren Teile des
Dünndarmes. Ihre Gesamtlänge beträgt ca. 3\,m Ihre Hauptaufgabe besteht in der
weiteren Verdauung der Nahrung und der Nährstoffaufnahme. Hierfür dienen
Darmzotten - kleine "`Ausstülpungen der Oberfläche"' - zur
Oberflächenvergrößerung.

Jejunum und Ileum sind ab ihrem
Abgang aus dem Duodenum bis zum Übergang in den Dickdarm relativ frei
beweglich. Die einzige Befestigung dieses Darmabschnittes ist das Mesenterium
(Darmgekröse). Es befindet sich ungefähr auf Höhe des Nabels und enthält die
Blutgefäße, die zum und vom Darm weg führen.

Das besondere am Ileum ist, dass es einzelne Zellbezirke des Abwehrsystems
(Peyersche Plaques) enthält. Sie sorgen dafür, dass Bakterien aus dem Dickdarm
nicht den weiteren Dünndarm bevölkern können. Wenn dies nicht gelingt,
resultiert eine Entzündung des Darmes (Enteritis bis Gastroenteritis) mit den
Symptomen von Durchfall, Übelkeit und Erbrechen.
}
{
\begin{itemize}
    \item ca. 3\,m lang
    ...
    \item Ileum -> Abwehr von Darmbakterien
    aufgehängt}
\end{itemize}
}
...
\\\hhline{~--}
 &
\EE{Aufsteigender} Teil (\T{Colon ascendens})\index{Dickdarm!aufsteigender
Teil} &
\multirow{4}{\columnwidth}{Wasserentzug, Eindickung, \\
Bakterienbesiedlung, \\Speicherung} 
\\\hhline{~-~}
 &
\EE{Querender} Teil (\T{Colon transversum})\index{Dickdarm!querender 
Teil}
 &
\\\hhline{~-~}
 &
\EE{Absteigender} Teil (\T{Colon descendens})\index{Dickdarm!absteigender
Teil}
 &
\\\hhline{~-~}
 &
...
\\\hhline{~-~}
\EE{Mastdarm} (\T{Rektum})\index{Mastdarm}&"`Speicherung"' des
...
\Layout{60}{\TxBeschreibung}{
Der Leerdarm (Ileum) mündet in den Dickdarm. Diese Mündung ist wie eine
\E{T-Kreuzung} gebaut: \E{Kopfwärts} geht es in den
\EE{aufsteigenden Teil} des Dickdarmes, \E{zehenwärts} in
den \EE{Blinddarm}. Wie der Name des Blinddarmes schon sagt
endet er blind, er ist daher eine Sackgasse. Am Blinddarm
angehängt findet man den \EE{Wurmfortsatz}
...
(wegen der vielen Blinddarm-Operationen). Es wird vermutet, dass er eine
immunologische Funktion hat.

Vom Blinddarm geht es weiter in Colon ascendens, transversum und descendens.
Hierbei durchquert der Dickdarm alle Quadranten des Bauches, von rechts unten
über rechts oben nach links oben bis links unten. Nach dem Colon folgt das
Sigmoideum und danach das Rektum, welches den Stuhl über den Anus nach außen
entlässt.

Die Hauptfunktion des Dickdarms ist das Eindicken des Stuhls. Der Großteil der
Wasseraufnahme aus der verdauten Nahrung findet hier statt. Im Dickdarm
befinden sich Darmbakterien (Escherichia coli).
}
{
\begin{itemize}
  \item T-Kreuzung
...
{\TxBeschreibung}
{%
Das Bauchfell (\T{Peritoneum}) ist eine dünne Haut, welche \EE{die gesamte
...
Netz (Omentum majus und minus). Das große Netz bedeckt die Bauchorgane und ist
die erste Struktur, die man bei einer Baucheröffnung sieht. Es ermöglicht die
Abkapselung von Entzündungen, sodass sich diese nicht sofort auf den ganzen
Bauchraum ausbreiten können.
}.
...
glatte Oberfläche dafür, dass die Organe, wie z.\,B. Darm, ihre Bewegungen zur
Verdauungen besser durchführen können. Das Bauchfell ist auch verantwortlich
für die \EE{Schmerzen}, die z.\,B. bei einer Entzündung der Verdauungsorgane
(z.\,B. Blinddarmentzündung (Appendizitis)) entstehen. Es kann sich auch selbst entzünden
...
linken Oberbauch, verdeckt von den Rippen. Sie hat nichts  mit der Verdauung  zu
tun.

Sie dient der immunologischen Überwachung des Blutes und ist daher Teil des
Immunsystems. Die Milz filtert das Blut, zu alte Erythrozyten werden durch
weiße Blutkörperchen in der Milz erkannt und entfernt. Beim Embryo fungiert sie
auch an der Blutbildung beteiligt. Damit sie ihre Aufgaben erfüllen kann, ist
sie \EE{sehr gut durchblutet}! Ihre Vene mündet in die Pfortader.

Sie ist weiters von einer festen \EE{Bindegewebskapsel} umgeben und in das
Bauchfell gehüllt.
}
{
\begin{itemize}
...
\Layout{0}{\TxGefahren{}: \protect\T{Zweizeitiger Milzriss}}
{%
\index{Zweizeitiger Milzriss}%
\index{Milz!-riss, zweizeitiger}%
Aus den Tatsachen, dass die Milz gut durchblutet ist, und über eine feste Kapsel
verfügt, ergibt sich bei Verletzungen eine besondere Gefahr:  

Es kann vorkommen, dass bei einem Unfall die
Milz einreisst, die Kapsel aber erhalten bleibt: Es blutet zuerst in die
Kapsel ein und der Druck steigt, die Kapsel wirkt als eine Art Druckverband.
Nach einer gewissen Zeit reißt jedoch auch die Kapsel, der Patient blutet nun
\E{plötzlich} ungehindert in den Bauchraum. → \Ca{Der Patient wirkt
zuerst "`ganz normal", unerwartet und plötzlich "`verfällt"' er.}
}{%
\begin{itemize}
...
\Layout{23}{\TxBeschreibung}
{
Die Niere ist ein paarig angelegtes Organ und befindet sich links und rechts
neben der Wirbelsäule auf Höhe der unteren Rippenpaare. Durch die Lage der Leber
liegt die rechte Niere etwas weiter fußwärts als die rechte Niere. Die Nieren
besitzen wie die Milz eine Bindegewebskapsel.

Die Niere filtert ausscheidungspflichtige  (z.B. Stickstoff und Abfallstoffe
von Medikamenten) aus dem Blut. Daher ist auch sie äußerst gut durchblutet und
reagiert sehr empfindlich auf einen Blutdruckabfall. Aus diesem Grund kann
auch sie den Blutdruck regulieren.

Weiters ist sie Verantwortlich für die Verhältnisse der
Elektrolytkonzentrationen im Blut und in den Geweben und damit für den
Säure-Basen-Hasuhalt im Körper (pH!!!) sowie für den Wasserhaushalt.

Zusätzlich bildet die Niere Hormone für die Blutdruckregulierung (Renin) und für
die Blutbildung (Erythropoetin).

Mit zunehemendem Alter, insbesondere bei Diabetikern, ist die Niere eines der
Hauptorgane, die "`den Geist aufgeben"'. Dadurch resultiert eine chronische
Niereninsuffizienz, die Patienten müssen regelmäßig zur Dialyse
("`Blutwäsche"'). Außerdem zeigen diese Personen häufig eine Anämie durch den
...
\Layout{20}{\TxBeschreibung}
{
Die Niere hat eine bohnenförmige Form (ca. 11cmx5,5cmx4cm). Wenn man sie längs
durchschneidet kann man ihren Aufbau erkennen. Sie ist unterteilbar in ein
Nierenmark, und eine Nierenrinde. Das Nierenmark ist pyramidenförmig, mit der
Spitze nach innen. In ihm sammeln sich die Nierenkanäle. An der Spitze des Marks
folgt der Übergang in das Nierenbecken und somit der Anschluss an den Harntrakt.
Als Hilus wird der Bereich der Niere bezeichnet, wo die Nierenarterie, Die
Nierenvene und der Harnleiter befestigt sind.
}
{
\begin{itemize}
\item Rinde, Mark
...
\Z{
Die funktionelle Einheit der Niere ist das Nephron. Es gibt davon ca. 1 Mio
in jeder Niere. Im Neprhon kann man unterscheiden:
    \begin{itemize}
...
\Z{
In der Niere werden pro Tag 1500\,l Blut "`bearbeitet"'. Das Blut wird in den
Glomerula zunächst gefiltert. Dadurch entstehen 150 l Primärharn (inkl.
Elektrolyte usw.). Dieser fließt dann weiter durch den Schleifenapparat. Dort
werden für den Körper wichtige Stoffe wieder aufgenommen und Wasser
rückresorbiert, sodass aus 150 l Primärharn schlußendlich 1,5-2 l Endharn pro
Tag den Körper verlassen.
}
...
\Layout{20}{\Tx Beschreibung}
{
\Z{Paariges, jeweils am oberen Nierenpol gelegenes Organ. Sie überdecken die
Niere "`mützenformig"'. Mit der Harnproduktion haben die Nebennieren nichts zu
tun. Die topographische Nähe hat mit einem Hormon für die Blutdruckregulierung
zu tun, mit dem die Nebenniere der Niere dabei hilft.

Man kann die Nebenniere im Querschnitt in Nebennierenrinde und -mark
unterteilen. Es werden jeweils andere Hormone produziert.
...
{
\begin{itemize}
  \item Hauptaufgabe: Hormonproduktion für verschiedenste Körperaufgaben
\end{itemize}
}{}{}
...
Durch die kürzeren Harnwege bei der Frau kann es bei ihr leichter zu
...
Fortpflanzungsorgane wie Eierstöcke, Eileiter und Uterus bzw. Prostata,
Samenbläschen. Diese werden allerdings im Kapitel "`Geschlechtsorgane"'
...
Bei Mann und Frau kann man zwschen äußeren und inneren Geschlechtsorganen
unterscheiden.
...
\Captionof{figure}{\AuthNotesPicLegalOk{}{}Die männlichen Geschlechtsorgane im
Querschnitt. \SourcePicture{Lena Hirtler}}

\subsubsection{Hoden}

\Layout{0}{TxBeschreibung}
{
Im Hoden findet die Spermiogenese - also die Bildung der Spermien statt. Der
Hoden hat eine Pflaumenfrom mit einem ungefähren Längsdurchmesser von 5 cm und
paarig ist angelegt. Er steht über die Nebenhoden in Verbindung mit den
ableitenden Samenwegen.

Da der Hoden während der Schwangerschaftswochen im Ungeborenen von der
Bauchhöhle seinen Weg in den Hodensack finden soll, muss dies nach der Geburt
kontrolliert werden.
}
{
\begin{itemize}
...
\subsubsection{Nebenhoden}

\Layout{0}{TxBeschreibung}
{
Der Nebenhoden liegt dem Hoden kappenförmig an. Er befindet sich daher auch im
Hodensack. Er ist verantwortlich für einige Enzyme die z.B. den Spermien helfen
sollen, die Eizelle zu finden. Weiters verfügt die Wand des Nebenhodens über
eine starke Muskulatur, die mit einer eigenen Peristaltik den Transport der
Spermien bewirkt. Die Spermien bleiben im Nebenhoden ca. 7-14 Tage und erlangen
...
\subsubsection{Samenstrang}

\Layout{0}{TxBeschreibung}
{
Unter Samenstrang versteht man die Struktur, die vom Hoden durch den
Leistenkanal in Richtung Prostata und dann Penis führt.
...
In der Wand des Samenstrangs verläuft der Hodenheber-Muskel, ein Ausläufer der
schrägen Bauchmuskeln. Er ist dafür verantwortlich, den Hoden vor zu großen
Temperaturunterschieden zu schützen (z.B. zieht er den Hoden bei Kälte Richtung
Bauchraum).
}
{
\begin{itemize}
...
\subsubsection{Prostata und Bläschendrüse}

\Layout{0}{TxBeschreibung}
{
Die Prostata ist eine etwa kastatinengroße Drüse. Sie liegt direkt der
Harnblase an und die Harnröhre läuft durch sie hindurch. Die Prostata
liegt außerdem dem Enddarm an - sie ist so tastbar und jeder Mann über
50 sollte sich regelmäßig untersuchen lassen. Mit dem zunehmenden Alter
kann sie nämlich entarten. Postatakrebs ist eine der häufigsten
Tumorarten beim Mann.

Die Prostata ist zuständig für ca. 30\% der Ejakulatflüssikeit und erzeugt
Enzyme, die die Beweglichkeit der Spermien fördern.

Die Bläschendrüse liegt der Prostata ann beiden Seiten an. Sie sorgt für ca.
60\% der Ejakulatflüssigkeit.
}
{
\begin{itemize}
  \item Prostata -> 30\% des Ejakulates, Beweglichkeit der Spermien
  \item Bläschendrüse -> 60\% des Ejakulates
\end{itemize}
}{}{}{}
...
\Layout{0}{TxBeschreibung}
{
Der wichtigste Bestandteil des Penis ist der Schwellkörper. Er besteht aus zwei
Untereinheiten:

\begin{itemize}
  \item Corpus cavernosum: Ist in zwei Teile geteilt und befindet sich an der
  Oberseite. Es sorgt durch Blutaufnahme für das steif-werden des Penis 
  \item Corpus spongiosum: liegt an der Unterseite. In ihm liegt die Harnröhre,
  die in der Eichel endet.Dadurch dass sich dieser Körper nicht so sehr mit Blut
  füllt, wird die Harnröhre nicht abgedrückt und die Ejakulation ermöglicht.
\end{itemize}

Der berührungsempfindlichste Teil des Penis ist die Eichel. Sie ist im
erektionslosen Zustand unter der Vorhaut verborgen, die sich bei Erektion
zurückzieht.

Erektion und Ejakulation erfolgen reflektorisch. Zur Erektion kommt es durch
einer Kombination aus physischen und phsychischen Reizen. Die Ejakulation
erfolgt durch Kontraktionen der Beckenbodenmuskulatur.

Unter einer Potenzstörung versteht man die Unmöglichkeit zur Durchführung
eines Beischlafes. Bei einer Potenzstörung kann entweder die Erektion gestört
sein oder die Ejakulation verhindert sein.
}
{
\begin{itemize}
...
\subsubsection{Hodensack}

\Layout{0}{TxBeschreibung}
{Der Hodensack umschließt Hoden, Nebenhoden und Samenstränge. Er ist
aus einzelnen Schichten aufgebaut. Am wichtigsten ist, dass auch er
Fasern des Hodenheber-Muskels enthält.

Der Hoden kann sich im Hodensack um seine eigene Achse drehen. wenn
dies passiert und er sich nicht zurückdreht, spricht man von einer
Hodentorsion. Es werden die wichtigen Blutgefäße abgeschnürt, es kommt
zum Absterben des Hodens begleitet mit unvorstellbaren Schmerzen und
vegetativen Symptomen wie Übelkeit und Erbrechen. Wenn diese Torsion
nicht schnellstmöglich operiert wird, stirbt dieser Hoden vollständig
ab. Bei der Operation wird der Hoden an den Hodensack festgenäht, damit
dies nicht nocheinmal vorkommen kann.
}
{
\begin{itemize}
  \item Hodenheber-Muskel
...
\includegraphics[width=\linewidth]{./images/anatomie/anatomie-genitale-weiblich-edited.png}
\Captionof{figure}{\AuthNotesPicLegalOk{}{Lena }Weibliche Geschlechtsorgane,
Querschnitt. \SourcePicture{Lena Hirtler}}

\subsubsection{Eierstöcke}

\Layout{0}{TxBeschreibung}
{
In den Eierstöcken (Ovarien) reifen die Eizellen heran. Sie sind paarig
und liegen im Beckenbereich und sind ca. 4x2x1 cm groß. Dadurch dass in den
Ovarien regelmäßig Eizellen heranreifen, kann man in ihnen alle
Entwicklungsstadien finden. Nach dem Eisprung ist das Ovar auch zuständig für
die Bildung von Hormonen, die im Falle einer Befruchtung der Eizelle
...
\subsubsection{Eileiter}

\Layout{0}{TxBeschreibung}
{
Der Eileiter (Tuba uterina / Salpinx) ist ein ca. 10-18cm langer Schlauch mit
einer freien Öffnung in die Bauchhöhle. Nahe dem Ovar zeigt der Eileiter einen
Trichter mit fingerförmigen Ausläufern, worin die Eizelle nach dem Eisprung
"`aufgefangen"' werden soll.

Die Befruchtung der Eizelle findet imm Eileiter statt. Dadurch kann man sich
auch vorstellen, dass es, wenn die Eizelle den "`falschen"' Weg nimmt, oder sich
na der Befruchtung nicht in die Gebärmutter weiterbewegt, es zu einer
Eileiterschwangerschaft kommen kann. Dies kann zu einer Lebensgefährdung der
Mutter führen, wenn durch das Wachstum des Embryos der Eileiter "`gesprengt"'
wird und dabei viel Blut verloren wird.
}
{
\begin{itemize}
  \item 10-18 cm, freie Öffnung in die Bauchhöhle
  \item Trichterförmig
  \item Befruchtung
  \item CAVE: Eileiterschwangerschaft
\end{itemize}
}{}{}{}

\subsubsection{Gebärmutter}

\Layout{0}{TxBeschreibung}
{
Die Gebärmutter (Uterus) kann grob in zwei Teile, dem Körper und dem Hals,
eingeteilt werden. In den Körper münden die beiden Eileiter, die die befruchtete
Eizelle in den Uterus führen sollen.

Der Körper der Gebärmutter ist äußerst muskulös und die Schleimhaut unterliegt
bei der geschlechtsreifen Frau zyklischen Veränderungen (siehe "`der weibliche
Zyklus"'). Bei der Geburt ist der Gebärmutterkörper außerdem für die
Autreibung des Fetus zuständig. Über den Hals und den Muttermund ragt der Uterus
in die Scheide hinein.

Eine häufige Krebsart bei der Frau ist Gebärmutterhalskrebs. Daher sollte sie
regelmäßig beim Gynäkologen einen Abstrich machen lassen um eine Früherkennung
zu ermöglichen.
}
{
\begin{itemize}
...
\subsubsection{Scheide}

\Layout{0}{TxBeschreibung}
{
Auf den Muttermund der Gebärmutter folgt die Scheide. Sie ist erheblich dehnbar
und ihr Epithel unterliegt ebenfalls hormonellen Zyklusschwankungen. Ihre
Dehnbarkeit ermöglicht die Anpassung an die Passage des klindlichen Kopfes
...
\subsubsection{Schamlippen und Klitoris}

\Layout{0}{TxBeschreibung}
{
Bei der Frau gibt es insgesamt vier Schamlippen, jeweils zwei große und zwei
kleine. Die großen Schamlippen liegen außen, die kleinen innen und umrahmen den
Scheideneingang mit den Öffnungen der Scheide und der Harnröhre. 

An den kleinen Schamlippen münden Drüsen, die für den Feuchtigkeitserhalt dieser
Region zuständig sind.

Die Frau besitzt, wie der Mann auch, Schwellkörper. Diese liegen in der Klitoris
und im Bereich der großen Schamlippen. Vergleichsweise mit der Eichel ist die
...
\Layout{0}{\TxBeschreibung}
{
Der weibliche Zyklus dauert 28 Tage, dies entspricht einem Lunarmonat (Zeit von
Vollmond bis Vollmond). Er wird von Menstruation (Monatsblutung) zu Menstruation
gerechnet. Während eines Zyklus wird die Uterusschleimhaut aufgebaut und bei der
Menstruation wieder abgestoßen.

Der Zyklus ist durch Östrogen und Progesteron hormongesteuert. Der Eisprung
findet in der Mitte des Zyklus statt, ca. am 14. Tag. Wenn es zu keiner
Befruchtung der Eizelle kommt, läuft der Zyklus normal weiter, es kommt zur
Menstruation.

Wenn es zu einer Befruchtung der Eizelle kommt, wird der Zyklus hormonell
...
\begin{WidePar}
\noindent\begin{minipage}{\linewidth}
\noindent\includegraphics[width=\linewidth]{./images/hirtler-aass/Zyklus.pdf}

\Captionof{figure}{Der weibliche Zyklus. Oben:
