\chapter{[JUS] Rechtliche Grundlagen}
\label{chp:JUS}
% \AasRsN{RS~III}
...
\begin{anfxwarning}[author=GABS]{Subsection: Layout-Anpassung notwendig.}
Dieser Unterabschnitt entspricht nicht dem Standard-Layout!
\end{anfxwarning}
...
& \multicolumn{5}{c}{\Es{$\Swarrow$ EU-Recht $\Searrow$}}
\\\addlinespace

& \multicolumn{5}{c}{\Es{Struktur der Österreichischen Rechtsordnung}}
\\\addlinespace
...
\Es{Verhältnis zwischen Patient und
...
\begin{itemize}
    \item {\mdseries
    \T{Heilbehandlungsvertrag}: Der Patient selber oder sein
    zuständiger Vertreter	wünscht behandelt zu werden}
       
       \item {\mdseries
       \Es{\T{Geschäftsführung ohne Auftrag} im Notfall}: der
...
Im \Es{Verhältnis zwischen Patient und
...
Das \Es{Verhältnis zwischen Rettungsorganisation
...
Org\-anisation gegen\-über dem Patienten treffen, auch
durch das Personal er\-füllt werden. Gleich\-zeitig
...
{\centering
\Es{{\sffamily\Large Eine Geschichte aus 17 Rechtsfällen}}
\par}
...
\begin{anfxwarning}[author=GABS]{Subsubsection: Layout-Anpassung notwendig.}
Dieser Unterunterabschnitt entspricht nicht dem Standard-Layout!
\end{anfxwarning}
...
tobend aggressiven Wut\-aus\-brüche im Zuge seiner psychiatrischen
...
\Es{Eine Festhaltung \underbar{darf} erfolgen nach
...
\Es{Eine Unterbringung \underbar{muss} erfolgen, nach
folgenden Gesichtspunkten}:
...
\begin{itemize}
    ...
    {\quotedblbase}Einweisung{\textquotedblleft} verfügt. Während
...
\Es{Rechtfertigender Notstand}
&
\Es{Entschuldigender Notstand}
\\\midrule
\begin{itemize}
...
\Es{Notstandssituation}: Gefahr ist:
...
\begin{itemize}
    ...
    Gefahrenverursachers)}
    \item {\mdseries Drohender Nachteil darf geringfügig kleiner als das
...
\begin{itemize}
    ...
    (siehe \See{jus-fall-aufklaerung}).
\end{itemize}

${\rightarrow}$ das Vorliegen einer Heilbehandlung wird als Rechtfertigung für
...
\begin{itemize}
    ...
    \begin{itemize}
        \item a) \Es{Körperverletzung} - {\S}{\S}\,83 ff StGB: Sonderfall der HB-Voraussetzungen: es benötigt nur med. Indikation und lege artis Durchführung; die Aufklärung ist hierfür irrelevant
        
        \item b) \Es{Eigenmächtige  Heilbehandlung } - {\S}\,110 StGB: siehe ad 4.
        
    \end{itemize}
    \item Zivilrecht:
    \begin{itemize}
        \item a) \Es{Schadenersatzansprüche} - {\S}{\S}
        1293 ff ABGB: die Heilbehandlung muss alle drei
        Voraussetzungen erfüllen (auch Aufklärung);
        Umfang des Schadenersatzes siehe \See{jus-fall:fortbildung}.
    \end{itemize}
    
\end{itemize}

\Es{Blutzuckermessung}:
...
Eklampsie führen kann und einen raschen, aber möglichst reizarmen
Transport auf die Gynäkologie benötigt. Da er aber wenig von jungen
...
{\quotedblbase}gefälligst{\textquotedblleft} durchzuführen sind. 
...
%\begin{multicols}{2}
\subparagraph*{\Es{Aufklärungspflicht}:}
...
\Fazit{Seit dem 1. Dezember 2009 wurde das \Es{Recht auf
Aufklärung} durch das Inkrafttreten des Artikel~3 Abs.~2
lit.~a der Grundrechtecharta der \Abk{EU} zu einem
eigenen \Es{Grundrecht}.} 

\subparagraph{\Es{Einsichts- und Urteilsfähigkeit}:}
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item grundsätzlich unmündigen Minderjährigen (0-14 LJ.), wobei es vorrangig auf die Einsichts- und Urteilsfähigkeit ankommt
        
        \item bei mündig Minderjährigen (14.-18 LJ.) vorhanden, solange entsprechende Einsichts- und Urteilsfähigkeit nicht fehlt
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
\subparagraph*{\Es{Umfang der Aufklärung}:}
...
\subparagraph*{\Es{Unterlassen der Aufklärung}:}
...
\subparagraph*{\Es{Besonderheit \T{Revers}}:}\index{Revers}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item {\mdseries
        wenn das Einsatzpersonal eine Maßnahme für notwendig erachtet,
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item nur bei {\quotedblbase}Gefahr in Verzug{\textquotedblleft}\marginlabel{Gefahr in Verzug: \E{unmittelbar} drohende oder
...
Patientenverfügung}\label{topic:patientenverfuegung}
% TODO : Subsubsection: Layout-Anpassung
\begin{anfxwarning}[author=GABS]{Subsubsection: Layout-Anpassung notwendig.}
Dieser Unterunterabschnitt entspricht nicht dem Standard-Layout!
\end{anfxwarning}
...
Wiederbelebungsmaßnahmen gesetzt werden{\textquotedblleft}
...
%\begin{multicols}{2}
\subparagraph*{\Es{Allgemeines zur Patientenverfügung}:}
...
\begin{itemize}
    ...
    \item {\mdseries
    erst von Bedeutung, wenn der Patient seinen Willen nicht mehr ausdrücken
    kann}
    
    \item {\mdseries
    nur Ablehnung einer bestimmten Heilbehandlung kann Inhalt der Verfügung sein}
...
\subparagraph{\Es{zwei Arten der Patientenverfügung}:}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item {\mdseries
        \Es{grundsätzlich}: alle Verfügungen, die nicht
        die obigen Voraussetzungen erfüllen}
        
        \item {\mdseries
        \Es{Umfang der Beachtlichkeit}: bloß relative
        Beachtlichkeit:}
...
\subparagraph{\Es{Absolute Unwirksamkeit/Unbeachtlichkeit bei}:}
...
\subparagraph*{\Es{Suche nach Patientenverfügung} --
\Z{{\S}\,12 PatVG:}}~
...
Endlich am Stützpunkt angekommen, ist Alex auch schon der Erste der
...
glücklich durch die enteral erlangte Sedierung, zum Fahrzeug, mit dem
...
MCI\footnote{\E{MCI}: Myocardinfarkt: Herzinfarkt.} handelt.
Er möchte schleunigst einen NEF nachfordern, doch Alex
meint, dass er keine Lust hätte schon wieder auf
einen {\quotedblbase}Boder\footnote{"Bader": früher
Kundiger der Heilbäder, heute umgangssprl. für
...
\Es{Unterlassene Hilfeleistung} - {\S}\,95 StGB
...
Was sind die \T{Kompetenzen} der einzelnen Sanitäterstufen
(RS/NFS/NKA/NKV/NKI)? Darf man die Kompetenzgrenzen (bei \E{Gefahr in
Verzug}) überschreiten?

\Layout{2}{Rechtslage}{}{}{}{}{}

\Es{Kompetenzstufen} gem. \Z{{\S}{\S}\,9-12} SanG:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item {\mdseries
        Z 2: Übernahme und Übergabe des Patienten im Rahmen
        eines Transportes}
        ...
        \item {\mdseries
        Z 4: Durchführung von lebensrettenden Sofortmaßnahmen}
        
        \item {\mdseries
        Z 5: Durchführung von Sondertransporten}
    ...
    \end{itemize}
    \item \T{NKA} - \Z{{\S}\,11 Abs. 1 Z 1} SanG:
    ...
    Die \Es{Arzneimittelliste 2} wird vom leitenden
    ...
    \item
    \T{NKV} -
    \MarginNo{\includegraphics[width=\linewidth]{./images/venflon_in_situ.jpg}}\Z{{\S}\,11 Abs. 1 Z 2} SanG: zusätzlich zu den oben genannten Kompetenzen darf der NKV jene Medikamenten der Arzneimittelliste 2 verabreichen, die über venösen Zugang verabreicht werden können.
    \item 	\T{NKI} - \Z{{\S}\,12 SanG}:
...
\end{itemize}
{\mdseries
\Es{Überschreiten der Kompetenzstufen}:}
...
zivilrechtlichen SchEA. Selbst bei Wissen über Gefahren, Risiken und
...
anordnet, obwohl sie von Sepp über die gefundene Viagra-Packung informiert wurde.
...
\Es{Ablehnung von rechtswidrigen Anordnungen}:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        verstoßen würde.{\textquotedblleft}
...
\Z{{\S}\,5} \Es{SanG}:
{\quotedblbase}\C{Sanitäter haben [{\dots}] die von
ihnen gesetzten Maßnahmen zu dokumentieren.}{\textquotedblleft}

\Es{Vertragsrechtliche Relevanz}:
...
{\mdseries
\Es{Inhalt der Dokumentation}:}

\begin{itemize}
    \item gesetzte med. Maßnahmen
    (\See{topic:dokumentation-medizinisch})
...
\begin{itemize}
    \item {\mdseries
    \Es{strafrechtliche} Norm: \Z{{\S}~121} StGB}
    
    \item {\mdseries
    \Es{verwaltungsrechtliche} Norm: \Z{{\S}~6}~SanG}
...
\Es{Wann \E{darf} man Geheimnisse weitergeben?}
...
\Es{Wann \E{muss} man Geheimnisse weitergeben?} = \T{Auskunftspflicht} \index{Auskunftspflicht} --
\Z{{\S}\,7} SanG
...
\Es{Wem gegenüber muss man Auskunft erteilen?}
...
Fortbildung über die {\quotedblbase}Neuen
...
\subparagraph*{\Es{Fortbildungspflicht nach} \Z{{\S}~50} \Es{SanG}:}
...
\subparagraph*{\Es{Fortbildungspflicht nach} \Z{{\S}\,83}
\Es{MPG}:}
...
die Standardausbildung zum Rettungs\-sani\-täter abgedeckt ist. Für
die anderen, insbesondere höhere technische Produkte, muss jede
Org\-anisation separate Fortbildungen anbieten.

\subparagraph*{\Es{Rezertifizierungspflicht} \Es{gem}.\Z{{\S}~51} \Es{SanG}:}
...
\paragraph{Rechtsfrage - Schadenersatz wegen Körperverletzung}
...
\Es{Rechtliche Grundlage eines
Schadenersatzanspruches} (= SchEA):

\begin{itemize}
    
    \item {\mdseries
    allgemein: \Z{{\S}{\S}\,1293 ff ABGB}}
    
    \item {\mdseries
    konkret für Körperverletzungen: \Z{{\S}{\S}
    1325-1327 ABGB}}
    
\end{itemize}

\Es{Voraussetzungen der Schadenersatzpflicht}:

\begin{itemize}
    
    \item[] {\mdseries
    1. Schaden = Nachteil am Körper, an der Gesundheit, im Vermögen}
    ...
    {\mdseries
    4. Schuld = subjektiver Verstoß gegen jene Sorgfaltspflichten, die
    man einhalten hätte können}
    
\end{itemize}

\Es{Umfang des Schadenersatzes bei
Körperverletzungen}:
...
\begin{itemize}
    ...
    \item {\mdseries
    konkret für Sachschäden: \Z{{\S}\,1331 ABGB}}
...
\begin{itemize}
    \item[] {\mdseries
    gleiche Voraussetzungen wie bei Körperverletzungen}
\end{itemize}

\Es{Umfang des Schadenersatzes bei Sachbeschädigung}:
...
welches durch den großen Morgenstau gute 15 Min. entfernt
...
\begin{itemize}
    \item {\mdseries
    \Es{Behandlungspflicht für Krankenanstalten} (=
    KA) - \Z{{\S}{\S}\,22 und 40 KAKuG}:}
    ...
    \item {\mdseries
    \Es{Ärztliche Behandlungspflicht} --	\Z{{\S}\,48 ÄrzteG}:}
       ...
       \begin{itemize}
           ...
           zukommen zu lassen und wenn nötig weitere Einsatzkräfte nachzufordern oder den Patient in
           weitere medizinische Behandlung zu schicken.}
           
       \end{itemize}
       
       \item {\mdseries
       \Es{Behandlungspflicht für Sanitäter} -
       \Z{{\S}\,4 i.\,V.\,m. {\S}\,8 i.\,V.\,m. {\S}\,9 Abs. 1 Z 1 SanG}:}
       ...
       \item {\mdseries
       \Es{ersthelferische Versorgungspflicht für
       Durchschnittsbürger} - \Z{{\S}\,95 StGB}:}
       ...
       \item {\mdseries
       \Es{Unzumutbarkeit der Behandlung} -
       \Z{{\S}\,95 Abs. 2 StGB}:}
       ...
       \begin{itemize}
           \item {\mdseries
           man das eigene Leben oder die Gesundheit gefährden würde oder}
           
           \item {\mdseries
           höherwertige Interessen verletzen müsste.}
...
hat{\textquotedblleft}, da sie ihr Freund mit \index{HIV}\E{HIV} angesteckt hat.
...
{\mdseries
\Es{Meldepflichtige Krankheiten}:}
...
\Es{Wer ist zur Meldung verpflichtet?}
...
\begin{itemize}
    \item {\mdseries
    ärztliche Anzeigepflicht - \Z{{\S}\,54 Abs. 4 und 5
    ÄrzteG}:}
    ...
    \item {\mdseries
    polizeiliche Ermittlungspflicht - \Z{{\S}\,99 StPO}}
...
{\mdseries
\Es{Anzeigerecht} - \Z{{\S}\,80 Abs. 1 StPO}:}
...
entstammenden Pflicht für den Sanitäter {\quotedblbase}das Wohl der
Patienten [{\dots}] zu wahren{\textquotedblleft} begründen.
...
\emph{"`Geh, schalt doch das Hörndl und die Blitzer ei,
...
\Es{Rechtsquellen}: \Z{{\S}\,26}~StVO und \Z{{\S}\,107}~KFG
...
\Z{{\S}\,1} des {\quotedblbase}Bundesgesetzes gegen
den Missbrauch von Notzeichen{\textquotedblleft}.}

\subparagraph*{\Es{Gebot bei Blaulichtfahrten}:}
\Fazit{
Nur wer noch größere Sorgfalt und Vorsicht wahrt als bei normaler Fahrt, verhält sich richtig!

\Es{Das beinhaltet mitunter eine \underline{geringere} Fahrgeschwindigkeit als
die höchst zulässige!} }

\subparagraph*{Exkurs: \Es{Anschnallpflicht} --- \Z{{\S}\,106}~KFG:}
...
Rettungstransport besteht zusätzlich gem. \Z{{\S}\,106 Abs.\,3~Z\,3} KFG die
