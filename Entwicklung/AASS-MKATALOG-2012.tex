...
\input{./inputs/input-preamble-MKATALOG}

% Trennungen
\input{./inputs/input-hyphenation}

% Titel
\input{./inputs/input-title-MKATALOG}
...
Aller Verweise, welche sich auf die {\AASS} beziehen,
beziehen sich auf diese Version, und können somit von einer
...
%     besteht und eine gesundheitliche Gefährdung des Patienten ausgeschlossen werden kann.
    \Ca{Rechtliche Hinweise beachten (\prettyref{AASS-topic:revers})!}
...
\noindent\begin{minipage}[t]{\linewidth}
{\vspace{-\baselineskip}\scriptsize
% \begin{tabulary}{\linewidth}[H]{cp{8em}p{8.5cm}}
\begin{tabularx}{\linewidth}[H]{p{1cm}XX}
% \toprule\addlinespace
&
...
\begin{itemize}
  \item GAS-Maßnahmen (\prettyref{AASS-topic:gefahrengutunfall})
...
%   (\prettyref{AASS-topic:atemnot-zeichen-kinder})
...
(\FullPrettyRef{m:sauerstoffberieselung}): je nach Indikation,
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    (Beachte: Stabile Seitenlage hat ggfs. bei ungesichertem Atemweg Vorrang)
  \end{itemize}
...
\MassnahmenNG{Spezielle Maßnahmen}{Tachykarde
Attacke}{\label{m:tachykarde-attacke}}{
\begin{itemize}
    \item Lagerung: \EE{Oberkörper hoch}
  ...
  (Extremitätenableitung, \prettyref{AASS-topic:ekg}) abgeleitet werden. Der
...
\begin{itemize}
  ...
  \enquote{\Abk{COPD}} (\prettyref{AASS-topic:copd}). Bei sorgfältiger
...
\begin{itemize}
  ...
  Notfallsanitäter}
  \item Transportentscheidung: Intensivüberwachung
\end{itemize}
}
...
Patient}{\label{m:zerebraler-krampfanfall-bestendend}}{
\Ca{Vitale Bedrohung!}
...
Patient}{\label{m:zerebraler-krampfanfall-status-post}}{Neuerlicher Krampfanfall möglich!
...
Es
  ...
  steriler physiologischer
  Kochsalzlösung (\ce{NaCl}
  0,9\PC*) von innen
  nach außen. Wunden, die im Spital weiter versorgt werden, sollen
  nicht übermäßig gereinigt werden, auch soll keine
...
\opt{STATUS-WITH-BIBLIO}{{\loadgeometry{FullPage}\twocolumn\tiny\setlength{\parskip}{0pt}\cleardoublepage\bibliography{Literatur-AASS}\restoregeometry}\onecolumn}
