\chapter{Internes}
\label{chp:INT}

\section{Ausbilungsverordnung}

% Ausbildungsziele für österreichische Rettungssanitäter gem. der \emph{Verordnung
% der Bundesministerin für Gesundheit und Frauen über die Ausbildung zum
% Sanitäter - Sanitäter-Ausbildungsverordnung - San-AV StF: BGBl. II Nr. 420/2003
% -- Anlage 1, Modul I}.

% % \clearpage
% %%%%%%%%%%%%%%%
% \tablefirsthead{%
% \toprule
% \multicolumn{1}{c}{\bfseries Inhalt} &
% 
% \multicolumn{1}{c}{\bfseries Referenz} 
% \\
% \midrule}
% %%%%%%%%%%%%%%%
% \tablehead{%
% \toprule
% \multicolumn{2}{l}{\small\itshape Fortsetzung.}
% \\
% \midrule
% \multicolumn{1}{c}{\bfseries Inhalt} &
% \multicolumn{1}{c}{\bfseries Referenz} 
% \\
% \midrule}
% %%%%%%%%%%%%%%%
% \tabletail{%
% \hline
% \multicolumn{2}{r}{\small\itshape weiter \ldots}
% \\
% \bottomrule}
% %%%%%%%%%%%%%%%
% \tablelasttail{\bottomrule}
% \bottomcaption{This table is split across pages}
% 
% \begin{supertabular}{p{7.5cm}p{7.5cm}}
% 
% \TabHeading{Erste Hilfe und erweiterte Erste Hilfe}  
% 
% (Bestandteil des Sachgebiets 'Sanitätshilfe') 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:] 14
% \item[Lehrkraft:] Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
% Facharzt, Lehrsanitäter, fachkompetente Person  
% \end{inparadesc}
% \\
% Notwendigkeit, Verpflichtung der Hilfeleistung
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Unfallverhütung          
% & 
% 
% \\
% Rettungskette           
% & 
% \noindent\FullRef{topic:rettungskette}
% \\
% Gefahrenzonen            
% & 
% \noindent\FullRef{topic:gefahrenzone}
% \\
% Bergen (Wegziehen, Rautekgriff aus dem  Auto, Sturzhelm)         
% & 
% \noindent\FullRef{topic:wegziehen}\FullRef{topic:rautekgriff}\FullRef{topic:sturzhelmabnahme}
% \\
% Notfallpatient, Notfalldiagnose, Notfallhilfe
% & 
% \noindent\FullRef{topic:notfallpatient}
% 
% \FullRef{topic:notfalldiagnose}
% 
% \FullRef{topic:notfallhilfe}
% \\
% Kontrolle der Lebensfunktionen
% & 
% \noindent\FullRef{topic:lebensfunktionen-kontrolle}
% \\
% Notfalldiagnose Bewusstlosigkeit
% & 
% \FullRef{topic:notfalldiagnose-bewusstlosigkeit}
% \\
% Notfalldiagnose  Atem-Kreislaufstillstand
% & 
% \noindent\FullRef{topic:notfalldiagnose-atem-kreislaufstillstand}
% \\
% Starke Blutung (Blutstillung, Fingerdruck,  Druckverband, Abbindung)
% & 
% \noindent\FullRef{topic:}
% \\
% Schock (Schockbekämpfung,  Lagerungen)
% & 
% 
% \\
% Wunden und Wundverbände (Tierbisse, Insektenstiche, Verätzungen,
% Verbrennungen, Erfrierungen, Unterkühlung) & 
% 
% \\
% Quetschungen
% & 
% 
% \\
% Gelenksverletzungen
% & 
% Verstauchung: \FullRef{topic:verstauchung-eh}, 
% 
% Verrenkung: \FullRef{topic:verrenkung-eh}
% \\
% Knochenbrüche, allgemein
% & 
% \noindent\FullRef{topic:knochenbruch-eh}
% \\
% Brustkorbverletzungen
% & 
% 
% \\
% Plötzlich auftretende Erkrankungen
% & 
% 
% \\
% Vergiftungen
% & 
% 
% \\
% Transport
% & 
% 
% \\\toprule
% \TabHeading{Hygiene}  
% 
% (Bestandteil des Sachgebiets 'Sanitätshilfe') 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:] 2
% \item[Lehrkraft:] Arzt für Allgemeinmedizin, approbierter Arzt,
% Facharzt, Angehörige des gehobenen Dienstes für Gesundheits- und Krankenpflege,
% Hygienefachkraft,  Lehrsanitäter
% \end{inparadesc}
% 
% \\\midrule
% Persönliche Hygiene      
% & 
% \FullRef{topic:persoenliche-hygiene}
% \\
% Grundbegriffe der Desinfektion             
% & 
% \FullRef{topic:desinfektion-grundzuege}
% \\
% Grundbegriffe der  Sterilisation         
% & 
% \FullRef{topic:sterilisation-grundzuege}
% \\
% Entsorgung von infektiösem Abfall       
% & 
% \FullRef{topic:entsorgung-infektioeser-abfall}
% \\
% Allgemeine Infektionslehre          
% & 
% Erreger: \FullRef{topic:infektion-erreger}, 
% 
% Übertragungswege: \FullRef{topic:infektion-uebertragungswege}, 
% 
% Kreuzinfektion: \FullRef{topic:kreuzinfektion}.
% \\
% Vorgehen bei Verletzungen des Personals                
% & 
% Nadelstichverletzungen: \FullRef{topic:nadelstichverletzung}
% \\
% Hygienemaßnahmenplan     
% & 
% \FullRef{topic:hygieneplan}
% \\
% Infektionstransport      
% & 
% \Abk{MRSA} und andere resistente Keime: \prettyref{m:mrsa-transport} 
% \\\toprule
% \TabHeading{Berufsspezifische rechtliche Grundlagen}  ~
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:] 3
% \item[Lehrkraft:]  Jurist, fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% Aufgaben und Kompetenzen des  Rettungssanitäters       
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Dokumentation im Rettungswesen (Einsatzprotokoll, Leitstellendokumentation, Transportnachweis)
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Hilfs- und Rettungswesen
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Straßenverkehrsordnung
% & 
% \noindent\FullRef{topic:strassenverkehrsordnung}
% \\
% Patientenrechte
% & 
% \noindent\FullRef{topic:patientenrechte}
% \\
% Grundlagen des Haftungsrechtes
% & 
% \noindent\FullRef{topic:haftungsrecht}
% \\
% Unterbringungsgesetz
% & 
% \noindent\FullRef{topic:unterbringungsgesetz}
% \\
% Reversfähigkeiten und Effekten
% & 
% \noindent\FullRef{topic:reversfähigkeit}\FullRef{topic:effekte}
% \\
% Mitnahme von Begleitpersonen
% & 
% \noindent\FullRef{topic:begleitpersonen-mitnahme}
% \\\toprule
% \TabHeading{Anatomie und Physiologie}  
% 
% (Bestandteil des Sachgebiets 'Sanitätshilfe')  
% 
% \begin{inparadesc}
% \item[Stundenanzahl:] 4
% \item[Lehrkraft:]  Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
% Facharzt, Turnusarzt,   Angehörige  des  gehobenen  Dienstes für Gesundheits- 
% und Krankenpflege, Lehrsanitäter, fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% Blutkreislauf - Grundzüge               
% & 
% \FullRef{topic:blutkreislauf}
% \\
% Gliedmaßen - Grundzüge  
% & 
% Obere Extremität: 
% \FullRef{topic:schulterguertel}, 
% \FullRef{topic:obere-extremitaet},
% 
% untere Extremität: 
% \FullRef{topic:beckenguertel}, 
% \FullRef{topic:untere-extremitaet}
% \\
% Haut -- Grundzüge        
% & 
% \FullRef{topic:haut}
% \\
% Schädel und Rumpf - Grundzüge Skelett       
% & 
% Schädel: \FullRef{topic:schaedel}
% \\
% Brustkorb - Grundzüge   
% & 
% \FullRef{topic:thorax}
% \\
% Bauchraum - Grundzüge   
% & 
% Gastro-intestinaltrakt: \FullRef{topic:gastrointestinaltrakt}, andere Bauchorgane:
% \FullRef{topic:sonstige-bauchorgane}. 
% \\\toprule
% \TabHeading{Störungen der  Vitalfunktionen  und Regelkreise und zu setzende Maßnahmen}  
% 
% (Bestandteil des  Sachgebiets  'Sanitätshilfe') 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  8 
% \item[Lehrkraft:]  Notarzt, Arzt für Allgemein- medizin, approbierter Arzt,
% Facharzt,  Lehr- sanitäter,  fachkompe- tente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% Definition Vitalfunktion           
% & 
% \noindent\FullRef{topic:vitalfunktionen-definition}
% \\
% NACA-Schema             
% & 
% \FullRef{topic:naca}
% \\
% Bewusstsein             
% 
% (Bewusstseinsstörungen, Bewusstlosigkeit)   
% & 
% \noindent Bewusstsein
% allgemein: \FullRef{topic:bewusstsein};
% Bewusstseinsstörungen:
% \FullRef{topic:bewusstseinsstoerung}, 
% 
% Bewusstlosigkeit: \FullRef{topic:bewusstlosigkeit}
% \\
% Atmung 
% & 
% --- ↓ --- 
% \\          
% (Atemstörungen,  
% & 
% \noindent\FullRef{topic:atemstörungen}
% \\
% Atemstillstand)        
% & 
% \noindent\FullRef{topic:atemstillstand}
% \\
% - Kreislauf               
% & 
% --- ↓ --- 
% \\
% (Kreislaufstörungen,    
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Kreislaufstillstand)    
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Regelkreise}
% & 
% --- ↓ --- 
% \\
% \TabItemII{Wärmehaushalt} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemII{Wasser- und Elektrolythaushalt} 
% & 
% \FullRef{topic:wasser-elektrolythaushalt}
% \\
% \TabItemII{Säure-Basen-Haushalt} 
% & 
% \FullRef{topic:saeure-basen-haushalt}
% \\
% \TabItemII{Stoffwechsel} 
% & 
% \FullRef{topic:stoffwechsel}
% \\
% Starke Blutung
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Schock (Ursachen,
% & 
% --- ↓ --- \FullRef{topic:schock}
% \\
% Wirkung, Schockformen,
% & 
% \FullRef{topic:schock-arten}
% \\
% Verlauf, Schockzeichen)
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabSub{Akute Störung der Atmung} 
% & 
% --- ↓ --- 
% \\
% \TabItemI{Atembehinderung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Verlegung der Atemwege  durch Fremdkörper}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Verlegung durch Schwellung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Akute Atemnot}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Absaugung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Sauerstoff (Inhalationsrichtlinien, Dosierungsvorschriften)}
% & 
% \FullRef{topic:sauerstoff}
% \\
% \TabItemI{Assistierte Beatmung (Indikationen, Durchführung)}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Feststellung des Todes}
% & 
% \FullRef{topic:tod}
% \\\toprule
% \TabHeading{Notfälle bei verschiedenen Krankheitsbildern  und zu setzende Maßnahmen}  
% 
% (Bestandteil des Sachgebiets 'Sanitätshilfe') 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  6
% \item[Lehrkraft:]  Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
% Facharzt, Turnusarzt, Lehrsanitäter, fachkompetente Person
% \end{inparadesc}
% \\\midrule
% - Koma aus vorerst unbekannter Ursache      
% & 
% --- ↓ --- 
% \\
% (Schlaganfall,           
% & 
% \FullRef{topic:insult}
% \\
% Meningitis, 
% & 
% \FullRef{topic:meningitis}
% \\
% Diabetes,   
% & 
% \FullRef{topic:diabetes-mellitus}
% \\
% Vergiftung)             
% & 
% \#\#\# %\FullRef{topic:}
% \\
% - Krampfanfall             
% & 
% --- ↓ --- 
% \\
% (Epilepsie, Tetanie)    
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabSub{Akuter Gefäßverschluss an den Gliedmaßen}     
% & 
% --- ↓ --- 
% \\
% Venenthrombose,       
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Arterielle Embolie)     
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabSub{Pulmonale Notfälle}
% & 
% --- ↓ ---
% \\
% \TabItemI{Asthma bronchiale}
% & 
% \FullRef{topic:asthma-bronchiale}
% \\
% \TabItemI{Lungenödem} 
% & 
% \FullRef{topic:lungenoedem}
% \\
% \TabItemI{Lungenembolie} 
% & 
% \FullRef{topic:lungenembolie}
% \\
% \TabItemI{Lungenentzündung}
% & 
% \FullRef{topic:lungenentzuendung}
% \\
% \TabSub{Cardiale Notfälle}
% & 
% --- ↓ --- 
% \\
% (Herz-Rhythmus-Störungen, 
% & 
% \FullRef{topic:herzrhythmusstoerungen}
% \\
% Herzversagen, 
% Linksherzschwäche,
% Rechtsherzschwäche, 
% & 
% \FullRef{topic:herzinsuffizienz}, 
% \FullRef{topic:linksherzinsuffizienz},
% \FullRef{topic:rechtsherzinsuffizienz}
% \\
% Angina pectoris, 
% & 
% \FullRef{topic:angina-pectoris}
% \\
% Herzinfarkt, 
% & 
% \FullRef{topic:herzinfarkt}
% \\
% Hochdruckkrise) 
% & 
% \FullRef{topic:hypertensive-kriese}, \FullRef{topic:hypertensiver-notfall}
% \\
% \TabSub{Allgemeinchirurgische Notfälle}
% & 
% --- ↓ --- 
% \\
% Akutes Abdomen 
% & 
% \FullRef{topic:akutes-abdomen}
% \\
% Gastritis, 
% & 
% \FullRef{topic:gastritis}
% \\
% Ulcus, 
% & 
% 
% \\
% Gastroenteritis,
% & 
% \FullRef{topic:gastroenteritis}
% \\
% Pankreatitis,
% & 
% \FullRef{topic:pankreatitis}
% \\
% Appendizitis,
% & 
% \FullRef{topic:appendizitis}
% \\
% Gallenkolik, 
% & 
% \FullRef{topic:gallenkolik}
% \\
% Ileus,
% & 
% \FullRef{topic:ileus}
% \\
% Mesenterialinfarkt,
% & 
% \FullRef{topic:mesenterialinfarkt}
% \\
% Lebensmittelvergiftung,
% & 
% 
% \\
% gastrointestinale Blutung)
% & 
% \FullRef{topic:gastrointestinale-blutung}
% \\
% \TabSub{Gynäkologische und urologische Erkrankungen}
% & 
% --- ↓ --- 
% \\
% Nierenkolik,
% & 
% \FullRef{topic:nierenkolik}
% \\
% Nierenbeckenentzündung,
% & 
% \FullRef{topic:nierenbeckenentzuendung}
% \\
% Harnwegsinfekt, 
% & 
% \FullRef{topic:harnwegsinfekt}
% \\
% akutes Harnverhalten,
% & 
% \FullRef{topic:harnverhalten}
% \\
% chronische Niereninsuffienz und Hämodialyse,
% & 
% Niereninsuffizienz: \FullRef{topic:niereninsuffizenz}; 
% 
% Elektrolytstörungen durch
% Dialyse: \FullRef{topic:dialyse-elektrolyte}; 
% 
% Dialyse als Therapie bei chronischer
% Niereninsuffizenz: \FullRef{topic:cni-therapie-dialyse}; 
% 
% Dialysepatienten, besondere
% Gefahren: \FullRef{topic:dialysepatienten-gefahren}.
% \\
% extrauterine Gravidität, 
% & 
% \FullRef{topic:eileiterschwangerschaft}
% \\
% Abortus,
% & 
% \FullRef{topic:abort}
% \\
% Unterleibsblutungen,
% & 
% 
% \\
% Ovarialtumore,
% & 
% 
% \\
% Vergewaltigung (typische Verletzungen, psychische Betreuung)
% & 
% \FullRef{topic:vergewaltigung}
% \\\toprule
% \TabHeading{Spezielle Notfälle und zu setzende Maßnahmen  }  
% 
% (Bestandteil des Sachgebiets  'Sanitätshilfe')  
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  15  
% \item[Lehrkraft:]   Notarzt, Arzt für  Allgemein-  medizin,  approbierter 
% Arzt, Facharzt,   Turnusarzt,  Lehr-  sanitäter,  fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% \TabSub{Traumatologische Notfälle}
% & 
% --- ↓ --- 
% \\
% Schädel-Hirn- Verletzungen,          
% & 
% \FullRef{topic:sht}
% \\
% Hirnblutung 
% & 
% \FullRef{topic:hirnblutung}
% \\
% Hirndruck
% & 
% Hirndruckzeichen: \FullRef{topic:hirndruckzeichen}; 
% 
% Hirnstammeinklemmung bei
% Hirndrucksteigerung: \FullRef{topic:hirndrucksteigerung-hirnstammeinklemmung}; 
% 
% Ursachen:
% \FullRef{topic:hirndrucksteigerung-ursachen}; 
% 
% Lagerung: \FullRef{topic:hirndrucksteigerung-lagerung}.
% \\
% - Halswirbelsäulen- und  Wirbelsäulentrauma     
% & 
% --- ↓ --- 
% \\
% (Sturzhelmabnahme,      
% & 
% 
% \\
% HWS-Scheine Beschreibung (MP): \FullRef{topic:hws-schiene-beschreibung};
% 
% Durchführung: \FullRef{topic:hws-schiene-anwendung} 
% & 
% 
% \\
% Motorik-Durchblutung-Sensibilitätskontrolle,         
% & 
% 
% \\
% Body-Check, 
% 
% Umgang mit  Schaufeltrage und Vakuummatratze,
% 
% Sandwich-Technik)
% 
% - Extremitätentrauma
% 
% --- ↓ --- 
% 
% (Verletzungsarten,
% 
% Motorik-Durchblutung-Sensibilitätskontrolle,
% 
% Stiefelgriff, Prinzip der Schienung,
% 
% Ruhigstellung,
% \\
% \TabItemI{Schienung des Armes}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Schienung des Beines}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Pneumatische Schiene}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Vakuumschiene}
% & 
% Vakuumbeinschiene, Beschreibung: \FullRef{topic:}, Anwendung: \FullRef{topic:}
% \\
% \TabItemI{Extensionsschiene}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Thoraxtrauma 
% & 
% --- ↓ ---
% \\
% \TabItemI{offene, geschlossene Brustkorbverletzung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Serienrippenbruch}
% & 
% \FullRef{topic:serienrippenfraktur}
% \\
% \TabItemI{Pneumothorax}
% & 
% \FullRef{topic:pneumothorax}
% \\
% \TabItemI{Spannungspneumothorax}
% & 
% \FullRef{topic:pneumothorax}
% \\
% Bauchtrauma 
% & 
% ↓
% \\
% \TabItemI{offene, geschlossene Bauchverletzung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Verletzung der Harn und Geschlechtsorgane}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Beckentrauma}
% & 
% \FullRef{topic:beckentrauma}
% \\
% \TabItemI{Polytrauma (Definition, Prioritäten, Management)}
% & 
% \FullRef{topic:polytrauma}
% \\
% \TabItemI{Wunden (mechanische, chemische, thermische)}
% & 
% \FullRef{topic:wunden}
% \\
% \TabItemI{Dekubitus Prophylaxe und Lagerung bei Dekubitus}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Verbandlehre}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Akut auftretende Blutungen}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Vergiftung (Ursachen und Verdacht, Aufnahmearten)}
% & 
% \FullRef{topic:}
% \\
% \TabSub{Psychiatrische Notfälle}
% & 
% --- ↓ --- 
% \\
% (Suizid, 
% & 
% \FullRef{topic:suizid}
% \\
% \TabItemI{Psychose}
% & 
% \FullRef{topic:psychose}
% \\
% \TabItemI{Suchterkrankungen und Entzugssyndrom}
% & 
% \FullRef{topic:sucht}, \FullRef{topic:entzugssysndrom}
% \\
% \TabItemI{Depression}
% & 
% \FullRef{topic:depression}
% \\
% \TabItemI{Manie}
% & 
% \FullRef{topic:manie}
% \\
% Schwangerschaft und Geburt
% & 
% ↓
% \\
% \TabItemI{Notfälle in der Schwangerschaft}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Geburt}
% & 
% \FullRef{topic:geburt}
% \\
% \TabItemI{Geburtskomplikationen}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Versorgung des Neugeborenen}
% 
% - Notfälle im Säuglings- und Kleinkindalter
% & 
% --- ↓ --- 
% \\
% \TabItemI{anatomische und physiologische Besonderheiten}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Kontrolle der Lebensfunktionen und lebensrettende Sofortmaßnahmen}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{Krampfanfälle}
% & 
% Allgemein: \FullRef{topic:krampfanfaelle}, Fieberkrampf: \label{topic:fieberkrampf}
% \\
% \TabItemI{Pseudokrupp}
% & 
% \FullRef{topic:laryngitis}
% \\
% \TabItemI{Epiglottitis}
% & 
% \FullRef{topic:epiglottitis}
% \\
% \TabItemI{Keuchhusten}
% & 
% \FullRef{topic:}
% \\
% \TabItemI{plötzlicher Kindstod}
% & 
% \FullRef{topic:sids}
%  \\\toprule
% \TabHeading{Defibrillation mit halbautomatischen Geräten}  
% 
% (Bestandteil des Sachgebiets  'Sanitätshilfe')  
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]   8
% \item[Lehrkraft:]   Notarzt,  Arzt für Allgemeinmedizin,  approbierter Arzt,
% Facharzt, Lehrsanitäter, fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% - Der halbautomatische Defibrillator          
% 
% - Handhabung eines halbautomatischen Defibrillators         
% 
% - Gerätemanagement während der Reanimation 
% 
% - Erfolgskontrolle
%  \\\toprule
% \TabHeading{Gerätelehre und Sanitätstechnik }  
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  12 
% \item[Lehrkraft:]   Lehrsanitäter, fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% \TabItemI{Medizinproduktegesetz - Information}
% & 
% \FullRef{topic:medizinproduktegesetz}
% \\
% Bergungs- und Lagerungstechniken     
% & 
% ↓
% \\
% \TabItemI{Bergetuch}
% & 
% \FullRef{topic:bergetuch}
% \\
% \TabItemI{Einheitskrankentrage}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Tragsessel} 
% & 
% \FullRef{topic:tragsessel}
% \\
% \TabItemI{Fahrtrage}
% & 
% \FullRef{topic:fahrtrage}
% \\
% \TabItemI{Rollstuhl}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Einsatzfahrzeug}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Beatmungsbeutel}
% & 
% \FullRef{topic:beatmungsbeutel}
% \\
% \TabItemI{Absauggeräte}
% & 
% \FullRef{topic:absaugung}
% \\
% \TabItemI{Sauerstoff}
% & 
% \FullRef{topic:sauerstoff}
% \\
% \TabItemI{Infusionen und}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Infusionsgeräte}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Blutdruckmessung}
% & 
% \FullRef{topic:rr-messung}
% \\
% Stabilisierungs- und Schienungstechniken
% & 
% ↓
% \\
% \TabItemI{Stabilisierung der Halswirbelsäule}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Schaufeltrage}
% & 
% Allg.: \FullRef{topic:schaufeltrage}, Anwendung:
% \FullRef{topic:schaufeltrage-anwendung}
% \\
% \TabItemI{Vakuummatratze}
% & 
% Allg.: \FullRef{topic:vakuummatratze}, Anwendung:
% \FullRef{topic:vakuummatratze-anwendung}
% \\
% \TabItemI{Geburtenkoffer}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Transportinkubator}
% & 
% \#\#\# %\FullRef{topic:}
% \\
%  \\\toprule
% \TabHeading{Rettungswesen}  
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  4  
% \item[Lehrkraft:]   Fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% \TabItemI{Rechtliche Grundlagen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Zusammenarbeit mit  anderen Organisationen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Einsatzarten}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Transport- und Fahrzeugarten (Land,  Wasser, Luft)}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Normen, persönliche Schutzausrüstung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Fahrzeugausstattung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Rettungskette}
% &
% \#\#\#
% \\
% \TabItemI{Hilfsfrist}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Dienststellennetz}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Personal im Rettungsdienst}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Notarztsysteme}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Leitstelle}
% &
% \#\#\#
% \\
% \TabItemI{Kommunikationsarten}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Gefahren an der Einsatzstelle}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Gefahrguteinsätze}
% &
% \#\#\#
% \\
% \TabItemI{Sondertransporte}
% &
% \#\#\#
% 
%  \\\toprule
% \TabHeading{Katastrophen, Großschadensereignisse, Gefahrgutunfälle}  
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  4  
% \item[Lehrkraft:]  Fachkompe- tente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% Katastrophen  
% & 
% ↓
% \\
% \TabItemI{Rechtliche Grundlagen,  Geltungsbereiche}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Arten der Katastrophen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Phasen der Katastrophenbewältigung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Katastrophenhilfseinheiten}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Führungsorganisation}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{personelle, materielle und finanzielle Vorsorge}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Einsatzgrundsätze}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{generelle Einsatzrichtlinien}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Grundzüge der Triage}
% & 
% \FullRef{topic:triage}
% \\
% Großschadensereignisse
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Rechtliche Grundlagen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Einstufung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Alarmierung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Schadensraum}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Schadensplatz, Sicherheitseinrichtungen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Organisation beim Großunfall} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Grundzüge der Triage}
% & 
% \FullRef{topic:triage}
% \\
% \TabItemI{österreichisches Patientenleitsystem}
% & 
% \FullRef{topic:pls}
% \\
% \TabItemI{Material und Ausrüstung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Kommunikation}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Gefahrgutunfälle 
% & 
% ---
% \\
% \TabItemI{Arten von Gefahrgutunfällen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Gefahrzettel}
% & 
% \FullRef{topic:gefahrzettel}
% \\
% \TabItemI{Gefahrsymbole}
% & 
% \FullRef{topic:gefahrsymbole}
% \\
% \TabItemI{Warntafel}
% & 
% \FullRef{topic:warntafel}
% \\
% \TabItemI{Verhalten am Unfallort} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Koordination mit anderen Einsatzorganisationen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Absperrmaßnahmen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Sofortmaßnahmen}
% & 
% \#\#\# %\FullRef{topic:}
% \\\toprule
% \TabHeading{Angewandte Psychologie und Stressbewältigung}  
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:]  4 
% \item[Lehrkraft:]  Fachkompe- tente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% \TabItemI{Belastung, Anforderung, Beanspruchung, workflow}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Überforderung, Unterforderung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Beanspruchungsfolgen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Stressursachen, -entstehung und -faktoren}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Stressauswirkung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Früherkennung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Grundsätze der Stressvermeidung}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Maßnahmen zur Verhütung und Verminderung von Beanspruchungsfolgen}
% & 
% \#\#\# %\FullRef{topic:}
% \\
% Psychische Betreuung von Kranken/Verletzten
% & 
% 
% \\
% \TabItemI{Gesprächsführung} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Vertrauensaufbau und Patienteninformation} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{psychische Belastungssyndrome} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{verwirrte Patienten} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Begleitung und Betreuung Sterbender} 
% & 
% \#\#\# %\FullRef{topic:}
% \\
% \TabItemI{Supervision}
% & 
% \#\#\# %\FullRef{topic:}
% \\
%  \\\toprule
% \TabHeading{Praktische Übungen ohne Patientenkontakt}  
% 
% 
% 
% \begin{inparadesc}
% \item[Stundenanzahl:] 16
% \item[Lehrkraft:] Lehrsanitäter, fachkompetente Person
% \end{inparadesc}
% 
% \\\midrule
% 
% - Regloser Notfallpatient  
% & 
% \TakeNotes
% \\
% - Kontrolle der  Lebensfunktionen  (erwachsener Notfallpatient)
% & 
% \TakeNotes
% \\
% - Notfalldiagnose Bewusstlosigkeit (stabile Seitenlage)
% & 
% \TakeNotes
% \\
% - Notfalldiagnose Atemstillstand (Beatmung)
% & 
% \TakeNotes
% \\
% - Notfalldiagnose Kreislaufstillstand (Beatmung und Herzmassage)
% & 
% \TakeNotes
% \\
% - Blutstillung (Fingerdruck, Abdrückstellen, Druckverband, Abbindung, Amputatsversorgung)
% & 
% \TakeNotes
% \\
% - Blutdruckmessung
% & 
% \TakeNotes
% \\
% - Schockbekämpfung (Lagerungsarten)
% & 
% \TakeNotes
% \\
% - Halswirbelsäulen- und Wirbelsäulentrauma 
% &
% --- ↓ --- 
% \\
% (Sturzhelmabnahme, 
% & 
% \TakeNotes
% \\
% Halswirbelsäulenschienung, 
% & 
% \TakeNotes
% \\
% Motorik-Durchblutung-Sensibilitätskontrolle, 
% & 
% \TakeNotes
% \\
% Body-Check, 
% & 
% \TakeNotes
% \\
% Umgang mit Schaufeltrage, 
% & 
% \TakeNotes
% \\
% Umgang mit Vakuummatratze, 
% & 
% \TakeNotes
% \\
% Sandwich-Technik)
% & 
% \TakeNotes
% \\
% - Extremitätentrauma 
% & 
% --- ↓ --- 
% \\
% (Motorik-Durchblutung-Sensibilitätskontrolle, 
% & 
% \TakeNotes
% \\
% Stiefelgriff,
% & 
% \TakeNotes
% \\
% Ruhigstellung, 
% & 
% \TakeNotes
% \\
% Schienung des Armes, 
% & 
% \TakeNotes
% \\
% Schienung des Beines, 
% & 
% \TakeNotes
% \\
% pneumatische Schiene, 
% & 
% \TakeNotes
% \\
% Vakuumschiene, 
% & 
% \TakeNotes
% \\
% Extensionsschiene)
% & 
% \TakeNotes
% \\
% - Verbandlehre
% & 
% \TakeNotes
% \\
% - Notfälle im Säuglings- und Kleinkindalter 
% &
% --- ↓ --- 
% \\
% (Kontrolle der Lebensfunktionen und lebensrettende Sofortmaßnahmen)
% & 
% \TakeNotes
% \\
% - An- und Auskleiden, Körperpflege und Hygiene, Nahrungs- und Flüssigkeitsaufnahme, Harn- und Stuhlentleerung,
%   Erbrechen
% & 
% \TakeNotes
% \\
% - Ergonomische und schonende Arbeitsweise (Richtiges Heben und Tragen)
% & 
% \TakeNotes
% \\
% - Handhabung der in Einsatzfahrzeugen zu verwendenden Geräte  (insbesondere
% Krankentrage, Tragsessel,  Sauerstoffgeräte, Kommunikationseinrichtungen) sowie
% die  Handhabung von Rollstühlen und Gehhilfen
% 
% \\\bottomrule
% 
% \end{supertabular}
% 
% 
% 
% 
% 
% 
% 
% 
% 
% 

















































% \clearpage

\begin{gWideParEnv}


\begin{multicols}{2}

Ausbildungsziele für österreichische Rettungssanitäter gem. der \emph{Verordnung
der Bundesministerin für Gesundheit und Frauen über die Ausbildung zum
Sanitäter - Sanitäter-Ausbildungsverordnung - San-AV StF: BGBl. II Nr. 420/2003
-- Anlage 1, Modul I}.

\begin{enumerate}
  
  \item \TabHeading{Erste Hilfe und erweiterte Erste Hilfe}
  
  (Bestandteil des Sachgebiets 'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:] 14
    \item[Lehrkraft:] Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
    Facharzt, Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  \begin{enumerate}
    \item
    Notwendigkeit, Verpflichtung der Hilfeleistung
    \\
    \#\#\# %\FullRef{topic:}
    \item
    Unfallverhütung
    \\
    
    \item
    Rettungskette
    \\
    \noindent\FullRef{topic:rettungskette}
    \item
    Gefahrenzonen
    \\
    \noindent\FullRef{topic:gefahrenzone}
    \item
    Bergen (Wegziehen, Rautekgriff aus dem  Auto, Sturzhelm)
    \\
    \FullRef{topic:wegziehen},
    \FullRef{topic:rautekgriff}, 
    \FullRef{topic:sturzhelmabnahme}
    \item
    Notfallpatient, Notfalldiagnose, Notfallhilfe
    \\
    \noindent\FullRef{topic:notfallpatient}\\    
    \FullRef{topic:notfalldiagnose}\\   
    \FullRef{topic:notfallhilfe}
    \item    Kontrolle der Lebensfunktionen
    \\
    \noindent\FullRef{topic:lebensfunktionen-kontrolle}
    \item    Notfalldiagnose Bewusstlosigkeit
    \\
    \FullRef{topic:notfalldiagnose-bewusstlosigkeit}
    \item    Notfalldiagnose  Atem-Kreislaufstillstand
    \\
    \noindent\FullRef{topic:notfalldiagnose-atem-kreislaufstillstand}
    \item    Starke Blutung (Blutstillung, Fingerdruck,  Druckverband, Abbindung)
    \\
    \noindent\FullRef{topic:}
    \item    Schock (Schockbekämpfung,  Lagerungen)
    \\
    
    \item    Wunden und Wundverbände (Tierbisse, Insektenstiche, Verätzungen,
    Verbrennungen, Erfrierungen, Unterkühlung) 
    \\
    
    \item    Quetschungen
    \\
    
    \item    Gelenksverletzungen
    \\
    Verstauchung: \FullRef{topic:verstauchung-eh},
    \\
    Verrenkung: \FullRef{topic:verrenkung-eh}
    \item
    Knochenbrüche, allgemein
    \\
    \noindent\FullRef{topic:knochenbruch-eh}
    \item
    Brustkorbverletzungen
    \\
    
    \item    Plötzlich auftretende Erkrankungen
    \\
    
    \item    Vergiftungen
    \\
    
    \item    Transport
    \\
    
  \end{enumerate}
  \item \TabHeading{Hygiene}
  
  (Bestandteil des Sachgebiets 'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:] 2
    \item[Lehrkraft:] Arzt für Allgemeinmedizin, approbierter Arzt,
    Facharzt, Angehörige des gehobenen Dienstes für Gesundheits- und Krankenpflege,
    Hygienefachkraft,  Lehrsanitäter
  \end{inparadesc}
  
  \begin{enumerate}
    \item    Persönliche Hygiene
    \\
    \FullRef{topic:persoenliche-hygiene}
    \item    Grundbegriffe der Desinfektion
    \\
    \FullRef{topic:desinfektion-grundzuege}
    \item    Grundbegriffe der  Sterilisation
    \\
    \FullRef{topic:sterilisation-grundzuege}
    \item    Entsorgung von infektiösem Abfall
    \\
    \FullRef{topic:entsorgung-infektioeser-abfall}
    \item    Allgemeine Infektionslehre
    \\
    Erreger: \FullRef{topic:infektion-erreger},
    
    Übertragungswege: \FullRef{topic:infektion-uebertragungswege},
    
    Kreuzinfektion: \FullRef{topic:kreuzinfektion}.
    \item
    Vorgehen bei Verletzungen des Personals
    \\
    Nadelstichverletzungen: \FullRef{topic:nadelstichverletzung}
    \item    Hygienemaßnahmenplan
    \\
    \FullRef{topic:hygieneplan}
    \item    Infektionstransport
    \\
    \Abk{MRSA} und andere resistente Keime: \prettyref{m:mrsa-transport}
  \end{enumerate}
  \item \TabHeading{Berufsspezifische rechtliche Grundlagen}  ~
  
  
  
  \begin{inparadesc}
    \item[Stundenanzahl:] 3
    \item[Lehrkraft:]  Jurist, fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Aufgaben und Kompetenzen des  Rettungssanitäters
    \\
    \#\#\# %\FullRef{topic:}
    \item    Dokumentation im Rettungswesen (Einsatzprotokoll, Leitstellendokumentation, Transportnachweis)
    \\
    \#\#\# %\FullRef{topic:}
    \item    Hilfs- und Rettungswesen
    \\
    \#\#\# %\FullRef{topic:}
    \item    Straßenverkehrsordnung
    \\
    \noindent\FullRef{topic:strassenverkehrsordnung}
    \item    Patientenrechte
    \\
    \noindent\FullRef{topic:patientenrechte}
    \item    Grundlagen des Haftungsrechtes
    \\
    \noindent\FullRef{topic:haftungsrecht}
    \item    Unterbringungsgesetz
    \\
    \noindent\FullRef{topic:unterbringungsgesetz}
    \item    Reversfähigkeiten und Effekten
    \\
    \noindent\FullRef{topic:reversfähigkeit}\FullRef{topic:effekte}
    \item    Mitnahme von Begleitpersonen
    \\
    \noindent\FullRef{topic:begleitpersonen-mitnahme}
  \end{enumerate}
  
  \item \TabHeading{Anatomie und Physiologie}
  
  (Bestandteil des Sachgebiets 'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:] 4
    \item[Lehrkraft:]  Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
    Facharzt, Turnusarzt,   Angehörige  des  gehobenen  Dienstes für Gesundheits-
    und Krankenpflege, Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}
    \item    Blutkreislauf - Grundzüge
    \\
    \FullRef{topic:blutkreislauf}
    \item    Gliedmaßen - Grundzüge
    \\
    Obere Extremität:
    \FullRef{topic:schulterguertel},
    \FullRef{topic:obere-extremitaet},
    \\
    untere Extremität:
    \FullRef{topic:beckenguertel},
    \FullRef{topic:untere-extremitaet}
    \item    Haut -- Grundzüge
    \\
    \FullRef{topic:haut}
    \item    Schädel und Rumpf - Grundzüge Skelett
    \\
    Schädel: \FullRef{topic:schaedel}
    \item    Brustkorb - Grundzüge
    \\
    \FullRef{topic:thorax}
    \item    Bauchraum - Grundzüge
    \\
    Gastro-intestinaltrakt: \FullRef{topic:gastrointestinaltrakt}, andere Bauchorgane:
    \FullRef{topic:sonstige-bauchorgane}.
  \end{enumerate}\item \TabHeading{Störungen der  Vitalfunktionen  und Regelkreise und zu setzende Maßnahmen}
  
  (Bestandteil des  Sachgebiets  'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  8
    \item[Lehrkraft:]  Notarzt, Arzt für Allgemein- medizin, approbierter Arzt,
    Facharzt,  Lehr- sanitäter,  fachkompe- tente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Definition Vitalfunktion
    \\
    \noindent\FullRef{topic:vitalfunktionen-definition}
    \item    NACA-Schema
    \\
    \FullRef{topic:naca}
    \item    Bewusstsein
    \\
    (Bewusstseinsstörungen, Bewusstlosigkeit)
    \\
    \noindent Bewusstsein
    allgemein: \FullRef{topic:bewusstsein};
    Bewusstseinsstörungen:
    \FullRef{topic:bewusstseinsstoerung},
    \\
    Bewusstlosigkeit: \FullRef{topic:bewusstlosigkeit}
    \item
    Atmung
    \begin{enumerate}
      \item      (Atemstörungen,
      \\
      \noindent\FullRef{topic:atemstörungen}
      \item      Atemstillstand)
      \\
      \noindent\FullRef{topic:atemstillstand}
    \end{enumerate}
    \item    Kreislauf
    \begin{enumerate}
      \item      (Kreislaufstörungen,
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item    Kreislaufstillstand)
    \\
    \#\#\# %\FullRef{topic:}
    \item    Regelkreise
    \begin{enumerate}
      \item      Wärmehaushalt
      \\
      \#\#\# %\FullRef{topic:}
      \item      Wasser- und Elektrolythaushalt
      \\
      \FullRef{topic:wasser-elektrolythaushalt}
      \item      \Säure-Basen-Haushalt
      \\
      \FullRef{topic:saeure-basen-haushalt}
      \item      Stoffwechsel
      \\
      \FullRef{topic:stoffwechsel}
    \end{enumerate}
    \item
    Starke Blutung
    \\
    \#\#\# %\FullRef{topic:}
    \item
    Schock (Ursachen,
    \FullRef{topic:schock}
    \begin{enumerate}
      \item      Wirkung, Schockformen,
      \\
      \FullRef{topic:schock-arten}
      \item      Verlauf, Schockzeichen)
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item Akute Störung der Atmung
    \begin{enumerate}
      \item      Atembehinderung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Verlegung der Atemwege  durch Fremdkörper
      \\
      \#\#\# %\FullRef{topic:}
      \item      Verlegung durch Schwellung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Akute Atemnot
      \\
      \#\#\# %\FullRef{topic:}
      \item      Absaugung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Sauerstoff (Inhalationsrichtlinien, Dosierungsvorschriften)
      \\
      \FullRef{topic:sauerstoff}
      \item      Assistierte Beatmung (Indikationen, Durchführung)
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item    Feststellung des Todes
    \\
    \FullRef{topic:tod}
  \end{enumerate}\item \TabHeading{Notfälle bei verschiedenen Krankheitsbildern  und zu setzende Maßnahmen}
  
  (Bestandteil des Sachgebiets 'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  6
    \item[Lehrkraft:]  Notarzt, Arzt für Allgemeinmedizin, approbierter Arzt,
    Facharzt, Turnusarzt, Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  \begin{enumerate}\item
    Koma aus vorerst unbekannter Ursache
    \begin{enumerate}
      \item      (Schlaganfall,
      \\
      \FullRef{topic:insult}
      \item      Meningitis,
      \\
      \FullRef{topic:meningitis}
      \item      Diabetes,
      \\
      \FullRef{topic:diabetes-mellitus}
      \item      Vergiftung)
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item
    Krampfanfall
    \begin{enumerate}
      \item      (Epilepsie, Tetanie)
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item Akuter Gefäßverschluss an den Gliedmaßen
    \begin{enumerate}
      \item      Venenthrombose,
      \\
      \#\#\# %\FullRef{topic:}
      \item      Arterielle Embolie)
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item Pulmonale Notfälle
    \begin{enumerate}
      \item      Asthma bronchiale
      \\
      \FullRef{topic:asthma-bronchiale}
      \item      Lungenödem
      \\
      \FullRef{topic:lungenoedem}
      \item      Lungenembolie
      \\
      \FullRef{topic:lungenembolie}
      \item      Lungenentzündung
      \\
      \FullRef{topic:lungenentzuendung}
    \end{enumerate}
    \item Cardiale Notfälle
    \begin{enumerate}
      \item
      (Herz-Rhythmus-Störungen,
      \\
      \FullRef{topic:herzrhythmusstoerungen}
      \item
      Herzversagen,
      Linksherzschwäche,
      Rechtsherzschwäche,
      \\
      \FullRef{topic:herzinsuffizienz},
      \FullRef{topic:linksherzinsuffizienz},
      \FullRef{topic:rechtsherzinsuffizienz}
      \item
      Angina pectoris,
      \\
      \FullRef{topic:angina-pectoris}
      \item
      Herzinfarkt,
      \\
      \FullRef{topic:herzinfarkt}
      \item
      Hochdruckkrise)
      \\
      \FullRef{topic:hypertensive-kriese}, \FullRef{topic:hypertensiver-notfall}
    \end{enumerate}
    \item Allgemeinchirurgische Notfälle
    \begin{enumerate}
      \item
      Akutes Abdomen
      \\
      \FullRef{topic:akutes-abdomen}
      \item
      Gastritis,
      \\
      \FullRef{topic:gastritis}
      \item Ulcus,
      \\
      
      \item  Gastroenteritis,
      \\
      \FullRef{topic:gastroenteritis}
      \item  Pankreatitis,
      \\
      \FullRef{topic:pankreatitis}
      \item   Appendizitis,
      \\
      \FullRef{topic:appendizitis}
      \item   Gallenkolik,
      \\
      \FullRef{topic:gallenkolik}
      \item   Ileus,
      \\
      \FullRef{topic:ileus}
      \item    Mesenterialinfarkt,
      \\
      \FullRef{topic:mesenterialinfarkt}
      \item  Lebensmittelvergiftung,
      \\
      
      \item gastrointestinale Blutung)
      \\
      \FullRef{topic:gastrointestinale-blutung}
    \end{enumerate}
    \item Gynäkologische und urologische Erkrankungen
    \begin{enumerate}
      \item Nierenkolik,
      \\
      \FullRef{topic:nierenkolik}
      \item Nierenbeckenentzündung,
      \\
      \FullRef{topic:nierenbeckenentzuendung}
      \item Harnwegsinfekt,
      \\
      \FullRef{topic:harnwegsinfekt}
      \item akutes Harnverhalten,
      \\
      \FullRef{topic:harnverhalten}
      \item chronische Niereninsuffienz und Hämodialyse,
      \\
      Niereninsuffizienz: \FullRef{topic:niereninsuffizenz};
      
      Elektrolytstörungen durch
      Dialyse: \FullRef{topic:dialyse-elektrolyte};
      
      Dialyse als Therapie bei chronischer
      Niereninsuffizenz: \FullRef{topic:cni-therapie-dialyse};
      
      Dialysepatienten, besondere
      Gefahren: \FullRef{topic:dialysepatienten-gefahren}.
      \item extrauterine Gravidität,
      \\
      \FullRef{topic:eileiterschwangerschaft}
      \item Abortus,
      \\
      \FullRef{topic:abort}
      \item  Unterleibsblutungen,
      \\
      
      \item Ovarialtumore,
      \\
      
      \item Vergewaltigung (typische Verletzungen, psychische Betreuung)
      \\
      \FullRef{topic:vergewaltigung}
    \end{enumerate}
  \end{enumerate}
  
  \item \TabHeading{Spezielle Notfälle und zu setzende Maßnahmen  }
  
  (Bestandteil des Sachgebiets  'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  15
    \item[Lehrkraft:]   Notarzt, Arzt für  Allgemein-  medizin,  approbierter
    Arzt, Facharzt,   Turnusarzt,  Lehr-  sanitäter,  fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}
    \item Traumatologische Notfälle
    \begin{enumerate}
      \item      Schädel-Hirn-Verletzungen,
      \\
      \FullRef{topic:sht}
      \item      Hirnblutung
      \\
      \FullRef{topic:hirnblutung}
      \item      Hirndruck
      \\
      Hirndruckzeichen: \FullRef{topic:hirndruckzeichen};
      
      Hirnstammeinklemmung bei
      Hirndrucksteigerung: \FullRef{topic:hirndrucksteigerung-hirnstammeinklemmung};
      
      Ursachen:
      \FullRef{topic:hirndrucksteigerung-ursachen};
      
      Lagerung: \FullRef{topic:hirndrucksteigerung-lagerung}.
      \item
      Halswirbelsäulen- und  Wirbelsäulentrauma
      \\
      --- ↓ ---
      \item      (Sturzhelmabnahme,
      \\
      
      \item      HWS-Scheine Beschreibung (MP): \FullRef{topic:hws-schiene-beschreibung};
      
      Durchführung: \FullRef{topic:hws-schiene-anwendung}
      \\
      
      \item      Motorik-Durchblutung-Sensibilitätskontrolle,
      \\
      
      \item      Body-Check,
      
      Umgang mit  Schaufeltrage und Vakuummatratze,
      
      Sandwich-Technik)
      \item Extremitätentrauma
      
      \begin{enumerate}
        
        \item (Verletzungsarten,
        
        \item Motorik-Durchblutung-Sensibilitätskontrolle,
        
        \item Stiefelgriff,
        Prinzip der Schienung,
        
        Ruhigstellung,
        \item        Schienung des Armes
        \\
        \#\#\# %\FullRef{topic:}
        \item        Schienung des Beines
        \\
        \#\#\# %\FullRef{topic:}
        \item        Pneumatische Schiene
        \\
        \#\#\# %\FullRef{topic:}
        \item        Vakuumschiene
        \\
        Vakuumbeinschiene, Beschreibung: \FullRef{topic:}, Anwendung: \FullRef{topic:}
        \item        Extensionsschiene
        \\
        \#\#\# %\FullRef{topic:}
      \end{enumerate}
      \item Thoraxtrauma
      \begin{enumerate}
        \item
        offene, geschlossene Brustkorbverletzung
        \\
        \#\#\# %\FullRef{topic:}
        \item        Serienrippenbruch
        \\
        \FullRef{topic:serienrippenfraktur}
        \item        Pneumothorax
        \\
        \FullRef{topic:pneumothorax}
        \item        Spannungspneumothorax
        \\
        \FullRef{topic:pneumothorax}
      \end{enumerate}
      \item      Bauchtrauma
      \begin{enumerate}
        \item        offene, geschlossene Bauchverletzung
        \\
        \#\#\# %\FullRef{topic:}
        \item        Verletzung der Harn und Geschlechtsorgane
        \\
        \#\#\# %\FullRef{topic:}
        \item        Beckentrauma
        \\
        \FullRef{topic:beckentrauma}
      \end{enumerate}
      \item      Polytrauma (Definition, Prioritäten, Management)
      \\
      \FullRef{topic:polytrauma}
      \item      Wunden (mechanische, chemische, thermische)
      \\
      \FullRef{topic:wunden}
      \item      Dekubitus Prophylaxe und Lagerung bei Dekubitus
      \\
      \FullRef{topic:}
      \item      Verbandlehre
      \\
      \FullRef{topic:}
      \item      Akut auftretende Blutungen
      \\
      \FullRef{topic:}
      \item      Vergiftung (Ursachen und Verdacht, Aufnahmearten)
      \\
      \FullRef{topic:}
    \end{enumerate}
    \item Psychiatrische Notfälle
    \begin{enumerate}
      \item      (Suizid,
      \\
      \FullRef{topic:suizid}
      \item      Psychose
      \\
      \FullRef{topic:psychose}
      \item      Suchterkrankungen und Entzugssyndrom
      \\
      \FullRef{topic:sucht}, \FullRef{topic:entzugssysndrom}
      \item      Depression
      \\
      \FullRef{topic:depression}
      \item      Manie
      \\
      \FullRef{topic:manie}
    \end{enumerate}
    \item
    Schwangerschaft und Geburt
    \begin{enumerate}
      \item      Notfälle in der Schwangerschaft
      \\
      \FullRef{topic:}
      \item      Geburt
      \\
      \FullRef{topic:geburt}
      \item      Geburtskomplikationen
      \\
      \FullRef{topic:}
      \item      Versorgung des Neugeborenen
    \end{enumerate}
    \item Notfälle im Säuglings- und Kleinkindalter
    \begin{enumerate}
      \item      anatomische und physiologische Besonderheiten
      \\
      \FullRef{topic:}
      \item      Kontrolle der Lebensfunktionen und lebensrettende Sofortmaßnahmen
      \\
      \FullRef{topic:}
      \item      Krampfanfälle
      \\
      Allgemein: \FullRef{topic:krampfanfaelle}, Fieberkrampf: \label{topic:fieberkrampf}
      \item      Pseudokrupp
      \\
      \FullRef{topic:laryngitis}
      \item      Epiglottitis
      \\
      \FullRef{topic:epiglottitis}
      \item      Keuchhusten
      \\
      \FullRef{topic:}
      \item      plötzlicher Kindstod
      \\
      \FullRef{topic:sids}
    \end{enumerate}
  \end{enumerate}
  \item \TabHeading{Defibrillation mit halbautomatischen Geräten}
  
  (Bestandteil des Sachgebiets  'Sanitätshilfe')
  
  \begin{inparadesc}
    \item[Stundenanzahl:]   8
    \item[Lehrkraft:]   Notarzt,  Arzt für Allgemeinmedizin,  approbierter Arzt,
    Facharzt, Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}
    \item Der halbautomatische Defibrillator
    \item Handhabung eines halbautomatischen Defibrillators
    \item Gerätemanagement während der Reanimation
    \item Erfolgskontrolle
  \end{enumerate}
  \item \TabHeading{Gerätelehre und Sanitätstechnik }
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  12
    \item[Lehrkraft:]   Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Medizinproduktegesetz - Information
    \\
    \FullRef{topic:medizinproduktegesetz}
    \item    Bergungs- und Lagerungstechniken
    \begin{enumerate}
      \item      Bergetuch
      \\
      \FullRef{topic:bergetuch}
      \item      Einheitskrankentrage
      \\
      \#\#\# %\FullRef{topic:}
      \item      Tragsessel
      \\
      \FullRef{topic:tragsessel}
      \item      Fahrtrage
      \\
      \FullRef{topic:fahrtrage}
      \item      Rollstuhl
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item    Einsatzfahrzeug
    \\
    \#\#\# %\FullRef{topic:}
    \item    Beatmungsbeutel
    \\
    \FullRef{topic:beatmungsbeutel}
    \item    Absauggeräte
    \\
    \FullRef{topic:absaugung}
    \item    Sauerstoff
    \\
    \FullRef{topic:sauerstoff}
    \item    Infusionen und
    \\
    \#\#\# %\FullRef{topic:}
    \item    Infusionsgeräte
    \\
    \#\#\# %\FullRef{topic:}
    \item    Blutdruckmessung
    \\
    \FullRef{topic:rr-messung}
    \item    Stabilisierungs- und Schienungstechniken
    \begin{enumerate}
      \item      Stabilisierung der Halswirbelsäule
      \\
      \#\#\# %\FullRef{topic:}
      \item      Schaufeltrage
      \\
      Allg.: \FullRef{topic:schaufeltrage}, Anwendung:
      \FullRef{topic:schaufeltrage-anwendung}
      \item      Vakuummatratze
      \\
      Allg.: \FullRef{topic:vakuummatratze}, Anwendung:
      \FullRef{topic:vakuummatratze-anwendung}
    \end{enumerate}
    \item    Geburtenkoffer
    \\
    \#\#\# %\FullRef{topic:}
    \item    Transportinkubator
    \\
    \#\#\# %\FullRef{topic:}
  \end{enumerate}
  
  \item \TabHeading{Rettungswesen}
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  4
    \item[Lehrkraft:]   Fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Rechtliche Grundlagen
    \\
    \#\#\# %\FullRef{topic:}
    \item    Zusammenarbeit mit  anderen Organisationen
    \\
    \#\#\# %\FullRef{topic:}
    \item    Einsatzarten
    \\
    \#\#\# %\FullRef{topic:}
    \item    Transport- und Fahrzeugarten (Land,  Wasser, Luft)
    \\
    \#\#\# %\FullRef{topic:}
    \item    Normen, persönliche Schutzausrüstung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Fahrzeugausstattung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Rettungskette
    \\
    \#\#\#
    \item    Hilfsfrist
    \\
    \#\#\# %\FullRef{topic:}
    \item    Dienststellennetz
    \\
    \#\#\# %\FullRef{topic:}
    \item    Personal im Rettungsdienst
    \\
    \#\#\# %\FullRef{topic:}
    \item    Notarztsysteme
    \\
    \#\#\# %\FullRef{topic:}
    \item    Leitstelle
    \\
    \#\#\#
    \item    Kommunikationsarten
    \\
    \#\#\# %\FullRef{topic:}
    \item    Gefahren an der Einsatzstelle
    \\
    \#\#\# %\FullRef{topic:}
    \item    Gefahrguteinsätze
    \\
    \#\#\#
    \item    Sondertransporte
    \\
    \#\#\#
    
  \end{enumerate}
  \item \TabHeading{Katastrophen, Großschadensereignisse, Gefahrgutunfälle}
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  4
    \item[Lehrkraft:]  Fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Katastrophen
    \begin{enumerate}
      \item
      Rechtliche Grundlagen,  Geltungsbereiche
      \\
      \#\#\# %\FullRef{topic:}
      \item      Arten der Katastrophen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Phasen der Katastrophenbewältigung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Katastrophenhilfseinheiten
      \\
      \#\#\# %\FullRef{topic:}
      \item      Führungsorganisation
      \\
      \#\#\# %\FullRef{topic:}
      \item      personelle, materielle und finanzielle Vorsorge
      \\
      \#\#\# %\FullRef{topic:}
      \item      Einsatzgrundsätze
      \\
      \#\#\# %\FullRef{topic:}
      \item      generelle Einsatzrichtlinien
      \\
      \#\#\# %\FullRef{topic:}
      \item      Grundzüge der Triage
      \\
      \FullRef{topic:triage}
    \end{enumerate}
    \item
    Großschadensereignisse
    \begin{enumerate}
      \item
      Rechtliche Grundlagen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Einstufung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Alarmierung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Schadensraum
      \\
      \#\#\# %\FullRef{topic:}
      \item      Schadensplatz, Sicherheitseinrichtungen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Organisation beim Großunfall
      \\
      \#\#\# %\FullRef{topic:}
      \item      Grundzüge der Triage
      \\
      \FullRef{topic:triage}
      \item      österreichisches Patientenleitsystem
      \\
      \FullRef{topic:pls}
      \item      Material und Ausrüstung
      \\
      \#\#\# %\FullRef{topic:}
      \item      Kommunikation
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
    \item
    Gefahrgutunfälle
    \begin{enumerate}
      \item      Arten von Gefahrgutunfällen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Gefahrzettel
      \\
      \FullRef{topic:gefahrzettel}
      \item      Gefahrsymbole
      \\
      \FullRef{topic:gefahrsymbole}
      \item      Warntafel
      \\
      \FullRef{topic:warntafel}
      \item
      Verhalten am Unfallort
      \\
      \#\#\# %\FullRef{topic:}
      \item      Koordination mit anderen Einsatzorganisationen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Absperrmaßnahmen
      \\
      \#\#\# %\FullRef{topic:}
      \item      Sofortmaßnahmen
      \\
      \#\#\# %\FullRef{topic:}
    \end{enumerate}
  \end{enumerate}\item \TabHeading{Angewandte Psychologie und Stressbewältigung}
  
  
  
  \begin{inparadesc}
    \item[Stundenanzahl:]  4
    \item[Lehrkraft:]  Fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}\item
    
    Belastung, Anforderung, Beanspruchung, workflow
    \\
    \#\#\# %\FullRef{topic:}
    \item    Überforderung, Unterforderung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Beanspruchungsfolgen
    \\
    \#\#\# %\FullRef{topic:}
    \item    Stressursachen, -entstehung und -faktoren
    \\
    \#\#\# %\FullRef{topic:}
    \item    Stressauswirkung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Früherkennung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Grundsätze der Stressvermeidung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Maßnahmen zur Verhütung und Verminderung von Beanspruchungsfolgen
    \\
    \#\#\# %\FullRef{topic:}
    \item    Psychische Betreuung von Kranken/Verletzten
    \\
    
    \item    Gesprächsführung
    \\
    \#\#\# %\FullRef{topic:}
    \item    Vertrauensaufbau und Patienteninformation
    \\
    \#\#\# %\FullRef{topic:}
    \item    psychische Belastungssyndrome
    \\
    \#\#\# %\FullRef{topic:}
    \item    verwirrte Patienten
    \\
    \#\#\# %\FullRef{topic:}
    \item    Begleitung und Betreuung Sterbender
    \\
    \#\#\# %\FullRef{topic:}
    \item    Supervision
    \\
    \#\#\# %\FullRef{topic:}
  \end{enumerate}
  \item \TabHeading{Praktische Übungen ohne Patientenkontakt}
  
  \begin{inparadesc}
    \item[Stundenanzahl:] 16
    \item[Lehrkraft:] Lehrsanitäter, fachkompetente Person
  \end{inparadesc}
  
  \begin{enumerate}
    \item Regloser Notfallpatient
    \\
    \TakeNotes
    \item    Kontrolle der  Lebensfunktionen  (erwachsener Notfallpatient)
    \\
    \TakeNotes
    \item    Notfalldiagnose Bewusstlosigkeit (stabile Seitenlage)
    \\
    \TakeNotes
    \item    Notfalldiagnose Atemstillstand (Beatmung)
    \\
    \TakeNotes
    \item    Notfalldiagnose Kreislaufstillstand (Beatmung und Herzmassage)
    \\
    \TakeNotes
    \item    Blutstillung (Fingerdruck, Abdrückstellen, Druckverband, Abbindung, Amputatsversorgung)
    \\
    \TakeNotes
    \item    Blutdruckmessung
    \\
    \TakeNotes
    \item    Schockbekämpfung (Lagerungsarten)
    \\
    \TakeNotes
    \item
    Halswirbelsäulen- und Wirbelsäulentrauma
    \\
    \begin{enumerate}
      \item      (Sturzhelmabnahme,
      \\
      \TakeNotes
      \item      Halswirbelsäulenschienung,
      \\
      \TakeNotes
      \item      Motorik-Durchblutung-Sensibilitätskontrolle,
      \\
      \TakeNotes
      \item      Body-Check,
      \\
      \TakeNotes
      \item      Umgang mit Schaufeltrage,
      \\
      \TakeNotes
      \item      Umgang mit Vakuummatratze,
      \\
      \TakeNotes
      \item      Sandwich-Technik)
      \\
      \TakeNotes
    \end{enumerate}
    \item
    Extremitätentrauma
    \begin{enumerate}
      \item
      (Motorik-Durchblutung-Sensibilitätskontrolle,
      \\
      \TakeNotes
      \item      Stiefelgriff,
      \\
      \TakeNotes
      \item      Ruhigstellung,
      \\
      \TakeNotes
      \item      Schienung des Armes,
      \\
      \TakeNotes
      \item      Schienung des Beines,
      \\
      \TakeNotes
      \item      pneumatische Schiene,
      \\
      \TakeNotes
      \item      Vakuumschiene,
      \\
      \TakeNotes
      \item      Extensionsschiene)
      \\
      \TakeNotes
    \end{enumerate}
    \item
    Verbandlehre
    \\
    \TakeNotes
    \item Notfälle im Säuglings- und Kleinkindalter
    \begin{enumerate}
      \item
      (Kontrolle der Lebensfunktionen und lebensrettende Sofortmaßnahmen)
      \\
      \TakeNotes
    \end{enumerate}
    \item An- und Auskleiden, Körperpflege und Hygiene, Nahrungs- und Flüssigkeitsaufnahme, Harn- und Stuhlentleerung,
    Erbrechen
    \\
    \TakeNotes
    \item Ergonomische und schonende Arbeitsweise (Richtiges Heben und Tragen)
    \\
    \TakeNotes
    \item Handhabung der in Einsatzfahrzeugen zu verwendenden Geräte  (insbesondere
    Krankentrage, Tragsessel,  Sauerstoffgeräte, Kommunikationseinrichtungen) sowie
    die  Handhabung von Rollstühlen und Gehhilfen
    
    
  \end{enumerate}
\end{enumerate}

\end{multicols}

\end{gWideParEnv}