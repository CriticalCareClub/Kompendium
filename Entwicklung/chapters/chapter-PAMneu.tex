...
verwendet werden und die klassischen "`NACA-II-Patienten"' nicht in den
...
\Commentary[Treat first what kills first]
{%
Auch unser Modell folgt dem Grundsatz "Treat first what kills first" und
die traditionelle Reihenfolge "`Atemwege -- Atmung -- Kreislauf"' wird
natürlich beibehalten. Die Punkte A, B und C entsprechen damit den
Punkten der Primary Survey in den meisten anderen Konzepten (simple
Untersuchungen nach dem "`Sehen-hören-fühlen-Konzept"'). 
\ParBugfix
...
Rekapzeit}: Unter "`lokale Durchblutungsstörung"' fallen
auch in Folge von Kälte minderdurchblutete Extremitäten, also "`kalte Finger"'.}
oder Zentralisierung hin. Liegt ein Verdacht auf ein Trauma
vor, wird nach starken Blutungsquellen gesucht. Dazu werden
Bauch, Becken und Oberschenkel inspiziert und abgetastet.
...
\begin{itemize}
  ...
  \item Rekap-Zeit 
  \item Radialispuls 
  \item[{\tiny\color{Indigo}\ovalbox{Tr.:}}] Inspektion
  und Abtasten von
  \begin{inparaenum}
    \item \E{Bauch},
    \item \E{Becken} und
...
gebräuchlichen Begriff "'ständige Verlaufskontrolle"'. Die
Formulierungen "`regelmäßig"' und "`in angemessenem Umfang"'
...
\marginpar{\includegraphics[width=\linewidth]{./images/teaser/minen_hinweis2.jpg}}
...
\subsection{Strategie? Prioritäten setzen und Plan zurechtlegen}
...
Bei jeden Patienten ist zu beurteilen:
...
bzw. D, E
&
Eine vitale Bedrohung ergibt sich aufgrund von:
\begin{itemize}
  \item Diagnosen, welche an sich bedrohlich sind
  \item Alarmsymtomen
  \item Einschätzung stabil / instabil
\end{itemize}
&
Ergibt sich aus der Verdachtsdiagnose
...
\Layout{2}
{Transportpriorität}
{Bei manchen Krnakheitsbildern bringt eine ausgiebige
Behandlung vor Ort keinen Vorteil, andere hingegen
profitieren stark von erweiterten Maßnhamen \E{vor} dem
Transport. Ein Notarzt wird daher bei Bedarf nach Abwägen
...
% werden}!
% 
% {\tiny(Eingeklammerte) Punkte werden nur durchgeführt, wenn
% sie \E{situationsentsprechend} sind. "`Typische
...
\MassnahmenWide{Standardmaßnahmen}{Einschätzungsblock neu}
{\label{m:ersteinschaetzung}\label{m:einschaetzungsblock}}
{%
\CorrectionItemizeBox\renewcommand{\itemhook}{%
%   \setlength{\topsep}{0pt}%
...
wenn sie \E{situationsentsprechend} sind. "`Typische
Sofortmaßnahmen"' sind als \E{Beispiele} zu verstehen.}
\end{center}
}
\end{minipage}
...
angemessenen} Art und Weise durchzuführen. Im begründeten Ausnahmefall kann es
allerdings notwendig oder sinnvoll sein, dass manche Maßnahmen unterbleiben oder
aufgeschoben werden (Auf Grund des Selbstschutzes, "`Aufklärung"' eines bewusstlosen Patienten,
\ldots{}) oder angepasst werden müssen.
