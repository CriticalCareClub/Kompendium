...
\E{nach der Kontrolle des Bewusstseins} der \E{Notarzt
angefordert} und erst danach mit der \E{lebensrettenden
Sofortmaßnahme} begonnen. Die genaue Vorgehensweise und die
Techniken werden im Folgenden detailliert beschrieben.
\bigskip }{
\begin{itemize}
  \item Nach der Kontrolle des Bewusstseins Notarzt anfordern.
...
\begin{PictureSeriesWide}{}{Die drei Säulen der Basiseanimation}
\PictureSeriesRow{3}
...
Empfehlungen des \I{European Resuscitation Council} (\I{ERC}) 
und ist in der Bilderserie \FullPrettyRef{fig:hdm} illustriert:
%\begin{itemize}
...
%\end{itemize}
%
Nachdem der Patient auf eine \E{harte Unterlage} (ggf. auf den Boden) gebracht wurde, muss der Brustkorb frei gemacht und der \EE{Druckpunkt} für die Herzdruckmassage aufgesucht werden. Dieser liegt in der \E{Mitte des Brustkorbs}, d.\,h. in der unteres Hälfte des Brustbeins. Auf diesem Punkt wird ein \E{Handballen} des Helfers gelegt. Der Handballen der anderen Hand kommt auf die erste Hand, direkt über dem Druckpunkt. Die Finger werden ineinander verschränkt. Der für die \E{Kompression} notwendige Druck wird erzeugt, indem sich der Helfer über den Patient beugt, die Arme durchstreckt und mit Hilfe seines eigenen Gewichts \E{gerade nach unten} drückt. Dabei soll eine \EE{Eindrücktiefe} von  \E{\VonBis{5}{6}\CM*} erreicht werden.\footnote{Beim durchschnittlichen Erwachsenen entspricht das ungefähr \nicefrac{1}{3} des Brustkorbes.} Nach der Kompression folgt die \E{vollständige Entlastung} des Brustkorbs ehe eine neuerliche Kompression beginnt.\footnote{Vollständige Entlastung bedeutet, dass der Helfer keinerlei Druck mehr auf den Brustkorb des Patienten ausübt. Die Handballen werden also in ihre Ausgangslage zurückgeführt, wobei sie gerade nicht vom Brustkorb abheben. Sollte es versehentlich passieren, dass der Helfer den Kontakt zum Patient verliert, muss der Druckpunkt neu aufgesucht werden.} Die Abfolge von Druck- und Entlastungsphasen soll \E{harmonisch} erfolgen, d.\,h. dass beide Phasen gleich lange dauern und nicht ruckartig ablaufen \cite{Erc2010Sec2}. Die \EE{Druckfrequenz} (\enquote{Arbeitsgeschwindigkeit}) der Herzdruckmassage beträgt \E{mindestens 100 Massagen pro Minute}.\footnote{Die Herzdruckmassage soll nicht schneller als mit 120 Massagen pro Minute durchgeführt werden.}
...
\begin{PictureSeriesWide}{}{Bilderserie: \EE{Herzdruckmassage} \SourcePicture{Hirtler}\label{fig:hdm}}
\PictureSeriesRow{3}
...
{{\TxBeschreibung}}%1 Überschrift
{%
\label{topic:defibrillation-bls}%
...
\TextSlide{Verwendung eines AED}{
...
\begin{itemize}
  ...
  eingesetzt werden!
  \item \StepHeading{Kinder}: Viele Geräte dürfen bei {Kindern} erst ab einem 
  ...
  {Sauerstoffquellen} so weit wie möglich von den
...
%   nicht defibrilliert werden. % ex mit ERC 2010
  \item \StepHeading{Analyseprobleme}: Folgende Umstände können zu einem \E{Abbruch der
  ...
  da sonst die Analyse beeinflusst werden kann.
...
\begin{itemize}
  \item \StepHeading{Szeneüberblick mit (Selbst-)Schutz}: wie bei BLS
  \item \StepHeading{Eindruck/Hauptbeschwerde}: wie BLS, zusätzlich
  Abschätzen des Alters
  \item \StepHeading{Ansprechbarkeit prüfen, Bewusstsein beurteilen}:
  wie BLS, \E{kindgerecht} ansprechen
  \item Ist der \StepHeading{Patient ansprechbar} folgt das Vorgehen
  den allgemeinen Regeln des Patientenmanagements.
  \item Ist der \StepHeading{Patient nicht ansprechbar} wird ein
  Notarzt angefordert. Idealerweise deligiert man diese
  Aufgabe an einen zusätzlichen Helfer.
  \item \StepHeading{Atemweg kontrollieren}: Es wird die
  ...
  \item
  \StepHeading{Atmung prüfen}: Wie beim BLS wird für die Dauer von
  10\Sec* die Atmung des Patienten durch \E{Sehen},
  \E{Höhren} und \E{Fühlen} beurteilt. 
  \item \StepHeading{Beurteilung} ob die \StepHeading{Atmung normal} ist: wie
  BLS
  \item Liegt \StepHeading{normale Atmung} vor, wird der Patient in
  die stabile Seitenlage gedreht. Danach folgen alle
  weiteren {\TxMassVit}.
  \item Danach werden \StepHeading{5 Initialbeatmungen} verabreicht.
  \item Es wird erneut nach \StepHeading{Lebenszeichen} gesucht. Wenn
...
\PictureWide
[]% Float
{Basic Life Support mit AED.}% Titel
{}% Beschreibung
{\label{tab:algorithmus-bls-aed}%
\protect\index{Basic Life Support}%
\protect\index{BLS|see{Basic Life Support}}}% Label
{Josef Emhofer, nach \cite{Erc-2015-Sektion-02-De}}% Quelle
{}% Lizenz
{./images/emhofer-josef-aass/ReanimationBls-2015-4}% Datei
{}% Breitenmodifizierer
...
\PictureWide
[]% Float
{Pediatric Life Support}% Titel
{}% Beschreibung
{\label{tab:algorithmus-pls}%
\protect\index{Pediatric Life Support}%
\protect\index{BLS|see{Pediatric Life Support}}}% Label
{Josef Emhofer, nach \cite{Erc2015Section06}}% Quelle
{}% Lizenz
{./images/emhofer-josef-aass/ReanimationPls-2015-1}% Datei
{}% Breitenmodifizierer
...
\begin{itemize}
  \item Eintreffen von \StepHeading{weiterer Hilfe}: Der Helfer
  ...
  erfolgen.
  \item \StepHeading{Wiedererlangung des Kreislaufes} 
  ...
  \item \StepHeading{Erschöpfung}: Sollte ein Ersthelfer all seine Kräfte
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item {\TxMassVit}
    \item ausführliche Untersuchung und Fremdanamnese
    \end{itemize}
  \item (Erschöpfung)
\end{itemize}
}
